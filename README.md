...
- [2026-08-20] **Massive Flat-File screener seam** - the value-screener's `_fetch_ohlcv` now reads a configured Massive Flat-File day-aggregates CSV first (`TRADINGAGENTS_MASSIVE_FLAT_PATH` / `massive_flat_path`), giving bulk-history ATR/ATR-pct/scan bases before the per-ticker vendor chain. Opt-in, >=15-row gate; hermetic-tested. Also validated a live end-to-end `batch.py` run to AAPL (Underweight) exercising the new Massive tools.
...
- **NOI + Flat Files** (Massive.com, plan-gated) — `massive_noi.py` (WebSocket NOI monitor app) and `massive_flat.py` (bulk OHLCV loader) are standalone utilities: the screener's `_fetch_ohlcv` reads a configured Flat-File day-aggregates CSV first (bulk ATR/scan bases), and a NOI monitor app consumes the WebSocket feed. NOI needs the Imbalances Expansion add-on; Flat Files need Starter+.
