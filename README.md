...
- [2026-08-21] **True portfolio CVaR (risk governor)** - the governor's daily tail budget can now come from the *weighted basket's* historical CVaR (`book_risk.portfolio_cvar`) instead of the single analyzed name: set `risk_basket_tickers` (comma list) + optional `risk_basket_weights` (k=v pairs or JSON) in config/env. Falls back to single-name when unconfigured/unresolvable.
