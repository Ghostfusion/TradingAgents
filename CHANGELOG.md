...
### Changed
**The canonical financials now carry the cash-flow statement, so P/CF and P/FCF stop being None for every name (2026-09-24).** `statement_parsing.fetch_ticker` absorbed three vendor payloads - fundamentals, the balance sheet and the income statement - and never called `get_cashflow`, while `ratios.compute_ratios` declares `operating_cashflow` and `capex` among the canonical line items it consumes. Every consumer therefore read `price_to_cash_flow` and `price_to_free_cash_flow` as None. Measured over the deepest 30 US decliners that day: P/CF present **0/30** while market cap, P/E and P/B were 30/30. Found by building a screener whose P/CF bound failed closed on all 30 names and emptied its whole funnel.

**Two keys in, from a whitelist, and only where they are missing.** The first cut absorbed the cash-flow payload into the merge like the other statements, ordering it first so last-writer-wins could not flip `net_income`'s pinned provenance. That was not enough: `absorb` matches rows by loose label aliases, and a cash-flow statement also carries "Long-term debt" and "Cash" rows, so AAPL's `total_debt`/`cash` moved until it reported *equity value is not positive: debt exceeds enterprise value* and **six screens failed** (`test_analysis_tools` x3, `test_growth_screens` x2, `test_v2_v5_wiring` x1). The merge is now a **whitelist** - `operating_cashflow` and `capex`, the two keys `compute_ratios` pairs with P/CF and P/FCF - filled only when absent, with provenance recorded as `get_cashflow`. Nothing else on that payload can reach the canonical dict, so no pre-existing value can move.

Cost: one additional vendor call per `fetch_ticker` call (per symbol per run), on the same annual chain the other statements use. Failing-first: removing the whitelisted fill fails `tests/test_statement_parsing.py::test_fetch_ticker_carries_the_cash_flow_leg` by name, with the source restored byte-identical.

Web impact: none. No tool, CLI flag or JSON shape changed, and the webapp's contract pair (`test_engine_contract.py`, `test_doc_claims.py`) passes unchanged. `fetch_ticker`'s returned dict gains the `operating_cashflow` and `capex` keys for callers that read it directly.

**A value screen now ends in the engine's own fundamental score, and it reports what its number is (2026-09-24).** `scripts/value_score_screen.py` applies the broker-panel criteria (market cap >= $10B, 0 < P/E TTM <= 33, P/B <= 9, P/S TTM <= 8, Price-to-Cash-Flow TTM <= 25) plus the owner's ROE >= 15%, 5-day change <= +3%, RSI(14) <= 55 and NYSE/Nasdaq gate, plus a same-day decline filter, then ranks the survivors by the `fundamental_score` composite and prints the ones clearing a `--score-min` cut (default 64).

**Why each stage sits where it does.** ONE EODHD bulk real-time call answers the day filter for the whole US market (`get_top_movers_symbols_eodhd`, ~18k rows, field `change_p`) - the only whole-market intraday source in the repo (moomoo's movers rank caps at 200 rows, Alpaca's batch endpoints drop `prevDailyBar`, yfinance's free screener caps at 50 rows). The EODHD symbol list supplies the common-stock and exchange gate in the same pass, which is why the moomoo screen is called with `exchanges=None` - its own gate costs one `get_stock_basicinfo` call per row. Only mcap / P-E / P-B are server-side (Screening V2); **no vendor in this repo serves P/S or P/CF** - EODHD's screener 403s on the plan and Massive's ratios endpoint 403s on free - so `ratios.compute_ratios`, their sole producer, computes them client-side, and a ratio that cannot be computed fails CLOSED rather than passing. The scoring pass costs ZERO further vendor calls: `fundamental_score` scores a whole cross-section at once, and the panel is the run's own screened set, the convention `scripts/value_screener.py` already uses.

**Two defects found by running it, both fixed in the same pass.** (1) `fetch_ticker` never calls `get_cashflow` - its payloads are fundamentals, balance sheet and income statement - so the canonical items carried no `operating_cashflow` and `price_to_cash_flow` came back `None` for EVERY name; measured over the deepest 30 decliners, P/CF was present 0/30 while market cap, P/E and P/B were 30/30. Because the gate fails closed, that one missing leg emptied the entire funnel. The tool now fetches the annual cash-flow statement once per name - through the screener's own `_CASHFLOW_CACHE`, parsed by `statement_parsing._canonicalize` - and injects it as `fin["operating_cashflow"]`. (2) The drop counts reached only `logger.info`, below the WARNING level, so the funnel was invisible on stdout and an empty result carried no reason at all. Every run now prints `[funnel] decliners N -> candidates M -> passed the ratio gates K (dropped: ... on P/E, P/B, P/S, P/CF, unfetchable)`. The report's DEFAULT output is the qualifying set alone - the owner's `--score-min` criterion governs it, so a name below the cut is not listed - and an empty result states that nothing cleared the cut and names the best few rather than printing an empty table; `--show-excluded` adds the below-cut and withheld tables, which the pre-filter had hidden entirely.

**A third defect, and the most consequential one: the panel must be wider than the survivors.** `factors.category_scores` refuses a peer set below `min_peers` - *"a z over a handful of names is noise, not a score"* - and the four `FundamentalScore` sub-score wrappers pass `min_peers = 8`. Measured with the 5 names that survived the ratio gates: at the engine default **nothing** is scored and every sub-score reports `floor: None`; at `min_peers=5` FQS/VS/FRS score all 5; at `min_peers=2` the composite appears (FSLR 83.3, KGC 83.3, GEN 41.7, XYL 25.0, SNX 16.7). **The floor is deliberately NOT lowered** - it is the engine's own guard against exactly that failure - so the panel is now the whole **fetched** cross-section (every name stage 2 pulled financials for, tens of names) and the survivors are read out of it. A panel built from the survivors alone yields no composite at all, and the engine's per-name reason (`peer set below the floor (5 < 8 names)`) is now surfaced instead of rendering as "no reason recorded".

**The report states what its own number is.** The composite carries `status=RESEARCH_ONLY` and is a tie-aware percentile with NO band table - the engine's own `basis` says so, and the nearest PUBLISHED sub-score edges are 60 and 65 - so a `>= 64` cut is a research cut, not an engine boundary; the percentile is relative to the panel it is given rather than to the market; the screen's four valuation anchors overlap the composite; and neither layer reaches `opportunity_score` or may gate a trade. `--no-moomoo` runs the EODHD-only path when OpenD is down (candidate set = the deepest `--limit` decliners, labelled a PARTIAL scan, with its measured size bias recorded in the plan), and `--offline-demo` exercises the scoring, threshold, withholding and band output on a synthetic panel with zero vendor calls.

Docs: `docs/implementation_plan_value_screen_score.md` records the two locked owner decisions (the EODHD bulk call stays as the day filter; stage 3's panel becomes the SEC EDGAR XBRL panel so the cut is market-relative), the per-stage vendor budget, the open TTM-vs-annual item, the panel build prerequisite and its per-date caching contract, the measured size bias of the fallback, and the verification plan.

Web impact: none. No tool, CLI flag or JSON shape changed.

**Every env-mapped key now lives in the live `.env`, not only the gates (2026-09-24).** The gates went in first; this pass copies the **96 missing non-gate value keys** - model names, directories, basket weights, tier maps - so the file a run reads names every switch the engine ships. **Each literal was serialized from the SHIPPED default**, read with a *cleaned* environment so the ambient shell could not masquerade as one, and then **proved to round-trip through the loader's own `_coerce` before anything was written**: 96 written, 0 skipped, 0 mismatches. Twenty of them carry no value (`NAME=`) and are **inert by design** - for those keys the empty value *is* the default, and `_apply_env_overrides` skips an empty value, which is exactly the behaviour the default wants. A list default is written as JSON and a dict as JSON; a string carrying a newline or a double quote is rejected rather than mangled by dotenv (none occurred).

**Proved through the real parser, not asserted.** The file parses cleanly: 330 names present, **0 of the 283 mapped names missing**, and **251 keys proven to round-trip through `dotenv_values` + `_coerce` back to their shipped default** - 32 inert, **0 mismatches**. The write was append-only.

**One defect, mine, caught by the guard written for it.** The first write read `.env` in *text* mode, so Python's universal-newline translation rewrote every CRLF in the file to LF - content-identical, but a whole-file EOL rewrite of the file this pass had promised not to touch. The prefix assertion written for exactly that reason flagged it; the line endings were restored to CRLF and the pre-block bytes now hash **bit-identically** to the state before the pass (`53a71216e68d`). The scratch tool reads and writes bytes now.

Web impact: none. No tool, CLI flag or JSON shape changed.

**Every engine gate now has a line in the operator's own `.env`, and the rule that keeps it that way is written down (2026-09-24).** A config key that exists in `DEFAULT_CONFIG` and `.env.example` but not in the live `.env` is invisible in the file a run actually reads - the operator cannot see that the switch exists - so the live `.env` was topped up from **43 of 111 gates to 111**, append-only: an entry already present was never touched, and every created line carries that key's `DEFAULT_CONFIG` value. Proved two ways: the original bytes are an exact **prefix** of the new file, and loading the result through the engine's real path (`load_dotenv()` then `_apply_env_overrides`) adds **zero** gates that differ from their default - the 35 that do differ are the ambient environment's own, unchanged by this. Non-gate keys were deliberately left alone (96 of the 167 value keys are still only in `.env.example`). **[SUPERSEDED 2026-09-24 - completed the same day]** Those 96 were then copied in with the same append-only discipline, once each default was proved to round-trip through the loader's own coercion; see the entry directly below and reported rather than silently filled, because their defaults do not all round-trip through the loader as text.

**The rule is now in the working agreement and in the gate contract.** `docs/AGENT_ONBOARDING.md` rule 8 requires any config key - gate or value - to be created in `.env` carrying its default when the name is absent, and **never overwritten**, because an existing line is the operator's decision rather than a default to be refreshed. `docs/gate_registry.md` §8 counts the live `.env` as a registration point (seven, formerly six) and reconciles the two lists that had drifted apart - this file had counted a `CHANGELOG.md` entry and omitted `.env.example`, while the adoption set's ground rules counted `.env.example` and the `REGISTRY` entry and omitted the CHANGELOG. The registration tool creates the `.env` entry as part of registration, skips one that is already there, and warns instead of guessing when a default is not a scalar. `load_dotenv()` runs without `override=`, so a written default can never mask an exported variable.

Web impact: none. No tool, CLI flag or JSON shape changed.

**A prompt edit is now attributable: the prompt is versioned as a first-class input, and the condition is recorded per run (2026-09-24).** N5 of the survey's adoption set (the T1 batch), behind `enable_prompt_condition_harness`, default off. Adopt the harness, not the framing - and the module says so: 2606.00061's `n = 72` per cell, its sign flips across context windows and its missing multiple-comparison control make it weak evidence for its own framing, so nothing here asserts that an effect exists. What lands is **versioning and attribution**: `prompt_version(prompt)` = `sha256(text)[:16]`, `condition_descriptor(label, {stage: prompt})` returns the versions, a `version_id` and a status, and every ledger row carries BOTH `prompt_condition` (the arm label) and `prompt_version` (the version id) - so a label that silently spans two prompt texts is detectable, and `condition_accuracy` flags `version_conflict` when one arm mixes versions.

**No pooling.** `condition_accuracy` groups scored rows by condition; rows with no condition or no scored outcome are reported under `unattributed` with counts and are excluded from `n_scored` and from every arm's rate, and an arm below `CONDITION_MIN_N = 5` reports `accuracy: None` with `below_min_n: True` rather than a rate over a handful of rows. A run whose condition was not recorded is therefore `unavailable`, never folded into the total - which is the card's acceptance clause.

`prediction_ledger.log_decision` gains a first-class `prompt_condition: str | None = None` parameter, written as an explicit null when absent, so the column exists on every row and no existing caller changes. **No prompt string changes anywhere in the engine**, which is the card's own instruction. The harness is offline by construction: `record_condition_run` is called by nothing on the live path - the decision-ledger writer site in `graph/trading_graph.py::_finalize_run_body` was deliberately **not** touched (ground rule 8) - because the comparison doubles LLM spend.

Failing-first, re-proved by the integrator against the final source: replacing `prompt_condition=str(label)` with `prompt_condition=None` in `record_condition_run` fails `tests/test_prompt_condition_harness.py::test_condition_recorded_per_run` by name, with the source restored byte-identical (sha256 `258b3341…` asserted in a `finally`).

Web impact: none. No tool, CLI flag or JSON shape changed.

**A cross-sectional fragility read now carries the calibration that makes it a probability rather than a score wearing a probability's name (2026-09-24).** R5 of the survey's adoption set (the T1 batch), behind `enable_forward_stress_probability`, default off. `market_breadth.forward_stress_probability(panel, *, gate)` aggregates monthly cross-sectional fragility over the tracked universe - return dispersion, the downside share (names below mean - 1sd) and **inverted** skew - into a one-month-ahead probability of entering a high-stress regime, the outcome rule being the equal-weight panel's next month below `FS_STRESS_QUANTILE = 0.2` of the panel's own monthly return history.

**The calibration is part of the output, not a report beside it.** `p_stress` IS the current fragility band's realized hit rate from `calibration.calibration_table` - the engine's one bin table, called directly and band-mapped by parsing its own label, so no second bin definition exists (rule 2) - and `calibration` is a dict whenever `p_stress` is a number and `None` on every refusal. Dropping that key is the card's mutation: it fails `tests/test_forward_stress_probability.py::test_the_probability_carries_the_calibration_report` by name, re-proved by the integrator against the final source with the source restored byte-identical (sha256 `ad2cb652…`).

**The denominator rule is read, not restated.** A panel thinner than `FS_MIN_NAMES = 20` is refused through `sector_screener.breadth_with_gate` - the repo's existing min-n rule - carrying that function's own reason, and an empty panel is refused first (n = 0 would otherwise satisfy its `if n and n < min_n`). A band holding fewer than `FS_MIN_BIN_N = 5` rows is refused, as is a history with zero observed stress months, so a probability is never emitted from a bin that cannot support one.

**Honest labelling (R8), and printed rather than scored.** The read states what it is: `caveat` says the bands are fitted on the same panel history they score, so this is a FITTED read and not a walk-forward-tested detector. It is registered in `regime_score` under `out["printed"]["forward_stress_probability"]`, present only while the gate is on, and is **not** in `COMPONENTS`, `COMPONENT_ORDER` or `align_components` - so the scored set, the coverage denominator and every pinned count are untouched (rule 3), and the `regime_score` edit was kept line-neutral above the line its pin test names.

Web impact: none. No tool, CLI flag or JSON shape changed.

**Look-ahead is now inexpressible in the factor DSL rather than discouraged, and an undeclared field fails closed (2026-09-24).** H2 of the survey's adoption set (the T1 batch), behind `enable_factor_availability_gate`, default off. `factor_schema` gains `observability_for` and a `FactorSpec.observability` property - the record stays at exactly nine fields, as Q3 fixes it, and the existing `availability` field keeps its `present`/`NA` **data-presence** meaning rather than being repurposed - and `factor_expressions.availability_gate(expr, ...)` rejects at **registration time**: any negative literal shift in `ref`/`delta`/`pct_change` (the DSL's reachable shift operators), and any field whose declared class is later than the decision date. Zero per-run cost: this is static analysis at admission, not a check in a tool path.

**Only what this repo can honestly state is declared.** Market columns (`close`, `open`, `high`, `low`, `volume`, `returns`, `vwap`) are `SESSION_CLOSE` and every `FACTOR_SCHEMA` factor is `FILING_DATE`; `NEXT_SESSION` and `MACRO_RELEASE_LAG` are legal caller-declared classes applied to **no** field here, because inventing a vendor or macro publication lag is exactly the open H8 decision (the as-reported vintage store). An undeclared field is **refused with its reason** rather than assumed safe. Dated classes bind to `data_quality.fundamentals_pit_ok`, and a declaration naming a symbol is additionally corroborated by `dataflows.pit_registry.read_as_of` - both read, neither edited (rule 2).

Gate off returns the unchanged `purity_gate` verdict, so an unregistered gate cannot change admission; `purity_gate` itself gained an optional `extra_fields` parameter whose default leaves every existing caller byte-for-byte unchanged (pinned by `test_alpha_zoo` and by the gate-off test). The gate's only caller is the offline admission point in `scripts/factor_bench.py`; `alpha_zoo.evaluate_expr` was deliberately not touched, so nothing pays the cost per expression per run.

Failing-first, re-proved by the integrator against the final source: replacing the forward-shift branch with `shifts = []` fails `tests/test_factor_availability_gate.py::test_late_availability_field_rejected` by name (the assertion reads *a negative shift reads the future and must be refused*), with the source restored byte-identical (sha256 `10c0b0aa…` asserted in a `finally`).

Web impact: none. No tool, CLI flag or JSON shape changed.

**The candidate zoo now has a redundancy screen, with the L1 selection hand-rolled because scikit-learn is absent and adding it is an open decision (2026-09-24).** X2 of the survey's adoption set (the T1 batch). No gate: it is an offline scheduled screen and nothing is added to the in-run surface. `alpha_zoo.redundancy_screen(candidates, controls, target, *, selection_window, evaluation_window)` does post-double-selection per candidate - LASSO the controls, LASSO the candidate on those controls, OLS the target on the candidate plus the union of the survivors, then re-fit the same design on the **evaluation** window - and a candidate survives only with `|t| >= 1.96` in selection AND a same-sign non-zero coefficient out of sample. The L1 solver is ~40 lines of fixed-order coordinate descent (`_lasso_cd`) with the theoretical Belloni penalty `c * sd(y) * sqrt(2 log p / n)`: deterministic, no cross-validation, no randomness, no new dependency.

**The two windows must be disjoint** - an overlapping pair raises `ValueError`, because in-sample redundancy is unreadable - and the means and scales are fitted on the selection window only and applied to the evaluation window, the learn/infer split this repo already uses.

**Consumption is a decision, and it is logged.** A component that never survives becomes a **documented** component rather than a scored one, and every dropped component gets one refusal-ledger row (`refusal_ledger.log_refusal`, the landed H7 writer) naming the screen and both windows. Deleting that write is the card's mutation: `tests/test_redundancy_screen.py::test_every_dropped_component_is_logged` goes red by name - it asserts the dropped component has a row naming `select:0..100|eval:100..200` - re-proved by the integrator with the source restored byte-identical (sha256 asserted). The screen is reachable only offline: a new `scripts/redundancy_screen.py` runner calls it, and a static test asserts no file under `tradingagents/**` other than `alpha_zoo.py` mentions it. A window too short to measure drops every candidate **with its reason**, so `kept + dropped` still partitions the input and nothing is reported as a zero coefficient.

Web impact: none. No tool, CLI flag or JSON shape changed.

**A drawdown envelope now says how deep and how long a drawdown of a given Sharpe should run - report-only, because the mandate is still open (2026-09-24).** K2 of the survey's adoption set (the T1 batch of `docs/paper_survey_26/README.md` section 2.1), behind `enable_drawdown_envelope`, default off. `book_risk.drawdown_envelope(sharpe, horizon, skew, kurtosis, hurst)` returns the four measures the card asks for - `mdd_median`, `mdd_p90`, `max_loss`, `time_under_water`, `longest_recovery` - from a seeded Monte-Carlo over the book's own annualized volatility, and it carries the Sharpe's **estimation uncertainty** as a reported field (Lo 2002, `sqrt((1 + SR^2/2)/n)`), because the card's own caveat is that Sharpe-estimation uncertainty changes the answer.

**The time-scaling, and the honest fallback.** The `T^(H-1/2)` dispersion rescaling is applied to the DEPTH measures only, and only when a Hurst estimate exists (`mean_reversion.hurst_exponent`, read rather than re-estimated); without one the envelope uses square-root-of-time and says so, returning `hurst: "assumed"` and `hurst_scale: 1.0`. The card's failing-first claim is exactly this: pinning the exponent to `0.5` turns the H=0.72, T=252 p90 ratio from `3.3752668884` into `1.0` against the closed form `252^0.22 = 3.3752668884`. Re-proved by the integrator against the final source: `tests/test_drawdown_envelope.py::test_hurst_rescales_mdd` goes red by name under that mutation, with the source restored byte-identical (sha256 asserted in a `finally`).

**Report-only, deliberately.** The envelope is consumed by the strategy-evaluation row in `strategies/evaluate.py` as ONE additive gated key beside that row's realized `max_drawdown`. **The governor leg is not wired** - `risk_governor.govern`, `drawdown_gate` and `book_context.measured_book_drawdown` are untouched - because the mandate answer on letting `T^(H-1/2)` reach the drawdown governor is still open (the plan's decision 3). This is the card's own documented fallback: without authorisation K2 lands as a report-only read and the governor keeps the square-root-of-time convention. Nothing here changes a verdict, and a gate-off run is byte-identical to the run before the envelope existed.

Web impact: none. No tool, CLI flag or JSON shape changed.

**A regime VaR is now judged by Kupiec and Christoffersen instead of asserted, and the heavy-tailed emissions that make the difference are selectable (2026-09-24).** R2 of the survey's adoption set (the T1 batch), behind `enable_hmm_heavy_tails`, default off. `regime.hmm_filtered_regime` gains an `emission` parameter (`gaussian` default, plus `student_t`, `laplace`, `ged`) with selectable `K`, and `regime_conditional_var(posteriors, emissions, q)` inverts the posterior-weighted mixture CDF to give the running state's VaR. **One shared forward-backward kernel**: only the emission density and the per-state M-step swap, so the `gaussian` branch is arithmetically the pre-R2 code - verified numerically (loglik, means, covars, A and pi all `array_equal` against the pre-R2 module) as well as by the existing `tests/test_strategies_regime.py` pins, which is what the card's bit-for-bit acceptance clause demands.

**The coverage test is the shared deliverable, so the two tail items ship one, not two.** `book_risk.var_coverage_test(returns, *, alpha, var_series)` runs the Kupiec proportion-of-failures test and Christoffersen's joint conditional coverage as a standalone public instrument - and K1's tail layer imports it. It returns the verdict with the coverage level, the sample size and the held-out window it was computed on, and `unavailable` when that window is too thin to judge. The contrast the card asks for is measured: on a manufactured fat-tailed two-regime series the Gaussian-emission VaR **fails** (hit rate 0.0375, joint p 0.00616) while the Student-t one passes (0.0565, p 0.2478). Computing the VaR under the Gaussian CDF regardless of `emission` turns the Student-t verdict into `fail` (hit rate 0.1030) - that is the card's mutation, re-proved by the integrator against the final source: `tests/test_hmm_heavy_tails.py::test_the_coverage_test_rejects_the_gaussian_var` red by name, source restored byte-identical.

**Stated limits, in the output rather than in a footnote.** The heavy families' shape parameters are FIXED and reported (`EMISSION_NU`, `EMISSION_BETA`), not fitted - the M-step does not run a full ECM iteration for `nu`, and reporting the constant is better than reporting a number the fit did not produce. `K` is caller-supplied and the output's `selection` field says so, with loglik, BIC, `n_obs` and `n_params` beside it, so a `K != 2` is never unlabelled. An unknown family is refused by name. The default path never reads config, so a gate-off run's regime read is byte-identical.

Web impact: none. No tool, CLI flag or JSON shape changed.

**A tail number now carries its data quality and its estimation uncertainty, and no input can ever narrow it (2026-09-24).** K1 of the survey's adoption set (the T1 batch), behind `enable_tail_risk_layer`, default off. `strategies/tail_risk.py` (new) **composes** the producers the engine already has rather than re-deriving any of them (rule 2): `book_risk.simple_var`/`cvar`/`portfolio_cvar` for the number, `data_quality.aggregate_quality` for the quality score, `conformal.quantile_band`/`rolling_band` for the band, and the new `book_risk.var_coverage_test` for the verdict. It returns `{var, cvar, q_score, uncertainty, band, status}` with `status in {ok, widened, unavailable}` - and the public number is `None` whenever the status is `unavailable`, so a refused estimate can never be read as a zero.

**One-directional by construction, which is the card's claim.** The widening term is `max(0.0, half - raw_half)`, so quality and uncertainty can only move the band outward: a monotonically worsening quality input widens the band monotonically (`0.08448` → `0.19365` across six levels) and walks the status from `ok` through `widened` to `unavailable`, with the VaR moving outward until it refuses. Flipping that guard to `(1.0 - total_widen)` narrows the band and eventually drives it negative - the card's mutation, re-proved by the integrator against the final source: `tests/test_tail_risk_layer.py::test_quality_only_widens` red by name, source restored byte-identical (sha256 asserted). **A red quality verdict is `unavailable`, never a number**, and the trailing breach rate and the full Kupiec/Christoffersen record travel with the number.

**Printed, not scored.** `risk_score` declares `tail_risk` as a PRINTED component with its producer label named - the R3/R7 precedent - so no pinned scored count moves and `COMPOSITE_ENGINES` is untouched (rule 3). `uncertainty` is the mean of the measurable terms (ensemble dispersion over at least two members, an out-of-distribution distance, a trailing breach-drift term) and an unmeasurable term is **excluded, never counted as zero**, which is the difference between "we did not measure it" and "we measured none". The K lane's focused set - the three new files with `test_strategies_regime.py`, `test_strategies_book_risk.py` and `test_risk_score.py` - is 92 passed; `ruff` clean.

Web impact: none. No tool, CLI flag or JSON shape changed.

**An eigenspace that rotates inside the Marchenko-Pastur bulk is noise, and the read now refuses to call it a rotation (2026-09-24).** V5 of the survey's adoption set (the T1 batch), behind `enable_eigen_rotation`, default off. `tradingagents/strategies/eigen_rotation.py` (new) removes the market mode from a rolling correlation matrix, keeps the subdominant eigenvectors, and measures rotation between consecutive windows as the mean squared sine of the three principal angles - computed as `mean(1 - sigma_i^2)` of the singular values of `U^T V`, which is invariant to eigenvector sign **by construction** (the test proves it by monkeypatching `np.linalg.eigh` to flip half the signs, so the property is tested against a hostile input rather than asserted).

**The mandatory check, and the conservative direction of it.** A subdominant block is only a measurement when it sits outside the Marchenko-Pastur bulk; inside the bulk the eigenvectors are noise and the angle between two noise eigenspaces is not a state variable. The engine has exactly one producer of that bound - `market_breadth.mp_below_count` - so the read **imports it and reads its returned `mp_lower`** rather than re-deriving the formula (rule 2), and `market_breadth.py` was not edited. Only the lower edge is available from that producer, and the i.i.d. bulk lies at or above it, so the read computes only when **every** subdominant eigenvalue sits below the edge - the sufficient condition - and refuses otherwise.

**What the check is worth, measured.** On six names over a 252-session window, dropping the check produces a rotation of `0.700456` over angles `[0.504735, 1.219557, 1.451782]` from subdominant eigenvalues `[1.0847, 0.9861, 0.9511]` against an MP lower edge of `0.715203` - every one of them inside the bulk, which is precisely the noise eigenspace the check exists to keep out of a published number. That is the failing-first proof, run by the integrator against the final source: removing the check fails `tests/test_eigen_rotation.py::test_rotation_refuses_inside_the_mp_edge` by name, with the source restored byte-identical (sha256 asserted in a `finally`).

**Reachability, and why it is not dormant.** `covariance_models.spectral_functionals(..., *, cfg)` gains ONE gated `rotation` key, and that function is reached from the production path `regime.spectral_change_read` - so the read has a real in-run caller on the same day it lands, which is the working agreement the three dormant wave-1 reads earned. With the gate off every key the function returned before is byte-identical, including the one-eigendecomposition-per-window property its R3 test pins. The window must exceed the panel, and the refusal always carries the window it was computed over (rule 4).

Focused set with the covariance, spectral, options-surface, regime and wiring suites: **85 passed, 1 skipped**; `py -3.12 -m ruff check` clean on every touched file.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**The engine can now recover a risk-neutral density, but only when the quotes can identify one, and it says which (2026-09-24).** V3 of the survey's adoption set (the T1 batch of `docs/paper_survey_26/README.md` section 2.1), behind `enable_rnd_recovery`, default off. `tradingagents/strategies/rnd_recovery.py` (new) fits a two-component lognormal mixture per expiry under mass and forward constraints - the high component's forward is fixed by the mixture mean equalling the forward, not by a penalty term - and returns the decision functionals it exists for: the 1% quantile, the risk-neutral variance and the tail probability, the last with its threshold printed (`RND_TAIL_THRESHOLD`) so the number is never read against an unstated level. The fit is deterministic: a bounded `scipy.optimize.least_squares` from a fixed initial guess, no seed and no wall clock, asserted by `test_rnd_recovery_is_deterministic`.

**The deliverable is the refusal, not the density.** Identifiability is derived from the conditioning spectrum of the covered strikes: the SVD of the price-sensitivity matrix of the mixture's OTM quotes with respect to its free parameters, taken at the fitted point. A numerical rank below the parameter count, or a condition number above `RND_MAX_CONDITION = 1000`, returns `status="unavailable"` with `density=None` **and the rank and condition number reported**, so a reader can see the fit was refused rather than missing. On the card's own four-strike chain the condition number is **4031.5** and the read refuses; drop the flag and the same chain returns `quantile_01 = 50.4969`, `risk_neutral_variance = 0.288167` and `tail_probability = 0.359443` from a fit that quote noise moves. That contrast is the failing-first proof, run by the integrator against the final source: setting the guard to `False` fails `tests/test_rnd_recovery.py::test_sparse_chain_returns_unavailable_not_a_density` by name, with the source restored byte-identical (sha256 asserted in a `finally`).

**The learned operator is deliberately not built.** The card records the decline - per-expiry classical fits beat the adapted learned operator on 524 held-out NIFTY calls - so what is taken is the shape and the identifiability discipline. `options_math.model_free_implied_variance` seeds the initial guess and `variance_swap_strike` is reported as a cross-check against the fitted risk-neutral variance; both are **read**, neither re-derived (rule 2).

**Reachability.** The read hangs off `options_surface.expected_move_from_chain(rows, *, cfg)`, which `agents/utils/analysis_tools.get_options_iv_read` already calls, as ONE additive key present only while the gate is on - so a gate-off run is byte-identical to the run before the recovery existed. Focused set with the options-surface, covariance, spectral, regime and wiring suites: **85 passed, 1 skipped**; `py -3.12 -m ruff check` clean on every touched file.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A regime change is now declared only against calibrated noise, and the shrinkage input that would have made every move "detectable" is reported instead of trusted (2026-09-23).** R3 of the survey's adoption set (wave 1), behind `enable_spectral_null_band`, default off. `strategies/covariance_models.py` gains the numerics: `spectral_null_band(n_obs, shrinkage, *, z)` = `z * (1 - shrinkage) / sqrt(n_obs)` - a **pure function of the window length and the shrinkage intensity**, so the same length and intensity always carry the same band, which is the property the card asks for; `panel_spectrum(window, *, min_n, method)` does **one** eigendecomposition per window over `statistical.correlation_matrix`'s correlation and refuses per rule 4 (thin window, unresolved pair, unmeasurable intensity) as `unavailable`; `eigen_projector_distance(...)` is `||P_k - P_k'||_F` carrying move, normalized move, band, geometry, threshold, `exceeds` and the reading; `spectral_functionals(...)` reports the absorption ratio and the leading share, each with its own band and flag.

`strategies/regime.py` gains the gated per-**panel** read `spectral_change_read(...)` - two eigendecompositions in total - and every leg it returns carries its band, geometry, threshold, flag and reading. **A non-rejection reads "not detectable", never "no change"**, which is the card's own framing: the paper's non-rejections partly reflect wide null bands, so a wide band is an absence of evidence, not evidence of absence.

**What enters the score is the FLAG, not the functional.** `regime_score` registers `spectral_change` as a **seventh** component declared only while the gate is on - and the value it takes is the flag (or the read record, from which the flag is extracted). A functional on any other scale aligns to `None`, so neither the absorption ratio nor the projector distance can drift into the score. That keeps rule 2 intact (the read is calibrated *with* `ledoit_wolf_shrink`'s intensity rather than becoming a second authority beside it) and leaves `COMPOSITE_ENGINES` untouched (rule 3).

**The finding that shaped the design.** This item is where the `ledoit_wolf_shrink` divergence was found: the engine's intensity **saturates at 1.0** on a narrow panel, so a band scaled by `(1 - shrinkage)` collapses to zero and would flag *every* move as a change - the opposite of the item's purpose. The read therefore calibrates its band at an **explicit** intensity (default `0.0`, the raw correlation read, whose un-shrunk first-order null is the honest default), **refuses a zero band with its reason**, and reports the engine's own intensity beside it as `engine_shrinkage`. That divergence is recorded in its own entry above; what matters here is that a saturated input cannot silently become "everything moved".

**Stated gap.** The component and its glue are in place - `regime_score._spectral_change_values` accepts the read record or the bare flag - but **no in-run caller supplies a panel yet**, so the component is dormant in-run. It is queued in this round rather than skipped, like V6's tail leg and R1's mode selector before it, and because the gate declares the component only while it is on, a gate-off run's declared set is unchanged. Three stale `file:LINE` anchors in `regime_score.py` were repaired along the way.

Failing-first proof, run by the integrator against the final source: making the flag unconditionally `True` fails `tests/test_spectral_null_band.py::test_a_stable_covariance_is_not_flagged` by name, with the source restored byte-identical (sha256 asserted). Focused set with the regime, covariance and registry suites: **78 passed**; `ruff` clean.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**The engine can now flag an accuracy claim that is arithmetically impossible - and building it exposed a look-ahead in its own sigma recursion (2026-09-23).** H4 of the survey's adoption set (wave 0), behind `enable_accuracy_ceiling`, default off. `tradingagents/strategies/alpha_eval.py` gains `ceiling_ratio(returns, forecasts, *, train_frac, min_n)`: it fits `volatility_models.garch11_fit` - **read, never edited** (rule 2) - on the leading `train_frac` share, builds each hold-out row's `sigma_hat` from a one-step-ahead recursion, and reports `{kappa_hat, da_squared, r2_ratio, r2_oos, da, n, train_n, flagged, basis, unavailable}` with `flagged = r2_ratio > da_squared`. `calibration.excess_accuracy(pred, realized, baseline="always_up", *, folds, alpha, min_n)` reports the model hit rate minus the always-up base rate on the identical rows, per sequential walk-forward fold, carrying the interval from **H11's `block_bootstrap_interval`** (imported, not reimplemented). Both refuse to `unavailable` **with the reason** - never a zero - on no fit, a thin hold-out, no signed rows, no dispersion, or a `kappa_hat` that is not negative.

**Read it as a falsifier, never as a validator**, which is the card's caveat and now the docstring's: the inequality comes from a constructed oracle, it can flag an impossible point, it cannot reject a forecast, and `kappa` assumes sign/magnitude independence.

**The delivered recursion consumed a row's return before emitting that row's sigma - the exact defect the split exists to prevent.** `_oos_sigma_series` updated the conditional variance with `r_t` and *then* appended `sqrt(var)` as `sigma_hat_t`, which makes `eps_t = r_t / sigma_hat_t` the model's own standardized residual and the ceiling circular. The docstring already said the return must be consumed "only after the row it belongs to", so the order is fixed here: emit, then update. **This was found by the integrator verifying the failing-first claim rather than accepting it** - and the first attempt to prove it is worth recording because it failed: on the card's own fixture the GARCH(1,1) fit returns **`alpha = 0`** (iid data carries no clustering to fit), the recursion stops depending on the current return entirely, and the two orders are numerically identical - so neither the ordering mutation nor a mutation of the fit window turns `test_above_line_point_is_flagged` red. The card's test therefore did not guard the property it names.

**The guard was rebuilt on a fixture the instrument can see.** A new `test_sigma_hat_never_consumes_its_own_row` runs on a GARCH-clustered panel (the fit then returns `alpha ~ 0.2`) and asserts `kappa_hat < -0.30`; measured on that fixture the one-step-ahead order gives **-0.343** and consuming the row's own return first gives **-0.255**, so the bound is the property with ~0.045 of margin on both sides rather than a magic number. The docstring of the fixture records *why* it must be clustered, so the next reader does not rediscover that `alpha = 0` makes the property invisible. The card's named test is kept as it was - it still pins the card's own acceptance criterion, the flagged impossible point.

**Three interpretations are recorded rather than glossed.** (1) The card specifies a bare `(kappa, (2DA-1)^2, R2_ratio)` triple; a tuple can carry neither the `flagged` verdict the card's own acceptance test demands nor rule 4's `unavailable` reason, so the return is a record whose core triple is exactly those three fields plus the honest extras. (2) `kappa = (E[sqrt(eps)])^2/E[eps] - 1` is `<= 0` by Jensen, so on the card's axes the legal region is the **above**-line one; the implemented rule is the card's stated inequality (`flag iff R2_OOS/kappa > (2DA-1)^2`), the only reading under which the card's acceptance criterion is satisfiable - with `kappa < 0` a negative OOS `R^2` lands at a positive ratio and sits above the line. (3) The engine's resolved decisions carry a **direction** (the stored rating), not a magnitude, so the caller reads the ceiling on a unit-scaled sign forecast - the paper's own constant-magnitude construction - with Hold/REVIEW mapped to `0.0` and dropped rather than scored as misses.

The instrument's rule-7 caller is `trading_graph._maybe_report_accuracy_ceiling`, reachable in-run from `_resolve_pending_entries`, which already holds the realized holding-period returns and the resolved entries' directional calls on identical rows. Failing-first proof: the ordering mutation fails `test_sigma_hat_never_consumes_its_own_row` by name (`assert -0.255 < -0.3`), source restored byte-identical. Focused and neighbouring suites (`test_accuracy_ceiling`, the three calibration suites, `test_quantlib_lean_enhancements` at 64, plus the registry and api-reference contracts): green, `ruff` clean.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**"Not proven" and "disproven" are now different answers (2026-09-23).** H6 of the survey's adoption set (wave 0), behind `enable_materiality_verdict`, default off. `strategies/evaluate.py` gains `materiality_verdict(stat, ci_low, ci_high, delta)`: **SUPPORTED** iff `L > delta`, **REFUTED** iff `U < delta`, **INCONCLUSIVE** when the interval straddles the threshold, and `unavailable` on missing or non-finite input - never REFUTED, because a missing interval is not a verdict. The rule that matters is stated in the docstring and enforced by the card's test: **INCONCLUSIVE is never collapsed into REFUTED.** A straddling interval is *unresolved*, not evidence of no edge. That is the failure this vocabulary exists to prevent - a reader taking "we could not show it" for "we showed it is not there" - and it is exactly what the card's mutation does; the integrator proved it against the final source, where collapsing the verdict fails `tests/test_materiality_verdict.py::test_straddling_interval_is_inconclusive` by name (`assert 'REFUTED' == 'INCONCLUSIVE'`), with the source restored byte-identical.

`family_materiality(...)` is the **one** producer of the family adjustment: it reuses the stationary-bootstrap machinery `reality_check`/`spa` are already built from (`_rc_prepare`, `_stationary_indices`, `_rc_metric`), applies Benjamini-Yekutieli's step-up (`p_(i) <= (i/K) * alpha / H_K`), attaches each candidate's H11 interval, classifies the R axis at `delta_r`, and reports the S bar at `delta_s` as a **named necessary condition**. A `SUPPORTED` candidate that fails FDR survival is **demoted to INCONCLUSIVE, never to REFUTED** - the same rule one level up.

**The comparator was the flaw worth fixing:** `benchmark_table` gains an optional `exposure` series and appends an **exposure-time-matched** row - the benchmark replayed on held bars, cash elsewhere, cost-free - which is what "the same binary position" means when the strategy is flat part of the time. With no aligned exposure the row is dropped and the default path is byte-unchanged.

**The vocabulary reaches the report verifier.** `ReportVerification` gains `verdict` (SUPPORTED / REFUTED / INCONCLUSIVE / UNKNOWN, default UNKNOWN, with `overall` untouched); `report_materiality_verdict` returns REFUTED only on a contradicted, misquoted or internally-conflicting claim, otherwise the interval verdict over the grounded-fraction interval at threshold 0, and UNKNOWN when there are no claims; `_family_materiality` treats a run's stems as one FDR-corrected family, emitted as the tree-level `family` key.

**The thresholds are declared, not chosen after the fact.** `materiality_delta_s = 0.20` and `materiality_delta_r = 0.01` are plain `DEFAULT_CONFIG` **values** (no `enable_` prefix, no env row, no gate) read by literal key, so they are fixed before any result is seen - which is what makes the verdict a verdict. The card's own caveat travels with them: they are declared but admittedly non-canonical, and the unit of analysis is a catalogue rule, not a portfolio.

**Web impact: one file's payload, gated.** The app renders the engine's `verify_flags.json`; with the gate off the payload is exactly the pre-H6 shape, and with it on the tree gains a per-stem `verdict` and a tree-level `family` key. The app renders that file generically rather than pinning its key set, and the contract pair (`test_engine_contract.py`, `test_doc_claims.py`) is run against this change.

Focused and neighbouring suites (`test_materiality_verdict` at 4, `test_strategies_evaluate`, `test_verifier_formula_checks`, `test_reality_check`, `test_validation_guards`, `test_alpha_zoo`, the wiring gate, and the registry and api-reference contracts): **green**, `ruff` clean.

**The engine now records the candidates it REFUSED, and can measure how often a refusal was right (2026-09-23).** H7 of the survey's adoption set (wave 0), behind `enable_refusal_ledger`, default off. Every guardrail in the engine records what it *did*; none recorded what it *stopped*, so a guardrail's precision was unfalsifiable - a refusal with no ledger is a claim with no denominator. `tradingagents/strategies/refusal_ledger.py` is new: an append-only `refusals.jsonl` holding one immutable row per refused candidate (`symbol`, `as_of`, `gate`, `reason`, `snapshot_hash`, `ts`, `written`), read back by `rows()`; `classify_refusal(...)` scores each refusal against its own forward path into the paper's five tiers (`saved-windowed`, `missed-moon`, `saved-early-death`, `flat`, `unclassifiable`); `score_all(...)` joins on the row's own `(symbol, as_of)`; and `save_to_miss_ratio(..., *, min_events=30)` publishes the per-gate taxonomy with its **`events` count always beside the ratio**, plus explicit `underpowered` / `unavailable` reasons - a ratio without its denominator is the thing this file exists to prevent.

**The tie-break is the load-bearing branch.** A refusal that both saved capital and missed a run is classified as a **miss**, and it is an explicit branch rather than a consequence of ordering: without it, saved-first ordering calls every such refusal saved. That is exactly what the card's failing-first test mutates, and the integrator proved it against the final source - removing the branch fails `tests/test_refusal_ledger.py::test_tie_break_missed_beats_saved` by name, source restored byte-identical.

**Five of five writer sites, all additive.** `risk_governor.govern` (the REJECT, with its reasons), `knife_guard.knife_factor` (only the 0.0/block band, with identity threaded through `knife_score`), `market_tradability` (from `suspended`, `limit_gate` for `up`/`down`, and a `volume_gate` that truncates an order to zero), `news_relevance.admit_article` (a dropped article), and `value_dip.value_dip_setup` (a `candidate: False` with its floors and gates). Each site gained **keyword-only optional** `symbol`/`as_of`, its verdict and return value are unchanged, and every write sits under `contextlib.suppress(Exception)` - a ledger that can break the refusal path it observes is worse than no ledger. `scripts/refusal_scorecard.py` is the rule-7 caller.

**The card's caveat is the reason the taxonomy travels without the ratio:** the paper's generalisation is weak - 13 to 22 days, one chain, one deployment, one regime - and its own headline tier was refuted by its own matched-lifecycle test. What is adopted is the taxonomy and the discipline; the ratio is reported per gate, with its event count, and is not a constant anywhere in this code.

Focused set with the registry and api-reference contracts: **90 passed**; `ruff` clean across all seven files, including the five refusal-lifetime writes rewritten to `contextlib.suppress`.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A confirmed divergence from Ledoit-Wolf (2004) in `ledoit_wolf_shrink`, measured and deliberately NOT fixed (2026-09-23).** Found while calibrating R3's spectral null band. `tradingagents/strategies/covariance_models.py::ledoit_wolf_shrink` computes `shrinkage = clip(b^2 / d^2, 0, 1)`, where `b^2` is the mean over observations of `||x_t x_t' - S||_F^2` and `d^2 = ||S - F||_F^2` - both of them the paper's own quantities - but the paper's intensity is `delta = clip((b^2 / t) / d^2, 0, 1)`. **The division by the observation count is missing**, so the engine's intensity is `t` times too large and saturates at `1.0` wherever `t >~ d^2/b^2`, which in practice is every panel that is narrow relative to its own history.
...
Web impact: none. No engine surface, tool signature or JSON shape changed.

**Reproduction.** Build an iid Gaussian `t x p` panel, centre it, and compare `min(max(b2/d2, 0), 1)` with `min(max((b2/t)/d2, 0), 1)` where `b2 = mean(sum((einsum("ni,nj->nij", X, X) - S)**2, axis=(1,2)))` and `d2 = sum((S - mu*I)**2)`.

**The two wave-1 reads that were built but unreachable are now wired into the tools that own them (2026-09-23).** Both items had landed their read and their rule-7 chain but left the analyst-facing tool untouched, which under this repo's own working agreement (1b: a computed read that never reaches an agent tool loop is incomplete work) is an unfinished item rather than a landed one. Nothing here changes a gate's default; both wirings are inert unless the owner flips the switch, and the gate-off output is byte-identical.

**V6.** `get_tail_extreme_var` already fetched the price tape and threw the bars away - it passed only `rets`, so the jump leg had no window to read. It now passes the same `_ohlcv(ticker)` payload to `book_risk.extreme_quantile_var` (which accepts the run-cache column mapping) and renders the leg when it exists: gate off produces the line it always produced, gate on appends `, jump_share=0.75 (one_print)` on a one-print tape. The EVT numbers themselves are untouched.

**R1.** `get_shift_detection` called `bocpd(series)` with the default, so the duration-law modes existed and nothing could select them. It now reads a new **mode** key - `bocpd_hazard_mode`, env `TRADINGAGENTS_BOCPD_HAZARD_MODE`, default `"constant"` - through a helper that falls back to `"constant"` on an unknown or unreadable value rather than guessing. `constant` is today's read bit-for-bit (verified: the printed line is unchanged); `lognormal` changes it and adds `hazard_mode=lognormal covering=0.983` to the line so the segmentation's agreement with the constant-hazard segmentation travels with the probability; a bogus value is inert.

The key is a **mode, not a gate**: it changes what the already-gated read computes, so it has no `enable_` prefix, no registry row and no `enable_*` count, and the sibling app's derived allowlist does not see it (it admits `enable_*` keys plus its own curated set). It does appear in `docs/api_reference.md` §1.1, because that table is generated from every env var the config reads.

Evidence, not assertion: both tools were run against a synthetic tape with the real tool functions, once per state - `get_tail_extreme_var` with the gate off (no jump note) and on (`jump_share=0.75 (one_print)`), and `get_shift_detection` under `constant` / `lognormal` / a bogus value (`bogus` reproduces `constant` exactly). Existing suites: `tests/test_analysis_tools.py`, `test_bocpd.py`, `test_volatility_models.py`, `test_gate_env_toggles.py` and `test_api_reference_env_table.py` all green.

**Web impact: none.** No tool signature, CLI flag or JSON shape changed, and the new config key is a mode the app neither reads nor exposes.

**Every claim statistic the panel and the scorecard publish now carries a block-bootstrap interval and the IID one it is judged against (2026-09-23).** H11's attachment half, behind the same `enable_bootstrap_intervals` (default off) whose estimator half landed earlier in this round. `scripts/score_panel.py` gains a public `claim_interval(series, *, window)` returning the frozen conformal record plus the window, the IID baseline and the refusal reason; it is the first non-test caller of `conformal.block_bootstrap_interval` / `iid_interval` and the rule-7 caller for them. `factor_statistics` attaches it to the **rank-IC** series and the **decile-spread** series, `evaluate_panel` attaches one to the pooled per-period mean spread series for the family block, and `render_text` prints the bounds, the point, `n`, `block=`, `draws=`, the method and the IID baseline. `scripts/coverage_scorecard.py` keeps the per-tree present/absent indicator for every declared field **in tree order** and attaches an interval beside the unchanged `fill_rate`, rendered per field.

**The dependence assumption is printed, not implied.** The block length and the draw count travel with the bounds in both reports, so a reader can see what the interval was computed as if - which is the whole point of the item, and the reason the attachment was proven to fail when it is swapped for the IID arm: the card's mutation (`block = iid_interval(values)`) fails `test_the_block_length_is_not_a_constant_across_persistences` by name (`assert 1 > 1` - block length 1 everywhere) and takes two sibling tests with it (`assert 'iid' == 'moving-block'`).

**Refusal is a first-class outcome.** Below the floor, on an unusable series, or on a **zero-dispersion** series the record is `{window, n, unavailable}` with no bounds at all - never a zero-width interval, because a zero-width interval is the most confident possible lie. With the gate off neither report adds a key or a rendered line, asserted in both directions.

Failing-first proof, run by the integrator against the final source: the attachment swapped to the IID arm fails the named test and the source is restored byte-identical (sha256 asserted). Existing panel and scorecard suites: **70 passed**; the focused set with the registry contracts: **21 passed**. The 2 warnings are pre-existing, from `test_score_panel.py::test_a_non_finite_rank_ic_is_withheld_never_printed_as_nan`.

**Web impact: none.** Both are manual offline scripts; the sibling app reads neither.

**A stress read that names where the stress is centred, and says it is coincident (2026-09-23).** R7 of the survey's adoption set (wave 1), behind `enable_triadic_stress`, default off. `tradingagents/strategies/triadic_stress.py` is new: `triadic_stress(returns_by_name)` builds `TSI = (C*D/M)*Coex` over the correlation network `statistical.correlation_matrix` already produces - `C = Tr(A^3)/(n(n-1)(n-2))`, `D` the weighted `|rho|` density, `M = 1/lambda_2` of the normalized Laplacian, `Coex` the variance of the weighted degree - with a per-node `diag(A^3)` **epicentre** attribution, ordered descending with ties by name and cut at the cross-sectional mean (the cut is printed as `epicentre_cut`, so the policy is visible rather than implied). The engine could say concentration moved; it could not say *which names* the stress sits on.

**Every returned read carries `coincident: true`, and the refusals are the honest half.** `unavailable` below **eight** names, below 60 aligned bars, with no usable correlation matrix, or when `lambda_2 <= 1e-9` (a near-disconnected graph has no finite `M`). A four-symbol book IS a thin cross-section and refuses - which is the card's own limit stated as behaviour rather than as a footnote. Ground rule 4 is met by reading H10's `coverage_window` and reporting the window it was computed over, and the correlation matrix is **read**, never reimplemented.

**Registered as PRINTED, not SCORED, and that is a decision with a reason.** `risk_score.py`'s `COMPONENTS` gains `triadic_stress_index`, but PRINTED rather than SCORED: `tests/test_risk_score.py` pins the correlation category at exactly two scored components, and scoring this would have required editing an existing assertion to accommodate a new item. PRINTED also keeps the read out of competition with `market_breadth` (ground rule 2) and leaves `COMPOSITE_ENGINES` untouched (ground rule 3).

**Measured on a manufactured panel** where a known subset carries the correlated block: per-node attribution `A 4.76 / B 4.73 / D 4.72 / C 4.71` against `0.03 - 0.27` for the names outside it, with the epicentre `['A','B','D','C']` stable across seeds 1, 2, 3, 11 and 42 and `lambda_2` about 0.50-0.62. The card's caveat travels with it: this is coincident and not leading, it only marginally exceeds the Absorption Ratio, and it needs a cross-section.

**One card citation was wrong** and is recorded: the card named `tests/test_strategies_risk_score*.py`, which does not exist; the real file is `tests/test_risk_score.py` (33 tests, green).

Failing-first proof, run by the integrator against the final source: emptying the epicentre list - dropping the `diag(A^3)` attribution - fails `tests/test_triadic_stress.py::test_epicentre_names_the_stressed_block` by name (`assert set(rec["epicentre"]) == set(BLOCK)` - the epicentre comes back empty), with the source restored byte-identical (sha256 asserted). The focused set against the final source: **48 passed**, including the registry, api-reference, triadic-stress and risk-score suites.

**Web impact: none.** No tool, CLI flag or JSON shape changed. Naming names in the risk debate's output remains the owner's call (the plan's own open decision 4).

**The pre-event options read is indexed in event time, and it refuses when the calendar cannot name the event (2026-09-23).** V4 of the survey's adoption set (wave 1), behind `enable_event_iv_lift`, default off. `strategies/options_surface.py` gains `pre_event_iv_lift(chain, event_date=None, *, as_of=None, fed_rows=None, window_days=14)`, `PRE_EVENT_IV_LIFT_LABEL` and `PRE_EVENT_MIN_EXPIRIES`: it takes the chain (rows, or the bundle `{rows, as_of, fed_watch}`), asks `catalyst.fed_imminence` - the **sole authority** on the scheduled catalyst - which meeting is in the window, and indexes the ATM implied volatility by **days to the event** (`days_until_event - days_to_expiry`) rather than calendar days to expiry. Expiries at or after the catalyst are counted (`n_post_event_expiries`) and **never valued**, so there is no forecasting leg past the meeting - which is what the card demanded and the reason it demanded it: the authors' own tests leave the forecasting half statistically insignificant, and adopting it would be adopting a negative result as a positive one. The read is descriptive and its own printed line says so.

**It refuses rather than guesses.** `unavailable` with a reason - never a zero - when the as-of is missing, when the calendar certifies no meeting in the window, when a named `event_date` is not that meeting, or when fewer than `PRE_EVENT_MIN_EXPIRIES` expiries sit at or before the catalyst. The instrument's rule-7 caller is `agents/utils/analysis_tools.py::get_vol_surface_shape`, which renders one additive line beside K3's proxy block when `_flag("enable_event_iv_lift")` is on.

**Two notes a reader needs.** (1) The card's signature was `pre_event_iv_lift(chain, event_date)` while also requiring `fed_imminence`; the design doc's own "the chain the engine already fetches plus the calendar it already has" settles it - the calendar names the event (`event_date=None`), so there is no second authority on the day count. (2) **Pre-existing, recorded, not touched:** the tool's `_rows_for` derives days-to-expiry as `int(max(days, 1) / 365.0 * 365)`, which floors 9 days to 8, so a 9-day expiry can print as being 2 days before a 10-day catalyst. That is live tool behaviour from before this item and the event-time change does not fix it.

Failing-first proof, run by the integrator against the final source: indexing the shape by calendar days instead of days to event fails `tests/test_options_surface.py::test_pre_event_lift_is_indexed_in_event_time` by name, with the source restored byte-identical (sha256 asserted). The builder additionally ran the existing options and analysis-tool suites (232 passed) and a smoke test through the real tool with a mocked chain and calendar.

**Web impact: none.** No tool signature, CLI flag or JSON shape changed - one additive printed line behind a default-off gate.

**An interval now says what it was computed as if, and a band that is wide and uninformative can be told from a tight one (2026-09-23).** H11 of the survey's adoption set (wave 0), behind `enable_bootstrap_intervals`, default off. `strategies/conformal.py` gains two axes.

**Autocorrelation-aware intervals.** `iid_interval` resamples observations one at a time - it is the baseline, and it is public precisely because the comparison is the diagnostic. `block_bootstrap_interval` resamples non-overlapping blocks whose length comes from the series' own ACF decay (the first lag inside the `2/sqrt(n)` noise band, floored at `n**(1/3)`) **checked against an ADF stationarity test**: a series the ADF declares stationary has mean-reverting dependence, so its ACF horizon is the right block; a series that survives that check may be carrying a unit root, whose ACF decays only because the window ends, so it gets the base length instead and the caller can see why in `adf_t`. `block_length` and the block actually used travel with the interval, because the dependence assumption is the one thing a reader cannot recover from the width.

**The number that matters is the one this does NOT fix.** Measured against this implementation (n=250, 40 replicates, nominal 0.90), at an AR(1) with `rho = 0.8` the IID interval covers **0.45** and the block interval covers **0.75**. The block arm recovers most of what was lost and does **not** restore nominal coverage under strong persistence at panel scale. That is stated in the module as the most important thing to know about it - read the realized coverage, never the nominal level alone - because a diagnostic that overstates itself is the exact failure this module exists to prevent. The rule was also **corrected by measurement** during this work: the first cut had the ADF check inverted (it downgraded every real series to the base length and left coverage at 0.65), and inverting it - stationarity earns the ACF horizon - is what moved 0.65 to 0.75.

**The information gap.** A pooled band can be wide and still carry nothing, and that is invisible in the width. `information_gap` scores the pooled scale against a conditional-scale alternative (the residual sd inside quantile bins of the prediction) on the same window and reports the difference in nats per observation, with the verdict against a declared threshold of 0.02. Measured: homoskedastic pairs gap 0.004 (worst of 60 replicates 0.009), mildly heteroskedastic 0.092, strongly heteroskedastic 0.257 (best of 60 replicates 0.178) - so the threshold separates them with an order of magnitude of margin on both sides. `rolling_band` gains the gap **only** when the gate is on; with it off the returned dict is exactly the keys it returned before, asserted key by key.

**One decision taken conservatively.** The card left open whether to adopt the public `tsbootstrap` library or keep the in-house bootstrap (the survey's open decision 9). `tsbootstrap` is **not installed** and the adoption set forbids a new dependency, so this is an in-house moving-block bootstrap in pure Python - the only new import is `random` - and the agile-conformal calibrators the library would have bought are not used. Switching later is additive.

**The card's other half is in flight, and said so here:** attaching these intervals to the claim statistics `scripts/score_panel.py` and `scripts/coverage_scorecard.py` publish is a separate commit in the same round.

Failing-first proof, run by the integrator against the final source: making `_resample_means` take the IID arm for every block length fails `tests/test_bootstrap_intervals.py::test_block_interval_covers_ar1` by name, with the source restored byte-identical (sha256 asserted). Focused set with the registry, api-reference and existing conformal-band suites: **34 passed**.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**The changepoint read stopped asserting that a regime is as likely to end on its first day as on its hundredth (2026-09-23).** R1 of the survey's adoption set (wave 1), **no new gate** - it extends the existing `enable_bocpd`, default off, so every existing pin on that gate stays green. `strategies/regime.py::bocpd` gains `hazard_mode`: `constant` (today's behaviour, and the default) or a duration-law-compiled hazard from `{lognormal, pareto, geometric}`. The law's parameters are re-estimated at every step on the expanding window of run lengths observed so far - the completed runs plus the open run's age, a right-censored observation that extends as the window grows - so the run-length prior is compiled from an explicit law instead of being flat. Same recursion, `O(t)`; the compile is an `O(#runs)` pass; the change mass becomes `sum_r p_r * h(r)` rather than a constant `h`.

**The covering metric travels with the probability, which is the point.** A changed hazard can only be *judged* if the segmentation it produces is comparable, so the duration-law modes also report `duration_params`, `hazard_curve` (the hazard actually applied at each run length) and `covering` = the symmetric, length-weighted Jaccard of this read's segmentation against the **constant-hazard** segmentation of the same series - the reference being used because no labelled segmentation exists in-run. Fewer than two observed runs and the declared scalar hazard governs (its geometric prior), with `duration_params=None`; a degenerate fit (zero spread, non-finite, `p` outside `(0,1)`) takes the **same `None` refusal path an out-of-range `hazard` takes**; an unknown mode is refused, never guessed.

**Bit-for-bit, measured against the pre-change module.** The builder loaded `regime.py` from `git show HEAD:` and compared 59 cases across 4 seeds x 4 lengths x {default, `hazard=1/20`, `warmup=5`}, seven prefixes of a manufactured 12-sigma shift series, the long-regime series and the degenerate inputs: **0 mismatches on every field**, the only added key being `hazard_mode`. The existing `map_run_length == 240` stationary pin and the `hazard` echo field both stay green (15 passed in `tests/test_bocpd.py`).

**Stated gap.** The modes are **dormant in-run**: `agents/utils/analysis_tools.py::get_shift_detection` still calls `_bocpd(series)` with the defaults, so nothing in a run selects a duration law yet. It is queued rather than skipped - that file carries another item's edit in this same round - and the selector will default to `constant`, so it can only ever be inert. R1 adds no public function (every new module-level name is underscore-prefixed, which the wiring rule exempts) and no new config key.

**Read the caveat with it:** the paper's evidence is high-frequency NASDAQ order flow with **no daily-frequency validation**, so the daily-frequency transfer is a hypothesis and not a demonstrated gain here; and its multivariate extension was beaten by independent univariate filters, which is a warning against pooling.

Failing-first proof, run by the integrator against the final source: collapsing `law_mode` to `None` so the duration law never compiles fails `tests/test_bocpd.py::test_hazard_mode_lognormal_is_run_length_dependent` by name (`KeyError: 'hazard_curve'` - the mode's record collapses back onto the constant path), with the source restored byte-identical (sha256 asserted).

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A memory parameter, not just a roughness exponent (2026-09-23).** V2 of the survey's adoption set (wave 1), behind `enable_long_memory`, default off. `tradingagents/strategies/long_memory.py` is new: `memory_parameter(returns, as_of=None, *, window=750)` estimates `d` twice - by GPH log-periodogram regression and by local Whittle - on the same `log I(lambda_j) = c - 2d*log(lambda_j)` ordinates over `m = isqrt(n)` Fourier frequencies, with `se = pi/sqrt(24m)`, and `rv_forecast(...)` is the HAR/HAR-X family (`RV_{t+1} = b0 + b_d*RV_t + b_w*mean(RV,5) + b_m*mean(RV,22)`) whose extra regressor is a cross-sectional or sector persistence aggregate. **`d` is reported beside the forecast, never instead of it** - the forecast record always carries it, because a forecast from `d = 0` and one from `d = 0.44` are different objects.

**Backwardness is enforced, not assumed.** A window containing any observation at or after the as-of date is **refused** (`unavailable`, estimates `None`) rather than silently trimmed - the failure mode being guarded is a future stressed name entering a persistence aggregate whose whole purpose is to describe the past. The X_t leg reports `unavailable` with its reason until a panel read exists, never a zero.

**Validated against a series with a known answer.** On synthetic ARFIMA(0, d, 0) draws (750 points, the card's window): `d = 0` recovers GPH `0.020` / Whittle `0.028`; `d = 0.44` recovers `0.477` / `0.457`; `se = 0.123`, so both estimators bracket the truth within their standard error, which is the acceptance clause. The roughness leg stays where it was - `mean_reversion.hurst_exponent` is untouched - and the new `memory_profile` reports beside it, returning `unavailable` when the gate is off.

**Read the card's caveat with the numbers**: the gains are "moderate" by the paper's own account and largest in stress, and its evidence (`d = 0.226` GPH, `0.440` local Whittle) is a cross-section this engine does not hold at that depth - so this is a re-measurement on this engine's panel before any threshold is trusted, not a transfer of theirs.

Failing-first proof, run by the integrator against the final source: making the backwardness refusal unreachable (`if False and future:`) so `d` is computed on the full sample fails `tests/test_long_memory.py::test_memory_parameter_is_backward_and_reported_beside_forecast` by name (`assert 'ok' == 'unavailable'`), with the source restored byte-identical (sha256 asserted). Focused set with the registry and api-reference contracts: **23 passed**.

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A panel read that counts how many independent bets are left (2026-09-23).** X3 of the survey's adoption set (wave 1), behind `enable_mp_lower_spectrum`, default off. `market_breadth.mp_below_count(corr, n, w)` counts the eigenvalues of a panel's correlation matrix strictly below the Marchenko-Pastur lower bound `(1 - sqrt(n/w))^2` - the count rises when the effective number of independent bets collapses, which is a state a concentration statistic cannot express. `sector_breadth.mp_lower_spectrum(closes_by_name, *, window=252, cfg=None)` is the gated per-**panel** read (never per-symbol): it builds the panel correlation over the trailing window's returns from the names that carry the whole window, and reports `window`, `n_names` and `panel_n` - how many names the panel offered beside how many were used - so the read names its coverage rather than implying it. `market_breadth` attaches the one key when the gate is on and nothing at all when it is off.

**The boundary is the item.** `w == n` reads `unavailable` - never `count == 0`, because "no eigenvalue is below the bound" and "this window cannot support the bound" are different claims and only one of them is true here. The same refusal covers a missing, non-square or non-finite correlation matrix, and a panel that cannot yield two full-window names reports its coverage instead of a number. `multi_breadth`'s output is pinned to a frozen literal snapshot and asserted unchanged, and the gate-off `market_breadth` record is asserted to carry no extra key.

**Read it as a state, never a signal** - the card's own caveat: the statistic is coincident and direction-blind (it cannot separate a crash from a bubble), the paper's predictive claim is asserted more than demonstrated, and at this book's size the cross-section is thin.

**A third gate had to be fixed for the registry.** Like H10's and H1's, `mp_lower_spectrum` first read its key through a module constant (`GATE_NAME`), which the registry's read-site scan cannot see - the gate scanned as read by nothing and the registry test said so. It now reads the literal. Because this is the third occurrence in one round, `docs/gate_registry.md` §8 now states the trap as part of the gate's contract: a gate read through a constant is a gate the registry cannot check, and the constant may stay for the messages it builds while the read itself names the key.

Failing-first proof, run by the integrator against the final source: changing the `w <= n` guard to `w < n` so the `w == n` case returns a measured count fails `tests/test_strategies_market_breadth.py::test_mp_below_count_unavailable_when_window_not_longer_than_names` by name (`assert 'ok' == 'unavailable'`), with the source restored byte-identical (sha256 asserted).

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A forward-rank study of the liquidity estimators the engine already has (2026-09-23).** X6 of the survey's adoption set (wave 1) - an **offline study, with no gate and no new estimator**. `scripts/liquidity_forward_rank.py` computes `rank_ic(illiquidity_month_t, fwd_return_{t+1})` over the report panel, using the existing `strategies/liquidity_risk.py` `amihud_illiquidity` and `kyle_lambda` and the existing `strategies/signal_analysis.py` `rank_ic`. Neither module was modified: the study adds an estimator nothing and a statistic nothing, which is the point - the question is whether an estimator the engine already reports predicts the *next* month, and the card's own caveat is that on daily bars the trade-direction proxy is crude and the estimator's quality at that frequency is unmeasured.

The table names its window (`return_basis = month-end t -> month-end t+1`), and every row where trade direction cannot be determined reads `unavailable` **with its reason**, never a substituted value. An empty panel is reported rather than crashed on. Nothing in `tradingagents/**` calls it (ground rule 8: offline by construction), and it adds no public function to `strategies/` or `dataflows/`, so the calc-wiring gate is untouched.

**A real run on the engine's own data, reported as it came out.** Over the 2026-09 panel: 25 symbols, one month, and **every forward leg `unavailable`** with the reason "no month-end close for 2026-09 -> 2026-10" - the forward month is unrealized as of today, so there is no honest number to print yet. The 25 names' `amihud_illiquidity` and `kyle_lambda` are computed from the engine's own OHLCV chain and shown beside it. That is the deliverable: a study that refuses to report a rank IC it cannot support.

**Two decisions a reader needs.** (1) `--panel` defaults to the repo's `reports/` tree (one directory per run, each holding the `research_decision.json` the engine wrote), **not** the WP-10 metric panel under `<data_cache_dir>/panels/` - those rows are `{metric: value}` with no volume column and cannot feed either estimator. (2) `kyle_lambda`'s default `min_obs=30` exceeds a month of daily bars (~21), which would make every row unavailable *by construction*; the study passes `min_obs=5` through its own flag. The formula is untouched - only the guard's threshold for this frequency, stated so the next reader does not mistake "always unavailable" for a defect.

Failing-first proof, run by the integrator against the final source: shifting the return leg from `t+1` to `t` fails `tests/test_liquidity_forward_rank.py::test_forward_window_not_contemporaneous` by name (`assert 'return_t' == 'fwd_return_t+1'`), with the source restored byte-identical (sha256 asserted). Two further mutations were proven the same way by the builder: substituting `0.0` for an undetermined value, and marking an empty panel `ok`.

**Web impact: none.** A new offline script; no tool, flag or JSON shape changed.

**A tail read can now say whether the tail came in one print or in a diffusion (2026-09-23).** V6 of the survey's adoption set (wave 1), behind `enable_jump_robust_proxies`, default off. `tradingagents/strategies/volatility_models.py` gains `bipower_proxy(ohlc, window=None, *, min_obs=20)`, `quarticity_proxy(...)` and `jump_robust_proxies_enabled(config=None)`: the daily-bar analogues of bipower variation and realized quarticity - `BPV = (pi/2) * sum |r_i||r_{i-1}|`, `jump share = max(RV - BPV, 0)/RV`, `RQ = (n/3) * sum r_i^4` over close-to-close log returns - so a jump is distinguishable from a diffusion **without tick data**. Every output is named a *proxy* in its docstring, its record keys and its basis string, because that is what it is: the paper's value is its tick-level archive, and only the daily-range subset is reproducible here. Below `min_obs`, or on a zero-variance window, every value is `None` with the reason in `basis` - never `0.0`.

The read fires from `strategies/book_risk.py::extreme_quantile_var`, the EVT/GPD tail read, which today measures tail risk "without asking whether the tail came in one print". It gains a `jump` leg - `{tail_shape: one_print | mixed | diffuse | unavailable, jump_share_proxy, bipower_proxy, quarticity_proxy, n, basis}` - with the two verdict thresholds named as `_ONE_PRINT_SHARE` (0.50) and `_DIFFUSE_SHARE` (0.25) rather than buried in a comparison. With the gate off, or with no bars, that key is **absent** and the record is byte-identical to the pre-V6 one.

**Rule 2 shaped the record, not just the prose.** Neither proxy publishes a realized variance of its own - the jump share is computed from an internal RV and never exposed - so the module gains no second authority beside `semivariance`'s `rv` or `regime.realized_vol`.

**Two notes for the next reader.** (1) The card named `tests/test_volatility_models.py`, which did not exist; it now does, holding the card's test. The pre-existing module suite is `tests/test_strategies_volatility_models.py`, which was left untouched and stays green (24 passed together with `test_formulas_p2.py`). (2) **Stated gap:** the tool wrapper `analysis_tools.get_tail_extreme_var` is not wired to pass the bars it already fetches, so today the jump leg fires only when a caller supplies the window. The rule-7 chain is satisfied through `book_risk` and the calc-wiring gate is green, but making the read visible in a run needs one extra argument at that tool; it is queued rather than silently skipped, because that file carries another item's edit in this same round.

Failing-first proof, run by the integrator against the final source: dropping the lag-1 cross-product term so the proxy collapses to a squared return fails `tests/test_volatility_models.py::test_jump_proxy_separates_one_print_from_diffusion` by name (`assert 0.0 > 0.0`), with the source restored byte-identical (sha256 asserted).

**Web impact: none.** No tool, CLI flag or JSON shape changed.

**A deflated Sharpe is now deflated by a measured search, not by a number the caller supplied (2026-09-23).** H1 of the survey's adoption set (wave 0), behind `enable_trial_ledger`, default off. `tradingagents/strategies/trial_ledger.py` is new: `record(candidate_id, window, returns_sha, sharpe, as_of)` appends one immutable row per evaluated candidate, and `trial_stats()` reads the trial count `N` and the trials' Sharpe dispersion `V` **back from those rows** - never from a caller's assertion - reporting `unavailable` (never `0.0`) with fewer than two usable Sharpes. `evaluate.deflated_sharpe` gains an optional `sharpe_dispersion`, and with it supplied *and* the gate on, the selection threshold becomes the closed form

`SR*_0 = sqrt(V) * [ (1-g)*Phi^-1(1 - 1/N) + g*Phi^-1(1 - 1/(N*e)) ]`, `g` = Euler-Mascheroni,

instead of the pre-existing `sqrt(2 ln N)` approximation, which is preserved **bit-for-bit** when no dispersion is supplied. `deflated_sharpe_report` returns the number beside its `N` and tags whether the dispersion was `measured` or `assumed`, so a deflated figure can no longer be read without its search width. `alpha_zoo.bench_zoo` gains a ledger path (`trial_ledger_dir=`): candidates are recorded, `N`/`V` are read back, and each row carries `deflated_ic_n_trials` and `deflated_ic_dispersion`; the default path adds no keys at all.

**Two things the card got wrong, and one gap left open.** (1) The card writes `sqrt(V)` with `V` called "the trials' Sharpe dispersion" - implemented as the **variance** of the trials' Sharpe ratios, because that is what makes the threshold come out in Sharpe units; a caller passing a standard deviation must square it, and the docstring says so. (2) The card's instruction "CSCV uses `S = 8`, not `S = 16`" has **no target in this tree**: `evaluate.py` carries no `S` constant at all, and the combinatorics knob is `purged_cpcv_splits(n, n_splits=5)`, whose default is pinned by an existing test (`test_splits_cover_all_indices`). Nothing was changed there, deliberately - the instruction describes a paper's configuration, not this engine's code. (3) **Stated, not hidden:** the publication site `scripts/score_panel.py` still passes its own `n_trials` into `deflated_sharpe`. It already carries `n_trials` beside the value, but that number is an assertion; switching that one call site to the ledger is a follow-up, deferred only because that file carries the H11 interval attachment in the same round. `scripts/coverage_scorecard.py` publishes no deflated number at all, so the card's second publication leg has nothing to carry an `N` today.

**The gate reader needed the same fix H10's did.** `trial_ledger.gate_on` first read its key through a module constant (`GATE_NAME`), which the registry's read-site scan cannot see: the gate read as "claimed wired but nothing reads it". It now reads the literal key, for the reason recorded in its docstring - a gate read through a variable is a gate the registry cannot check. Failing-first proof, run by the integrator against the final source: making the dispersion branch unreachable (`if False and ...`) fails `tests/test_trial_ledger.py::test_dispersion_changes_deflated_sharpe` by name, with the source restored byte-identical (sha256 asserted). Focused set: `test_gate_env_toggles.py`, `test_api_reference_env_table.py`, `test_trial_ledger.py`, `test_strategies_evaluate.py`, `test_alpha_zoo.py` - **35 passed**.

**Web impact: none.** No tool, CLI flag or JSON shape changed; the app's `GET /api/config` picks the new default-off key up on its own.

**The survey's adoption set landed its first three items: a coverage window, a cross-strike skew proxy, and a trend-spectral read beside the swing factor (2026-09-23).** Wave 0 (`H10`) and wave 1 (`K3`, `X8`) of `docs/paper_survey_26/` - all additive, all default off, none of them a producer of a quantity the engine already owns. They land together because their gate registrations share four files (`default_config.py`, `.env.example`, `docs/gate_registry.md`, `tests/test_gate_env_toggles.py`); splitting them would leave a commit whose gates are registered before their read sites exist, which the registry test checks and would fail.

**H10 - a panel statistic refuses to compute over a padded window.** `tradingagents/strategies/coverage_window.py` is new and is a **reader** of the already-loaded frame - `coverage_window(series)` reports `{first_valid, last_valid, n_bars, padded_days, alignment}` - never a second data-quality authority, which is why `market_data_validator._INDICATOR_MIN_BARS` and `volatility_models._IGARCH_AB` are untouched: H10 adds the window, not the threshold. `strategies/data_quality.py::panel_statistic` reads that window and reports the statistic `unavailable` when `padded_days > 0`, because a mean over a padded window averages histories that did not trade together; on a dense frame the statistic is the plain mean and is unchanged. `SURVIVOR_ONLY = "survivor_only"` is the one definition site for the label a current-constituent backtest must carry - nothing writes it today, because no such backtest runs in-run. `scripts/coverage_scorecard.py` gains a per-symbol window section read from each tree's own `tool_evidence.json` (the run's own input, no new vendor call); reading the file corrected the plan, which had assumed that script reported per-symbol facts - it reports per-**engine** fill rates aggregated across trees.

**K3 - the option-implied skew read is a labelled proxy, conditioned on the regime cell.** `strategies/options_surface.py::rn_skew_proxy(rows, regime_cell=None)` is the OLS slope of the fractional IV deviation on standardized moneyness over OTM rows only, dimensionless so a percent-quoted chain gives the same number. It requires **five** distinct OTM strikes and returns `unavailable` (never `0.0`) below that, its record names itself a **cross-strike IV proxy** rather than a BKM moment, and it carries the regime cell it was conditioned on with `conditioned: True/False`. Its only read site is `agents/utils/analysis_tools.py::get_vol_surface_shape` behind `_flag("enable_rn_skew_proxy")`. **The plan's named enforcement site is deliberately not wired:** feeding a scored, regime-conditional skew read into `risk_score` is the owner's decision (the plan's own open decision 4), not something to slip in behind a new gate. The plan also cited a `tests/test_options_surface*.py` that does not exist; the real guards are `test_quant_p1_options.py`, `test_formulas_p4.py`, `test_phase8_ops.py` and `test_analysis_tools.py`.

**X8 - a trend read that states when the swing factor deserves weight.** `strategies/technical_factors.py` gains `spectral_excess_mass(returns, span=250)` - the low-frequency spectral excess in volatility-normalized returns, net of the white-noise null bias, with `condition_met` at twice the analytic 2-sigma band (measured false-positive rate **0.003** over 2000 white-noise draws, against ~0.5 for a bare positive sign) - and `cost_optimal_span(cost_bps, vol)`, which converts the engine's own cost estimate and the symbol's volatility into a lookback. Both are reported **beside** the swing factor, never inside its score: `strategies/swing.py::swing_report` gains one additive `trend_spectral` key when `enable_trend_spectral` is on, and with it off the returned dict is exactly the 12 keys it had before, verdict unchanged either way (asserted).

**The gates are read, which took a fix on H10.** Its first cut implemented the refusal but never consulted `enable_coverage_window`, so the behaviour was ungated and `tests/test_gate_env_toggles.py::test_wired_gates_are_read_somewhere_and_inert_ones_are_read_nowhere` said so in as many words ("claimed wired but nothing reads it"). Both H10 callers now read the flag through the house idiom - `get_config()` then `cfg.get("enable_coverage_window", False)`, a config read that can never break the read it guards - so with the gate off no window is read and no new section is printed. Failing-first proofs, each run by the integrator against the final source: removing H10's `padded_days > 0` refusal branch fails `tests/test_coverage_window.py::test_padded_panel_is_unavailable` by name (`assert 30.5 is None`); lowering `RN_SKEW_MIN_STRIKES` from 5 to 4 fails `tests/test_rn_skew_proxy.py::test_proxy_unavailable_below_five_strikes`; returning the zero-cost span fails `tests/test_strategies_technical_factors.py::test_cost_optimal_span_is_monotone_in_cost`. Every mutated source was restored byte-identical (sha256 asserted).

**Web impact: none.** No tool, CLI flag or JSON shape changed. The app's `GET /api/config` derives the gate half of its allowlist at request time, so the three new keys appear on their own; the count in its README is corrected in the sibling repo once this wave lands.


**The survey's adoption plan is now six per-theme plans, one beside each design doc (2026-09-23).** `docs/implementation_plan_paper_survey_26.md` - a single 692-line plan covering all 49 items - is deleted and replaced by six plans in `docs/paper_survey_26/`, the folder that now also holds the six themed design docs it sequences. One plan per theme (honesty, regime, risk and tails, volatility and options, cross-section and allocation, news and filings), each carrying its own items' targets, gates, failing-first tests and acceptance criteria, and each sitting beside the design doc that owns the argument. What is genuinely shared - the eight inherited ground rules, the measured dependency surface, the cross-theme phase map, the hard ordering constraints, the in-run-versus-offline split against the measured ~40-minute four-symbol budget, the eleven open owner decisions and the landing protocol - moved into `docs/paper_survey_26/README.md` instead of being repeated six times, because six copies of a rule is six places for it to drift. No item changed in the move: same 49 items, same 34 config gates plus one extension, same P0-first ordering, same zero `COMPOSITE_ENGINES` changes. The split surfaced one defect, in the survey's own header: its plan cross-link had been inserted inside the `**Method:**` paragraph and split that sentence, and it is repaired here. **Web impact: none** - docs only; no tool, flag, gate or JSON shape touched.

**The corpus survey now has its adoption plan, and writing it found one real defect (2026-09-23).** The plan set written for the six themed design docs (since split per theme) sequences them into five phases (P0 measurement instruments, P1 zero-new-data state reads, P2 composite reads over existing producers, P3 new producers, P4 offline and panel-scale) covering 49 items, each with its target files and symbols, its config gate, the test that must be proven failing first, and its acceptance criteria. It is grounded rather than asserted: all 44 `file.py:LINE` anchors the six docs cite were re-verified against the tree (44/44 held), the dependency surface was measured rather than assumed, and the load-bearing paper ids were spot-checked against arXiv.

Three measurements change what the plan can say. **scikit-learn is absent**, so X2's double-selection LASSO, X5's gradient-boosted peers and V1's forecast pool each need either a new declared dependency or a hand-rolled equivalent. **torch 2.7.1+cpu is installed but undeclared** and imported nowhere, which matters because the news doc had assumed N1's tagging and N6's comparison would need "torch or a hosted encoder" - local torch is available, but only if the dependency-profile change is made explicit. **`tsbootstrap` is a real public library** matching 2607.06690's claim (block, residual, sieve and wild bootstrap plus EnbPI, ACI, NexCP and AgACI), so H11 can adopt it or keep the thirty-line in-house block bootstrap whose block-length rule is already specified.

The plan also carries the corpus's negative results as *constraints* rather than as builds - R8 (regime arrival is not estimable from training data, `rho = 0.084` with a CI containing zero) and R9 (appending regime variables to a feature vector degrades both accuracy and stability, IC 0.5469 gated vs 0.5378 appended) - and it holds the standing rule that nothing enters `COMPOSITE_ENGINES` by adjacency: six docs, 49 items, zero member changes.

**Two direct imports were undeclared, so a clean install could not import the engine's statistical surface (2026-09-23).** `tradingagents/strategies/statistical.py:18` does `from scipy import stats` and `:19` `from scipy.special import betainc`; `sentiment_research.py:27` does the same; five modules import numpy directly. Neither numpy nor scipy was declared in `pyproject.toml` or `requirements.txt`. The gap is real and not theoretical: **every scipy edge from a declared dependency is extras-gated** - `pandas -> scipy>=1.14.1; extra == "computation"`, `yfinance -> scipy>=1.6.3; extra == "repair"`, `networkx -> scipy>=1.11.2; extra == "default"` - so a clean install of the declared manifest resolves **no scipy at all**. The only packages requiring it unconditionally in the working interpreter (`PyMatting`, `albumentations`, `colour-science`, `rembg`, `scikit-image`) are image packages this engine does not depend on. The module is reachable at runtime, not dead code: twelve sites in `agents/utils/analysis_tools.py` import from it, plus `strategies/lottery.py:74`. Fixed by declaring `numpy>=1.26.0` and `scipy>=1.11.2` in both manifests, which mirror each other. The floors are conservative because the API surface in use is long-stable - `np.array`/`np.linalg`/`np.asarray`, `scipy.stats`, `scipy.special.betainc`, `scipy.optimize` - and no numpy-2-only API is used.

Web impact: none. No tool, flag, gate or JSON shape changed; the app's contract tests pass unchanged.

**The interactive CLI and the batch path stopped producing different reports (2026-09-23).** The owner reported that a batch report and an interactive `tradingagents` report differ in content and format. They did, and the cause was structural rather than a rendering difference: the CLI builds its own state and streams the raw compiled graph instead of going through `propagate()`, so it had re-implemented the graph's setup by hand - and that copy had drifted.

**Three defects, one root cause.**
- **The engine scorecard never reached the CLI's state.** `_run_graph` seeded `init_agent_state["quant_scorecard"]` before the graph ran; the CLI's copy seeded `risk_context` (added later, under a "Mirrors propagate()" comment) but not the scorecard. Every interactive run's four analyst reports and `run_card.json` were therefore missing the engine sections the batch path renders. Proven on the owner's own trees: batch `MCD_20260923_123808/1_analysts/market.md` carries `## TechnicalScore (engine score) - 45.01/100`, and the interactive `MCD_20260923_145910/1_analysts/market.md` has no engine section at all.
- **Interactive agents ran without the memory log.** The CLI passed no `past_context` and no aggregate track record, so the Portfolio Manager could not weigh its own historical accuracy on that path.
- **Interactive runs wrote no state log, no ledger row and no memory entry.** The CLI stopped after `_apply_strategy_overlays`. `MCD.md` in the memory log was stamped 12:38:08 (the batch run); the 14:59 interactive run wrote nothing, so its decision never reached deferred reflection.

**The fix is one producer per step, not a re-sync.** The pre-graph setup moved into `TradingAgentsGraph.prepare_initial_state(...)` and the post-graph work into `TradingAgentsGraph.finalize_run(...)`; `_run_graph` and `cli/main.py` both call them, so there is no second copy left to drift. `prepare_initial_state` also sets `self.ticker`, which only `propagate()` used to set and which `_log_state` read to name the state log, so a CLI run could not write one at all; `_log_state` now takes the ticker explicitly instead of depending on that attribute.

**Proven by a real run, not only by tests.** `py -3.12 -m cli.main --symbol MCD --no-display-report --save-path _parity_check/MCD_cli` (the non-interactive CLI path, saved outside `reports/` so the live `--execute` signald daemon could not see it). The tree it produced carries `quant_scorecard` with all eight engines scored (`scorecard_status: PARTIAL`), an engine section in every analyst report (`## TechnicalScore (engine score)`, `## NewsScore (engine score)`, `## FundamentalScore (engine score)`, plus the scorecard table in the sentiment report), `risk_disagreement`, and a written state log (`full_states_log_2026-09-23.json`). Its decision landed in the memory log at 17:36:30 as a pending MCD entry. None of those existed on the CLI path before.

**Four differences that REMAIN, and are not defects.** Stated so the next comparison is not misread as a regression: (1) the CLI sets `quick_think_llm`/`deep_think_llm`/`llm_provider` from its own picker where batch reads `.env` - the compared trees ran `deepseek/deepseek-v4.1-flash` and `openai/gpt-6-luna-pro` respectively, which is why `config_hash` differs (`04058a07...` vs `de7ea1a8...`) and why the analyst prose differs in length; (2) batch's `--depth` defaults to `deep` while the CLI's picker starts highlighted on Shallow; (3) batch writes `jev_verdict.json`/`verify_flags.json`, which the CLI does not; (4) **the memory log is split** - batch writes a per-symbol file (`memory/MCD.md`, a deliberate concurrency choice so parallel workers never interleave appends) while the CLI writes the shared default (`memory/trading_memory.md`), so the two paths still cannot see each other's history. Item 4 is a real functional gap, left alone because changing where decision history lives is the owner's call, not a silent fix.

**Verification.** `tests/test_cli_propagate_parity.py` (new, 8 tests) pins what the two shared producers owe every entry point: the scorecard reaches the state when the gate is on and its key is absent entirely when off (the documented `ResearchLayerWiring.md` §9.3 byte-identical guarantee), resolved memory entries reach `past_context`, the run ticker is available to the state log, and `finalize_run` records the decision, writes the state log, returns the overlay-applied state and appends rather than overwrites. `tests/test_cli_no_console.py::test_cli_applies_strategy_overlays_and_seeds_risk_context` - the existing guard for an earlier instance of this same divergence - now asserts the CLI drives the shared producers in order instead of calling the internals directly; `tests/test_memory_log.py::test_full_pipeline_no_regression` binds the two new methods alongside `_run_graph`. Failing-first proof: removing the scorecard seed fails the scorecard test and stubbing the memory write fails the deferred-reflection test, both named in the output, with the source restored byte-identical (sha256 asserted). Full suite: **5349 passed, 6 skipped, 0 failed**.

**Web impact: none.** The app reads the engine's report trees and this makes them more consistent, not less; no key, file name or JSON shape changed. The app's contract tests pass unchanged.
**The 2026 arXiv `q-fin` corpus was read in full, and it produced seven documents (2026-09-23).** Owner's instruction: read every PDF in `e:\fin paper` whose id starts `26`, understand each, and write design docs for what this engine can learn from them. 309 papers, January-September 2026, 728 MB.

**Method, so the coverage claim is checkable.** `.state/metadata.jsonl` supplied title and abstract for all 309; full text was extracted from all 309 PDFs and read directly for a 64-paper shortlist. Every paper carries a recorded verdict - **31 high, 111 medium, 124 low, 43 none** - and the `low` and `none` verdicts are kept in the appendix rather than dropped, because a survey that lists only what it liked is a wish list. Of the 309, exactly **one** (`2602.00086`) was already cited anywhere in this repo.

**What the corpus actually says, in one line each.** (1) Its largest transferable theme is not a signal at all: it is **how to tell whether a financial claim is real** - 42 papers across four measurement surfaces, and the sharpest of them shows a deliberately leaky oracle posting Sharpe 35 scoring DSR 1.00 and surviving the overfitting test completely, which is why look-ahead must be inexpressible rather than corrected. (2) **Most of what is forecastable is volatility, not returns** - a cross-sectional volatility rank carries multi-step memory while the matching return rank is close to unforecastable, and several papers find an LLM's apparent edge dissolving against a non-LLM baseline fed identical decision-time inputs. (3) **Coverage is a first-order variable**: calendar-aligning instruments with different listing histories suppresses measured volatility by ~20% and breaks GARCH estimation outright in 22 of 53 cases.

**The documents.** `docs/design_fin_paper_survey_26.md` is the index - the theme synthesis plus all 309 papers by verdict. Six themed design docs follow it, now in `docs/paper_survey_26/`: `design_research_honesty_gates.md` (12 proposals), `design_regime_estimation_hardening.md` (9), `design_risk_tail_and_coverage.md` (6), `design_vol_surface_and_vrp.md` (8), `design_cross_section_and_allocation.md` (8), `design_news_and_filing_signals.md` (7). Every proposal names what to compute, its inputs, its point-in-time status, its cost, the owning module, and the paper's own caveat.

**Three findings about THIS repo, found while reading - two of them corrections to the survey itself, recorded because a survey that only proposes is not evidence.**
- **`liquidity_risk` already computes what one paper was routed here to add.** `amihud_illiquidity` (`liquidity_risk.py:71`) and `kyle_lambda` (`:262`) both exist; the paper's real contribution is a forward-rank *test* of them, and the design doc was rewritten accordingly rather than proposing a rebuild.
- **`volatility_models._IGARCH_AB = 0.999` is the exact threshold one paper reports as GARCH breakdown** on padded panels (`volatility_models.py:40`), and `market_data_validator._INDICATOR_MIN_BARS` (`:34`) already guards short history. The engine is closer to that paper's remedy than the survey first assumed; the missing half is recording the observation *window*.
- **A module docstring promises two functions that do not exist.** `strategies/sentiment.py` documents `consensus_over_seeds` and `agree_rate` at docstring lines 7 and 10, and neither is implemented anywhere in the package. Noted in `design_news_and_filing_signals.md` §2 rather than fixed here, because adding them is a behaviour change and this entry is docs-only.

**Docs-only.** No code, no test, no gate and no `.env` line changed; all seven files are new `.md` under `docs/`. Every `file.py:LINE` citation in them was checked against the tree by script - **85 of 85 resolve**, and every markdown table was validated for ragged rows (none). **Web impact: none** - the app's contract tests read the engine's tools and JSON shapes, and nothing here touches either.

**Max pain is now a scored TechnicalScore component, and two gates went on (2026-09-23).** The owner's calls: score it, monthly expiry, and turn on `enable_security_context` + `enable_options_surface`.

**`derivatives_gamma.max_pain(rows)`** computes `argmin_S [ sum(max(S-K,0)*call_OI) + sum(max(K-S,0)*put_OI) ]` over the listed strikes - the settlement at which the most option value expires worthless, which is the standard construction (verified against sources, not taken from the write-up that prompted it). It returns `None` for fewer than two strikes carrying open interest: one strike cannot make a minimum, and absent OI is not zero OI. Duplicate strike rows ADD rather than overwrite - a test pins that, because overwriting would silently halve a strike's weight.

**Monthly expiry, as asked.** `_options_chain_rows_lambda` gained `monthly=True`, which selects the standard third-Friday expiry from `opex_dates` instead of the third *listed* expiry. Max pain is expiry-specific, and pooling expiries is the standard way to produce a meaningless number; the pre-existing default is unchanged for the gamma/IV reads.

**Scored: `max_pain_dist_atr` in `price_structure`**, `lower_better`, ramp 0-2 ATRs - spot sitting on the pin is the mean-reverting read and outscores spot 2+ ATRs away. **This changes the composition**: `price_structure` went from 4 components to 5, so the 12 category points now split five ways and several long-standing component scores shift. That is the direct consequence of "scored", not a side effect.

**The one component that fetches.** `_technical_components`' docstring promised "no new fetch, no new formula"; max pain needs the monthly chain, which the OHLCV cache does not hold. The docstring now names that exception, the fetch is best-effort, and the component is absent rather than scored 0 when the chain or the ATR is missing. Cost is visible: a live `_technical_components('GOOG')` call took 92 s end-to-end and returned 39 components with `max_pain_dist_atr = 1.1467`.

**Gates on** (`.env`, which is gitignored - recorded here because the behaviour is not):
- `TRADINGAGENTS_ENABLE_SECURITY_CONTEXT=true` - the classification block the SC layer writes into `run_card.json`, built earlier today and until now producing nothing. Next run's card carries it; `scripts/security_context_ledger.py` is how to see it across runs.
- `TRADINGAGENTS_ENABLE_OPTIONS_SURFACE=true` - the options-surface reads (IV / implied move / parity). **Web impact: this one is additive to the app** - `trading_web` registers `options_iv_read` (`backend/capabilities.py:1243`, `docs/web_TOPICS.md:83`), so that Value-tools entry becomes functional rather than returning its gated notice. No app change needed and none made.

**One test deleted, and why (not a re-pin).** `test_the_shipped_gate_defaults_off` asserted `DEFAULT_CONFIG["enable_security_context"] is False` while claiming to test the shipped *default*. `DEFAULT_CONFIG` is the resolved dict (`_apply_env_overrides` runs at import), so it was asserting the *live* policy and calling it a default - the `.env` flip exposed that. Its durable claims are each owned elsewhere (`tests/test_gate_env_toggles.py`'s REGISTRY pins the key-to-env mapping; the SC-7 tests pin the gate-off byte-identical card), so the test is deleted rather than re-pinned to `is True`: a test that must be rewritten on every policy change is not a contract.

**Tests.** 3 new in `tests/test_derivatives_gamma.py` (the argmin, duplicate-row merging, and the four degenerate refusals). No new technical-score test: `test_monotone_ramps_are_ordered` already iterates EVERY `RAMPS` entry, so it covers the new ramp's direction generically - a bespoke test would have been the same path twice. Full engine suite: 5341 passed, 6 skipped, 0 failed (the previous run 5341 passed + 1 failed, now green). `ruff check .` clean.
...
**Provenance travels with the value, because the repo has four taxonomies under one string.** `yfinance_sector.fetch_sector` was documented as returning "GICS sector" while its four legs return FMP's own taxonomy, Finnhub's proprietary `finnhubIndustry`, Yahoo's sub-industry label and the repo's own ETF label - and the answering source was discarded. Every value here carries the source that produced it, and **no field is named GICS**: the repo has no GICS source, code or licence. The canonical label is reached through the repo's ONE normalizer, `_canonical_sector`.

**Three decisions the design forced, taken explicitly rather than silently.**

1. **No cross-source agreement field.** The draft carried `agreement` / `disagreement` over "two sources that canonicalize differently". `resolve_instrument_identity` makes **one** vendor read (`agent_utils.py:595`), so `agree`/`disagree` were unreachable: `sector_source` could only ever be `"yfinance"`, the dict could only be empty, and the `disagree` -> confidence-penalty input was dead. A state that cannot occur is worse than a named absence - it reads as a check that ran and passed. **Deleted**, with the design doc's §9.2 shape, §9.4, §19 tests 3-4, §24 criterion and §12 input corrected in the same pass. The SEC SIC survives as an **independent regulatory signal, recorded and never compared** (comparing it needs the SIC-to-sector crosswalk this layer refuses to build). Your call, 2026-09-22.
2. **The matrix is named for what it does.** `THEME_PRIORITY_MATRIX`, not "applicability matrix": applicability implies "not applicable -> not evaluated", which is exactly what the invariant forbids. The design doc's own Appendix B already preferred the rename.
3. **The industry bridge is named for what it is.** `industry_implied_sector` - an industry value mapped to a *sector*, not a normalised industry. The repo has no industry vocabulary and does not claim one.

**It makes no network call of its own.** The context reads the already-`lru_cache`d identity, and the SIC is recovered from the payload `get_sec_filings` **already fetched and then discarded** - a new `sec_edgar.peek_sic` reads what the run paid for and **never fetches**, so `(None, None)` means "not read", never "no SIC exists".

**The 66-cell matrix is declared policy and wants your review.** Every cell carries a one-line policy basis, enforced by the validator - a cell that cannot state its rationale does not belong in the matrix - and relevance is **categorical**, so no cell can be quoted as a score. Two rows (technology, energy) are the design doc's own; the other nine are declared here at `MATRIX_VERSION 2026-09-22.1`. A wrong cell costs a missed *earlier* check, never a missed theme.

**A contract that named a state it could not return - fixed at the PRODUCER, not the report (found while building).** `strategies.security_type.classify_security` advertised `security_type: "ETF" | "operating_company" | "UNKNOWN"` in its own docstring, in `docs/api_reference.md:1054` and in `docs/design_etf_fundamental_valuation.md:233`'s acceptance criterion - and `docs/implementation_plan_defect_audit.md:114` (P0-9) asked for `operating_company` for JPM/GS/MS/BLK/SCHW/NTRS/STT **by name**. The fallback returned `UNKNOWN` for every equity, so one of the three advertised states was **unreachable**. Four documents agreed and the code did not, which is this repo's recurring class: a field whose name promises more than it delivers. Fixed in the producer - an `EQUITY`/`stock` provider `quote_type` is positive evidence and now returns `operating_company` (confidence `high`). **Behaviour is unchanged for every consumer**: they branch only on `security_type == "ETF"` (`fundamentals_analyst.py:61`, `quantitative_scores.py:524/606`, `is_etf`), so an operating company keeps exactly the company path it had as `UNKNOWN`; only the label it prints in a basis changes. A bare company name is still **not** proof, so a fund that escapes the universe lists and the wrapper patterns stays honestly `UNKNOWN`. The two tests that pinned the old label are re-pointed, and the claim they exist to defend - a bare issuer name must not make an operating company an ETF - is preserved verbatim in the `not is_etf` half.

**The gate-registration rule under-counted its own list (found by adding a gate).** `docs/gate_registry.md` §8 said a new gate needs **five** places. There are **six**: the sixth is the `docs/api_reference.md` §1.1 env-var table, which `tests/test_api_reference_env_table.py` regenerates from every env var the config reads - so a gate missing from it fails the suite with *"the table omits 1 env var the config reads"*. The rule now lists all six, names which are machine-enforced, and records that the count was wrong until today. `docs/api_reference.md` gained the one row (`scripts/gen_api_reference_table.py --write`; the regenerated table differs by exactly that row).

**Noted, not changed - the wiring gate counts a MENTION as a reference.** `tests/test_calc_agent_wiring.py` decides "reachable" with a substring count over the production sources (`blob.count(fn) - text.count(fn) > 0`), so naming a symbol in a docstring satisfies it. The new module cites `factor_schema.validate_schema` as the pattern its own validator mirrors, which silently retired that symbol's parametrized case in that file (33 collected -> 32). Nothing is mis-wired - the symbol keeps its `LEGACY_WHITELIST` entry and its stated reason - but the gate now believes it is reachable, so a future removal of that entry would go unnoticed. Reported rather than changed: tightening the gate to require a *call* would surface every other entry that is only textually referenced, which is a separate piece of work.

**Web impact**: none - a new default-off gate and one additive `run_card.json` key. No tool, CLI flag, JSON shape, score, gate or returned-dict key changed; `trading_web`'s `read_report_tree` walks artefacts generically, so the new card key is invisible to it.

**Tests.** 24 new in `tests/test_security_context.py` - the widening property over every sector, provenance, the one normalizer, the validator's four negative cases, determinism, no lookup when handed an identity, graceful degradation, the one-line render's three required properties, and the gate read both ways. Full engine suite: 5315 passed, 6 skipped, 0 failed - 5292 before this change. **The delta is +23, not +24**: the new file contributes 24, and one parametrized case in `tests/test_calc_agent_wiring.py` disappears (see the note below). `ruff check .` clean.

**R4 stage 1b - the gate registry's scope paragraph named 8 of the 48 keys it omits (2026-09-22).** Stage 1 narrowed the title away from "every gate" and named the eight score-engine gates as covered elsewhere, but **48 `enable_*` keys have no row** and the paragraph named only those eight. A reader trusts the *absence* of a row, so the softer title was the same over-claim in a lower voice: `enable_reflection`, `enable_debate`, `enable_independent_vote`, `enable_events`, `enable_factor_model` and `enable_screener` all still read as "no such gate". The title now says **registered**, and the scope paragraph states the numbers - **40 of the 88 `enable_*` keys** covered, **48 with no row** - and lists every omitted key in six families, with the sentence that makes the omission safe to read: a missing row means *not registered yet*, **never** "no such gate". A new test recomputes both numbers from `DEFAULT_CONFIG` and `REGISTRY` and fails if any omitted key goes unnamed, so stage 2 cannot add a family and leave the doc behind. **No row was added** - the 48 evidence rows remain deliberate future work, one family at a time.

**Web impact**: none - a title, a scope paragraph and one test in the engine repo; no tool, CLI flag, gate, JSON shape or returned-dict key changed.

**Also in this commit - the parent design doc is now TRACKED.** `docs/Conditional_Research_Overlays_Design.md` (1565 lines) was untracked while both this design doc's §13/§22 and the CHANGELOG cite it, so the reference dangled for any other reader. The owner authorised tracking it; it is committed **verbatim** (unchanged, 32024 bytes). It remains in history at `04890ec` regardless - removing that would need a force-push, which is not done.

**Tests.** 1 new (`tests/test_gate_env_toggles.py::test_the_doc_states_the_whole_enable_surface_and_names_every_excluded_key`). Full engine suite: 5292 passed, 6 skipped, 0 failed (5291 before, +1). `ruff check .` clean.

**Four reported defects fixed - and the report itself was WRONG about one of them (2026-09-22).** The four behavioural defects the security-context design reported for a decision (`docs/design_security_context.md` §20.2) are fixed on the owner's instruction, each with a failing-first proof.

**R1 - `finnhub.get_profile_finnhub` no longer renames the vendor's field.** The function copied `finnhubIndustry` onto a key named `sector` so its one caller could read `prof["sector"]` - which made a key named *sector* hold an *industry* value and hid the taxonomy from every consumer, while the function's own docstring promised the "raw" profile dict. It now returns the payload unmodified (`finnhub.py:468`, comment at `:482-493`) and `fetch_sector` does the mapping (`yfinance_sector.py:112`), preserving the old precedence exactly: `prof.get("sector") or prof.get("finnhubIndustry")`. The old rename only fired when `sector` was absent, so **behaviour is unchanged for real payloads** - the mapping moved, the precedence did not. Two test mocks that injected the post-rename shape now inject the real payload shape.

**R2 - the `industrygroup` alias is deleted, and the earlier "dead alias" claim was WRONG.** The report said the alias could never fire because `_norm` keeps the space in "Industry Group". That is true for the SPACED label and false for an **unseparated** one: `_norm("IndustryGroup")` has no separators to replace, so it matched the alias exactly - the failing-first test caught it. The consequence is real: `fin["sector"]` is the Altman variant selector (`statement_parsing.py:1565`) and feeds the Piotroski basis, and `altman_variant_for` routes it through `sector_group_of`, where a stray financial group resolves to `XLF` and **withholds the Altman variant entirely** for a non-financial company. The key now accepts `["sector"]` only (`statement_parsing.py:187`), with the reason recorded at the alias table.

**R3 - the grouping is named for what it holds (naming only).** `cross_section.industry_neutral_z` took a parameter named `sector_map` (`cross_section.py:90`) while `peer_universe.resolve_growth_medians` (`:304`) computed "Industry medians" - and the values are neither: they are a provider **sector** label from `fetch_sector`, which is neither GICS nor an industry group. The parameter is `group_map` now, both docstrings state the grouping is the caller's, and the locals `sector_mean`/`sec` became `group_mean`/`grp`. The **function name is unchanged** - `industry-neutral` is the standard Grinold-Kahn term for neutralising within the peer group, and renaming it is a 4-site public change (`__all__`, `factors.py` x2, 3 test refs) for a cosmetic gain. Both callers pass positionally, so no caller changed; the returned key stays `n_sectors` (reported, not renamed - a returned-dict contract is wider than a parameter name). **No behaviour change.**

**R4 stage 1 - the gate registry stops claiming the whole `enable_*` surface.** `docs/gate_registry.md` titled itself "every gate" over 40 of the 88 `enable_*` keys. The title now names the three families the file covers, and a new paragraph names where the eight score-engine gates are covered instead (`tests/test_quant_scorecard.py`, `tests/test_engine_ownership_map.py`) and states that growing the file is deliberate future work, one family at a time. **The 48 rows are NOT bulk-added** - an unverified "Proven by" cell recreates exactly the failure mode the file exists to prevent. A test fails if the over-claim returns. **[SUPERSEDED 2026-09-22 - stage 1b: see the newest entry above. Naming only the eight score-engine gates was itself an over-claim; the title now says "registered" and the paragraph states and lists the whole 48-key omission.]**

**Web impact**: none - no tool, CLI flag, gate, JSON shape or returned-dict key changed, and `trading_web` reads none of the four surfaces.

**Tests.** 8 new - 3 in `tests/test_yfinance_sector.py`, 2 in `tests/test_statement_parsing_aliases.py`, 1 in `tests/test_quant_engine_additions.py`, 2 in `tests/test_gate_env_toggles.py`. **6 of them fail against the previous revision and pass after**; the other 2 pin behaviour the change had to preserve (the `sector`-over-`finnhubIndustry` precedence, and the store of the pointer the doc now makes). Full engine suite: 5291 passed, 6 skipped, 0 failed - 5283 before this change, so +8. `ruff check .` clean.

**Design: the overlay layer's missing front end - and the taxonomy the repo mislabels as GICS (2026-09-22, design + 3 docstring fixes).** `docs/Conditional_Research_Overlays_Design.md` specifies a conditional research-overlay layer in 40 sections, and its Overlay Detector has three sub-steps (`doc:263-265`): `materiality test`, `evidence sufficiency`, `applicability test`. **The third has no producer anywhere in that doc.** "Applicability" is named once and never defined; no step reads any property of the company; the registry is keyed only on theme id (`doc:782-789`); and §36's ten open decisions (`doc:1397-1410`) do not include "which overlays are candidates for a given company". §17 (`doc:725-772`) is not that step - its four sector headings all sit under the same theme and enumerate *evidence fields for an already-chosen theme* ("the schema remains the same while the evidence adapters differ", `doc:772`).

**New `docs/design_security_context.md`.** Supplies the missing producer: a deterministic `SecurityContext` (sector + industry + provenance + canonical sector + SPDR key + SEC SIC + security type), a versioned **theme applicability matrix**, and the three-way split of the detector into Classification / Candidate detection / Materiality. Work items SC-1..SC-10 slot into the parent doc's §39 order between items 2 and 3.

**THE KEY IDEA - the widening invariant.** The matrix may **add** candidate themes and may **never remove** one: `candidates = ALL_REGISTERED_THEMES | matrix_candidates(sector)`. The parent brief's own example is why - a company in **Energy** whose AI exposure is material anyway (data-centre electricity demand). If a sector prior could exclude, that case is silently missed. Stated generally: **a prior that can exclude is not a prior, it is a decision.** Monotonicity makes "sector is a prior, not the final decision" a *structural, property-tested* guarantee instead of a discipline, and the matrix's real job becomes ordering evidence-gathering effort under the parent doc's `MAX_ACTIVE_OVERLAYS = 3` budget. Absence is not exclusion: an unclassified company yields **all** themes.

**DEFECT - the repo labels four taxonomies as one "GICS".** `yfinance_sector.fetch_sector` (`yfinance_sector.py:64`) was documented as returning "GICS sector". It does not: its four legs return FMP's own taxonomy (`fmp.py:68`), Finnhub's **proprietary** `finnhubIndustry` (`finnhub.py:468`), Yahoo's label (which the repo's OWN comment at `sector_rank.py:91` records as **sub-industry granularity**), or the repo's ETF issuer label (`yfinance_sector.py:39`). Nothing on that path normalizes to GICS, and **the answering source is discarded**, so a caller cannot tell which taxonomy it received. GICS is also not obtainable here - it is licensed by MSCI and S&P, assigned at company level, and the repo has no GICS source, no GICS code, and no SIC or NAICS anywhere. Three documentation-only claims were corrected (zero behaviour change): the module docstring, the `fetch_sector` docstring (which also omitted the Finnhub leg entirely), and the `finnhub.py:480` comment asserting `finnhubIndustry` is GICS.

**Reported, not changed (behavioural). [CLOSED 2026-09-22 - all four fixed; see the newest entry above.]** The key named `sector` is populated from a field named **`finnhubIndustry`** while the payload's real `industry` is left unread (`finnhub.py:487-488`) - **WRONG on the second half: live-probed 2026-09-22, `company_profile2` carries no `sector` key and no `industry` key at all, so the old rename was the only source of `sector`**; `statement_parsing.py:180` lets the canonical `sector` key accept an `industrygroup` value (a **dead** alias - no payload emits that row - but a latent trap, and `fin["sector"]` selects the Altman variant at `statement_parsing.py:1558`); and `cross_section.industry_neutral_z:89` takes a parameter named `sector_map` while `peer_universe.resolve_growth_medians:304` computes "Industry medians" by partitioning on a **sector** label. Each changes classification behaviour and needs the taxonomy decision the design doc proposes.

**Also reported:** **[CLOSED 2026-09-22 - stage 1 applied; see the newest entry above.]** `docs/gate_registry.md` titles itself "every gate" and §8 (`:229`) requires a row for every gate, yet **48 of the 88 `enable_*` keys in `DEFAULT_CONFIG` have no row** - including all eight score-engine gates and `enable_quant_scorecard`. The doc and `tests/test_gate_env_toggles.REGISTRY` agree with each other (40 rows), so nothing is inconsistent; coverage is simply 45% of the `enable_*` surface. The engine gates are covered instead by `tests/test_quant_scorecard.py:442-444` and `tests/test_engine_ownership_map.py`. Not changed: authoring 48 evidence rows is judgement-heavy, and an unverified row is worse than a missing one in a document whose purpose is to stop gates that cannot fire.

**Web impact**: none - a design doc plus three docstrings. No tool, CLI flag, gate or JSON shape changed.

**Tests.** Comment-only code change; the full engine suite is unchanged and green. `tests/test_yfinance_sector.py`, `tests/test_finnhub_basis.py`, `tests/test_sector_rank.py`, `tests/test_sector_breadth.py`, `tests/test_sector_screen.py`, `tests/test_analysis_tools.py`, `tests/test_moomoo_vendor.py` (357 passed). `ruff check .` clean.

### Changed

**Four declared fields are now measured - and the wiring exposed that five of the sixteen gaps are DECLARATION defects, not wiring gaps (2026-09-22).** The coverage scorecard named sixteen fields whose declared producer is never reached. Wiring four of them showed that in five cases the declaration itself is wrong: the named producer **cannot emit the declared quantity at all**. That is the same class as `ad_ratio` and the breadth legs - now five instances in one engine family - and it is why this entry wires the four that CAN be wired and makes the scorecard state why each remaining gap is unfillable, so an empty field is never filled with a number the repo cannot support.

**Tier 1 - `RiskScore`, the only tier inside `TradeScore`.** `risk_score` is in `COMPOSITE_ENGINES` at 0.20, so these two change the decision number rather than an advisory read.
- **`gap_atr`** - `pre_market.premarket_gap` over the run's own last two bars (`closes[-2]` -> `opens[-1]`) with `size.atr`; `_risk_components` never touched either gap leg. **`through_stop` is deliberately NOT read from that call:** `premarket_gap` answers `False` when handed no prior stop/entry, which would report "the gap did not trade through the stop" when the truth is that no prior plan was ever in hand - those levels live behind `pre_market.load_prior_state` / `parse_planned_levels`, which nothing under `tradingagents/` holds. NA, never a fabricated `False`.
- **`implied_move_pct` - the units trap.** `options_surface.implied_move_pct` returns **PERCENT** (its docstring says "(%)" and it multiplies by 100.0), while `risk_score` declares the component a **FRACTION** with ramp `(0.01, 0.10)`. Wired naively, a 1-sigma 7% move arrives as 7.17 and **saturates the ramp** - a confident wrong number with no error anywhere. The seam divides by 100.0 and a test pins the result inside the declared ramp. No new fetch and no gate flip: `_options_chain_rows_lambda` is the builder the gamma and derivatives-flow leaves already read, and `enable_options_surface` gates only the CBOE surface tool.

**Tier 2 - the attention legs (advisory; `sentiment_score` and `news_score` are OUTSIDE the composite).**
- **`sentiment_score.mention_heat`** and **`news_score.persistence`** both read `sentiment.mention_volume`, which takes a per-day mention COUNT history. That series **already existed and was already being computed**: `daily_sentiment_sma` returns `n` per calendar day (0 on a day with no articles), populated by all three sentiment feeds. `_sentiment_components` held the rows and never read `n`; `_news_components` carried a comment claiming the series did not exist. **It does.** The news leg sits OUTSIDE its `if articles:` block on purpose - mentions and headlines are different feeds, so an empty headline window must not withhold an attention leg the sentiment feed can measure.

**DEFECT (rule 15, behaviour-preserving).** `sentiment.aggregate_weighted_sentiment` re-derived the freshness weight inline as `2.0 ** (-age_days / half_life)` instead of calling `decayed_weight`. The two agreed **only** because the caller clamps `age_days` to `>= 0`, so the copy inherited the producer's `age_days < 0 -> 0.0` guard by accident; drop that clamp and the copy weights a future-dated article ABOVE 1.0 while the producer answers 0.0. One producer now. **This one has no behavioural test and cannot have one** - the mutation that reverts it leaves the suite GREEN, which is the evidence that it is a dedup, not a fix.

**The declaration defects (reported, NOT built).** Each is now recorded in the scorecard with its evidence:
- `risk_score.sector_max_share` <- `portfolio_optimizer.enforce_sector_exposure:169` returns the **adjusted weight DICT** (`:196`), not the scalar this component ramps `(0.15, 0.45)`.
- `sentiment_score.short_pct_float` <- `yfinance_short_interest.get_short_interest_yfinance:37` returns a **markdown STRING** (`:95`). The producer is **not dead code**: it is dispatched through `interface.VENDOR_METHODS['get_short_interest']` (`:591`) and runs in every market-analyst gather - an AST call scan cannot see a dict-lookup dispatch, so the scorecard's `unwired` means "no DIRECT call site", not "dead".
- `risk_score.iv_percentile` needs a per-day IV history no producer can supply; the code already says so.
- `news_score.guidance_change` and `industry_shock` are **BUILDABLE** (Benzinga already parses numeric guidance min/max/prior before discarding them into prose; the 11 SPDR series are already fetched by `get_sector_rank`). `fundamental_impact` and the cheap `regulatory_legal` are **refusals**: the repo holds no revenue/margin estimate, and the litigious-word proxy was already rejected by `event_state.FAMILY_AVAILABILITY`.

**`scripts/coverage_scorecard.py` now prints WHY.** Every non-measured field carries a `gap` reason, resolved authored-first, then from the engine's OWN `ABSENT_REASONS` (so the report and the engine cannot drift), then from a class default. `render_text` prints it under each field and `--actionable` prints it inline; the `unwired` default names the indirect-dispatch blind spot.

**Web impact**: none - no tool, CLI flag, gate or JSON shape the app consumes changed, and `trading_web` holds zero references to any field touched here. The `risk_score`, `sentiment_score` and `news_score` blocks in `run_card.json` gain measured values where they previously carried a named gap, which the app renders exactly as it renders any other score block.

**Tests.** `tests/test_risk_score.py` +4, `tests/test_sentiment_score.py` +2, `tests/test_news_score.py` +2, `tests/test_sentiment_weighted_agg.py` +2, `tests/test_coverage_scorecard.py` +5. Engine suite 5268 -> 5283, 6 skipped, 0 failed; `ruff check .` clean. Five mutations (the `gap_atr` wiring, the percent->fraction seam, the `mention_heat` wiring, the `persistence` wiring, and the scorecard reason) are RED with byte-identical sha256 restores; the sixth (reverting the `decayed_weight` dedup) is GREEN by design.

### Changed

**Market-wide breadth is now measured: P0-3's producer had zero callers, and `ad_ratio` had no producer at all (2026-09-22).** `strategies/market_breadth.py` (P0-3) was fully implemented with seven tests, and both consumers declared it in their component tables - `technical_score`'s `pct_above_50d` / `pct_above_200d` / `ad_ratio` and `regime_score`'s `breadth` - while **nothing in the live path ever called it**. So on every run, for every symbol, those four fields were declared and absent: `technical_score`'s `breadth` category rendered `0 of 3 components present, floor is 2` and `regime_score` reported `absent: ["breadth"]`. `regime_score`'s own table even records the caller it did not have ("prerequisite 2 (P0-3); the leaf passes pct_above_50d"), and both assemblers carried a comment explaining why the panel was not fetched.

**The panel (P0-2).** New `dataflows/market_panel.py::market_closes(fetch_closes, *, universe=None, limit=None)` builds the `{name: closes}` map `market_breadth` takes, over the full S&P 500 constituent list (`sp500_universe`, disk-cached weekly), and caches it for the process - the owner chose the full universe over a sample, fetched once per run. The fetcher is injected rather than imported: the run's OHLCV cache lives in `agents/utils/analysis_tools` and a `dataflows` module must not import from `agents`. Every series comes from that same `_ohlcv` cache, so the panel cannot describe a different price basis than the rest of the run, and a name whose fetch failed stays in the map with an empty series so `market_breadth`'s `coverage` reports it rather than dropping it from the denominator.

**One read, five consumers.** `analysis_tools._market_breadth_read()` assembles the panel once per process and is read by both assemblers (`_technical_components`, `_regime_components`), so the two agent tools and the run card share one panel rather than one each. `_clear_ohlcv_cache` now clears the breadth read and the panel with the OHLCV cache, so existing test hygiene covers them.

**DEFECT - `ad_ratio` had no producer.** `technical_score` declares `ad_ratio` with `strategies/market_breadth.py::market_breadth` as its producer and a `(-0.30, 0.30)` ramp, but `market_breadth` returned only `advance_decline` (a difference), never a ratio - so even with the panel wired, that leg could not be measured. The ratio is now emitted by the one module that already computes both the numerator and the denominator (rule 15: no derived quantity gets a second producer), withheld on a small sample exactly like the percentages. Nothing else produced `ad_ratio`, so this is the single producer rather than a second one.

**Absent stays absent.** Both legs read `None` through unchanged: a panel below `market_breadth`'s `min_n`, an unavailable universe, or a failing read leaves the components out of the denominator rather than scoring a zero - the same rule the gate applies at the producer.

**Web impact**: none - no tool, CLI flag, gate or JSON shape the app consumes changed. The `regime_score` and `technical_score` blocks in `run_card.json` gain measured values where they previously carried a named gap, which the app renders exactly as it renders any other score block.

**Tests.** `tests/test_market_panel.py` (14) and `tests/test_strategies_market_breadth.py` +2 (`ad_ratio`). Engine suite 5252 -> 5268, 0 failed. Five mutations (the once-per-run read cache, both wiring loops, absent-never-zero, and `ad_ratio` not emitted) are RED with byte-identical sha256 restores.


**A field-level coverage scorecard, and the twelve producers it found that nothing calls (2026-09-22).** Coverage measured near-identical across four unrelated symbols - `technical_score` 0.95 four times, `regime_score` 0.83 four times, `news_score` 0.25 four times - which says the gaps are a property of the pipeline, not of the stock. Nothing read the per-field picture across runs, so a field that is never populated looked exactly like a field that is populated on most runs.

**`scripts/coverage_scorecard.py`.** Reads every `run_card.json` under a reports dir and answers one question per declared field: was it measured, and if not, why not? The declared universe comes from `score_panel.engine_registry()` - the one place each engine's own component table is read - so this adds no second vocabulary for the same fields. The denominator is the trees where the engine was actually **measured**, so an engine whose gate is off cannot drag a field's fill rate down.

**The classification is the point.** A zero fill rate is one of three things and only one of them is a defect here: `unbuilt` (the declared producer is empty), `unwired` (a producer is named and nothing outside its own module calls it), or `structural` (the producer is reached and this run supplied no value). Fill rate alone cannot separate them; the declared producer string plus a call-site scan can. The scan is AST-based and resolves **import aliases**, because the live path renames producers on import (`from ...liquidity_risk import days_to_absorb as _dta`) - reading only the call node's own name reports that producer as never called, which is a manufactured defect. A producer string that names no callable, resolves to no file, or names a function its file does not define returns "cannot tell" rather than a false `unwired`.

**What it found on the 68 trees.** Twelve fields whose declared producer is never reached from the live path. The largest single family is `strategies/market_breadth.py::market_breadth` - **four fields across two engines** (`technical_score` `pct_above_50d` / `pct_above_200d` / `ad_ratio` and `regime_score` `breadth`): a fully implemented, fully tested producer with **zero production callers**. `regime_score`'s own component table even records the caller it does not have ("prerequisite 2 (P0-3); the leaf passes pct_above_50d"). The rest: `sentiment.mention_volume` (2 fields), `pre_market.premarket_gap` (2), `options_surface.implied_move_pct` and `iv_percentile`, `portfolio_optimizer.enforce_sector_exposure`, `yfinance_short_interest.get_short_interest_yfinance`. Four further `news_score` components declare no producer at all.

**Presence is read from explicit lists, never from a value.** A measured `0.0` is a measurement, not a gap - `adx: 0.0` is present. Each engine publishes presence differently (`technical_score` category `components`, `risk_score` category `present`, `fundamental_score` subscore `coverage.metrics`, `regime_score` engine `components`, and for `sentiment_score`/`news_score` the complement of `absent`, which those engines define as the complement of what they measured), and this reads each shape rather than unifying them. A field the card says nothing about leaves the denominator instead of being guessed at.

**Not a defect, checked.** `risk_score`'s `liquidity` category reports 2/3 rather than 4/5 because `risk_score.category_score:486` scores over `SCORED` components only, leaving the `PRINTED` ones out of its denominator. The registry's declared set is a superset of the live engine's for `fundamental_score` too (28 declared against 25 counted), which is why an engine whose declaration carries no producer column is reported as "cannot tell" rather than `unbuilt`.

**Web impact**: none - a new `scripts/` entry point, no tool, CLI flag or JSON shape the app consumes changed.

**Tests.** `tests/test_coverage_scorecard.py` (25) - the zero-is-present rule, the complement rule including the empty-`absent` case, denominator integrity under a gated-off engine, the three-way classification, both declared producer shapes, and the call-site scan against a string citation, a plain call, an **aliased** call, an attribute call, and a function its file does not define. Four mutations (zero-is-present, alias resolution, the empty-`absent` complement, package-relative producer resolution) are RED with byte-identical sha256 restores.

### Changed

**The cross-sectional momentum book is now an advisory read - and building it exposed a defect that handed back an all-long book as "dollar+beta neutral" (2026-09-22).** The pasted strategy brief's top pick was "cross-sectional momentum with factor neutralization". Most of it was already in the repo as pure functions with **no production caller**: `cross_section.neutralize_book` (dollar+beta+sector projection), `quantile_split`, `centered_rank`, `factors.vol_adjusted_momentum` and `book_risk.net_beta`. What was missing was the composition, the panel and the wiring.

**`cross_section.momentum_book` - a composition, not a second producer.** Ranks a caller-supplied panel with the ONE risk-adjusted momentum producer, splits the top/bottom quintiles, equal-weights the two legs, neutralizes them **within the selected legs**, and reports the book's net beta BEFORE and AFTER the projection so the gate's effect is measured rather than asserted. Restricting the projection to the legs matters: projecting over the whole panel spreads the orthogonal adjustment onto names that were never selected, so the book quietly holds the middle too. Advisory by construction - not an engine, not a gate, not part of `TradeScore` (rule 17) - and explicitly NOT market breadth: the panel size travels with the read. `None` below 8 scored names, because a long/short book over four names is not a cross-section.

**`get_cross_section_momentum` (market analyst).** Resolves a bounded panel (caller-supplied names, or the ticker plus its vendor peers - the `get_composite_rank` path), fetches closes from the run's existing `_ohlcv` cache, and computes per-name betas **date-aligned** to the benchmark. `cross_section.residualize_returns` truncates two series to their common LENGTH, which pairs different sessions whenever a name has a gap; here an unalignable name gets no beta, and `net_beta` then reports the book's beta as unknown rather than summing a partial.

**DEFECT 1 - `neutralize_book` amplified floating-point residue into a book.** Found by the new read's own probe. The orthogonal projection is correct: for an equal-weighted leg set whose names share one beta per leg it leaves `~1e-16`, because that book lies entirely in the constraint space. But the guard tested `gross <= 0`, which that residue never satisfies, so `gross_target / 1e-15` **renormalized the noise into a fully-invested book with a plausible gross of 1.0** - observed as `{0.25, 0.25, 0.25, 0.25}`, an all-long book the function's own contract called dollar+beta neutral. The fallback then returned the RAW book, which violates the very constraints the caller asked for. Both are gone: the guard is relative (`1e-12 * raw_gross`) and the annihilation case returns `{}` - "no book survives" is not "a book that is not neutral".

**DEFECT 2 - the vol-target producer could not be called with the input it exists for.** `size.volatility_target_scale`'s `vol_override` is documented as supplying an externally computed annualized vol (GARCH's long-run vol), but the `len(returns) < 5` guard ran FIRST, so the override path returned **0.0** whenever the caller had no series - and 0.0 zeroes a position size instead of scaling it. That is also why `size.composite_position_size` and `analysis_tools.get_risk_overlay` re-derived `max(0, min(target_vol/vol, 3))` inline instead of calling the one producer (rule 15). The guard order is fixed, both sites now delegate, and the delegation is **bit-identical** for existing callers.

**Not a defect, checked.** `momentum.ts_momentum_weights` also scales by a vol target, but by `target_vol / mean(sigma_i)` over per-name ex-ante vols - a book-level quantity, not the portfolio realized-vol scale `volatility_target_scale` computes from a return series. It is not a second producer of the same quantity and was left alone. The review that produced this work claimed three re-derivations; two were real.

**Tests.** `tests/test_cookbook_gaps.py` 27 -> 33 (the residue regression, the ranking/neutralization contract, the leg restriction, the producer pin, the floor and the beta gap), `tests/test_strategies_size.py` +1 (the override path). Both defects are RED under a faithful mutation that restores the pre-fix code and nothing else, with byte-identical sha256 restores.

**Web impact**: none. No new tool for the app, no CLI flag, no env var and no JSON key. `get_cross_section_momentum` is an analyst tool whose text output is new; `neutralize_book` and `volatility_target_scale` keep their signatures.

**The debate full-stream test sat one second inside its own timeout, so a loaded machine killed the entire suite (2026-09-22).** `tests/test_debate_stream_hermetic.py` capped itself at `pytest.mark.timeout(120)` while `test_full_stream_reaches_research_manager_when_structured_on` performs the **real ~87-tool vendor gather** (the LLM factory is stubbed; the vendor calls are not). Measured call time: **119.21s** - 99.3% of the cap. `pytest-timeout`'s thread method does not fail one test when the cap is crossed: it dumps stacks and **terminates the process**, so a full-suite run died at 216s with no summary and a bare exit 1, and a re-run passed. The module now carries 300s, 2.5x the measured cost; the global 180s default is untouched, and the module-level override is the mechanism `pyproject.toml` documents for exactly this.

**Web impact**: none - test configuration only.

**The market-breadth read told the model it measured things the vendor never sends - and its one live table rendered reversed (2026-09-22).** `get_market_breadth` (moomoo) returns the vendor's rise/fall distribution, and nothing else. The market analyst's prompt promised "percent above the 20/50/200-day, advance-decline, new highs/lows ... with its panel size printed"; the news prompt told the model to call `get_market_breadth(ticker)` when the tool takes **no arguments**. Measured across 91 report trees: 42 got a real read, **0** contained a `% above the 20/50/200-day` claim, and the analysts correctly reported `NO_DATA_AVAILABLE` rather than substituting - so the false promise was **latent**, not active. The prompts were corrected anyway: a surface that promises a measurement nothing produces is the recurring defect class in this repo, and a wrong argument list is a call that cannot succeed.

**The render was the live defect.** The vendor NAMES each band's shape in `type` (`POSITIVE_INFINITY` = `(7%, +∞)`, `NEGATIVE_INFINITY` = `(-∞, -7%)`) and puts a sentinel on the border it does not mean. `get_market_breadth_moomoo` read that sentinel off the **border** instead, so both open-end branches were dead code and the table rendered `| 7% … 0% | 494 |` and `| 0% … -7% | 731 |`. Those labels reached analyst reports verbatim, and two reports summed them into an "advancers" figure no band held. `_rise_fall_rows` now reads the shape from `type`, sorts the bands ascending, and renders an absent border (the vendor sends the string `'N/A'`, never `None`) as `?` rather than an invented infinity.

**The panel is a named, deliberate gap, not a promise.** Market-wide breadth needs a `{name: closes}` panel; `strategies/market_breadth.py::market_breadth` takes one pre-fetched, and `constituent_universe` is budget-bounded against fetching it ("FMP 250 req/day, yfinance throttled"). `_technical_components` already declined it (`analysis_tools.py:5036-5041`) and the engine prints the gap instead of a sector proxy. The prompt now points at the participation read that exists - `get_sector_rotation_screen(enable_breadth=True)`, per-sector `% above 50d SMA` with its own sample size - and `docs/design_sector_rotation_screener.md` §5.5 records both options and why wiring the panel would **not** close the score leaf's gap.

**Tests.** `tests/test_moomoo_vendor.py` 54 -> 57, plus the existing breadth test strengthened: it asserted only the section headers, which is why the reversed table survived. All three fail under a faithful mutation that restores the historical defect (the sentinel read off the border), with a byte-identical sha256 restore.

**Web impact**: none. No new tool, CLI flag, env var or JSON key. The tool's text output changes shape - the bands are now ascending and the open ends are named - which is the point of the fix.

**The three genuinely-absent regime reads are now built, each on an existing producer (2026-09-21).** The screener review left exactly three gaps worth filling: an event-anchored AVWAP, a true 52-week new-high/new-low count, and a walk-forward HMM with filtered probabilities. Each one consumes the machinery that already exists rather than adding a second producer of the same quantity.

**1. Event-anchored AVWAP - `extended_indicators.anchored_vwap_levels`.** `anchored_vwap` answered one question (the cumulative VWAP from a price level); an analyst asks which anchor and whether price has lost it. Anchors are now located as bar INDICES and the level is computed by the existing `anchored_vwap` over the slice from that bar, so there is one implementation of the arithmetic. Reported anchors: `swing_low`/`swing_high` (the most recent CONFIRMED fractal pivot - `swing_pivots` requires the right-hand confirmation bars, because a low that can still be undercut is not a swing, and a tied minimum is not a pivot), `opex_monthly`/`opex_quarterly` from `derivatives_gamma.opex_dates` (the repo's one third-Friday calendar, not a second one), and any `event_dates` the CALLER supplies - FOMC and earnings dates come from a calendar, so they are input and never inferred from the bars. `shift` is the read the anchors exist for: price below the swing-low VWAP **and** volume at or above `volume_multiple` (default 1.5x) its average since that anchor. Surfaced through the existing `get_extended_indicators` tool. The market prompt's bullet for it had to be TRIMMED, not grown: `test_prompt_budget_contract` caps the longest bullet at 523 chars and the original was 427, so the addition is 69 chars (496 total) - the tool guidance grew only deliberately, and the ceiling was not raised.

**Six score-engine citations moved and were updated in the same pass**, because inserting lines shifts the `def` lines those citations promise: `regime.py::choppiness` 132 -> 133 and `::vol_percentile` 50 -> 51 (the module docstring grew a line), `extended_indicators.golden_death_cross` 53 -> 55, `.ichimoku` 78 -> 80, `.roc` 149 -> 151, `.chaikin_money_flow` 261 -> 263 (the `datetime` import), and `market_breadth.py::market_breadth` 37 -> 44. `test_regime_score.py::test_every_component_names_one_producer_with_a_line` checks the `regime.py` and `market_breadth` ones point at the `def`.

**2. True 52-week new highs/lows - new fields on the ONE breadth producer.** `market_breadth`'s `new_highs`/`new_lows` are counted against the panel each name was given, so a 60-bar panel cannot produce a 52-week figure. The new `new_highs_52w`/`new_lows_52w`/`net_new_highs_52w` count over a FIXED 252-session year, so a 60-bar and a 300-bar panel agree for the same last year of bars. A name with less than a year is excluded and reported in `new_highs_52w_n`, never counted and never assumed not-at-a-high; with no name carrying a year the pair is `None`, not `0`. New fields only - the existing counters and the printed `basis` keep their meaning, and the basis now states the 52-week coverage. `regime_score.py`'s citation of the moved definition was updated in the same pass (the `test_every_component_names_one_producer_with_a_line` gate checks it points at the `def`).

**3. Walk-forward HMM with FILTERED probabilities - `regime.hmm_filtered_regime`, and `hmm_regime` now delegates to it.** `hmm_regime` was a 2-state Viterbi label gated on `hmmlearn`, which is not installed and not declared - so it returned `unknown` on **every** run (247 occurrences, zero real labels, across 91 report trees). The replacement is pure NumPy (the same choice `garch11_fit` makes), so there is no optional dependency and it is testable offline. It runs the **forward recursion only**, so row `t` is `P(S_t | x_1:t)` - the value available at that bar's close. `hmmlearn.predict_proba` runs the backward pass too, so its rows use the future; that difference is the whole point, and it is pinned by a test. Parameters are refit on `x[:t]` every 20 bars and the filter is re-run over that same past window after each refit, so the carried state is the new model's posterior in its own canonical order; states are canonicalized by mean return at every refit. Features are `[log return, yang_zhang_vol_series]` - the drift-independent, gap-aware estimator built in the entry below, so the volatility leg has one producer. Output is BAR-ALIGNED like `yang_zhang_vol_series`.

**Verified live, not just asserted.** The no-lookahead property is measured: appending 50 future bars to a 300-bar series left **0 of 39** overlapping filtered rows changed (a smoothed posterior changes instead). The filter recovers a seeded two-regime series (`bear` at the end, the high-vol state as the terminal one, canonical order by mean return), is deterministic across runs, and returns `None` rather than a label when the history is too short to train on. `hmm_regime` keeps its contract: `bull`/`bear`/`unknown`, the last when there is not enough history.

**A defect my own test caught, fixed before landing.** `date_anchor_index` length-checked the date string - and `"not-a-date"` is exactly ten characters, so it string-compared against every ISO date and returned a bar instead of `None`. It now PARSES the date. The semantics were also wrong: an event after the last session anchored the last bar, inventing a bar for an event that has not happened; a future event is now a named gap, and the anchor is the FIRST session at or after the event (an intraday reaction anchors that session, a Saturday statement anchors the next one).

**Tests.** `tests/test_strategies_regime.py` 15 -> 20, `tests/test_extended_indicators.py` +9, `tests/test_strategies_market_breadth.py` +2. One existing assertion was repointed rather than re-pinned: `test_the_new_high_count_is_measured_against_the_window_it_was_given` asserted `"52" not in basis`, a proxy for "no 52-week claim"; now that a real 52-week counter exists the claim is pinned on the counter (`new_highs_52w is None`), not on wording.

**Web impact**: none. No new tool, CLI flag, env var or JSON key. `get_extended_indicators` gains lines in its existing text output, and `get_regime_state`'s `hmm=` field now carries a real label instead of the permanent `unknown` - a behaviour change to an analyst tool's text, which is the point of the fix.
...
**A ledger correction, made by the same mistake the fix addresses.** An earlier note recorded that "AAPL has no Form 4 rows in the current window and was never a valid known-good". That was concluded from a **3,000-row bounded scan** - it repeated the very defect under repair. AAPL does have Form 4 rows; they sit at index 5,354.

**Tests.** None added: this entry records a live verification and corrects comments, with no behaviour change, and a test that pinned the vendor's paging regime would be testing the vendor rather than this repo.

**Web impact**: none. Both readers' render shape, tools, CLI flags, env vars and JSON keys are unchanged.

**The analyst report-stub guard could not fire for a bare verdict line - NTR 2026-09-21 shipped two stub analyst reports (2026-09-21).** The NTR batch tree's `1_analysts/market.md` and `1_analysts/sentiment.md` looked plausible on disk (1,843 and 1,564 bytes) but their **model text was 36 and 407 chars**. `_looks_report_stub` tested `len(t) < 400 and _ANALYST_STATUS_TURN_RE.search(t)`, where the regex is `\b(progress|let me (now )?(continue|gather|fetch|pull|collect|check|dig))\b`. A bare verdict line matches none of those alternatives, so the length clause was **unreachable** for exactly this shape: `_looks_report_stub("FINAL TRANSACTION PROPOSAL: **HOLD**")` returned `False` on a 36-char "report". Reproduced before the fix.

**The engine-score block masked both stubs.** `report_hygiene.engine_report_section` appends the TechnicalScore/SentimentScore section *after* the guard runs, adding ~1,157-1,807 chars. So the saved artefact reads as substantial while the model contributed 36 chars. Any length check on the saved file passes; the guard must test the model text.

**Fix: the test is length alone.** `_looks_report_stub` now returns `len(t) < _REPORT_STUB_MIN_CHARS` (400). The regex is deleted rather than kept as a second condition - it was a *proxy* for "not a real report" that could not fire for the pathology it was meant to catch. Measured across **64 report trees** before choosing the floor: the shortest genuine report is ~8.4 KB (market), ~6.2 KB (fundamentals), ~2.7 KB (sentiment), against stubs at 36 and 149 chars - a 20x margin, so the floor discards nothing real. With the floor applied to the 64-tree corpus, exactly **two** reports newly classify as stubs (NTR market 36, MSFT 2026-09-16 fundamentals 149), both unambiguous.

**Tests.** `tests/test_analyst_report_stub.py` gains two: the bare-verdict shape (the exact NTR text, plus that the retry path replaces it with `**Report unavailable**` rather than shipping it) and a short-but-real report just over the floor. The bare-verdict test is **RED** under a faithful mutation that restores the regex gate, with a byte-identical sha256 restore.

**Web impact**: none. An internal analyst-chain guard; no tool, CLI flag, env var or JSON shape changes. A report that is now correctly classified reaches the same `**Report unavailable**` notice the other stub paths already emit.

**Known gap, NOT fixed here: the sentiment analyst has no stub guard at all.** `retry_chain_if_stub` is wired into the market, news and fundamentals nodes only; `sentiment_analyst.py` renders from the structured `SentimentReport` via `invoke_structured_or_freetext` and stores the result unconditionally. Its required `narrative` field has no substance floor, so a one-sentence narrative renders a report with no analysis (NTR 2026-09-21: 407 chars of model text). Closing it means either a `narrative` minimum (a schema/contract change) or wiring the guard into the structured node - and the floor is a genuine judgement call, because short sentiment reports are a *cluster*, not a one-off: across 64 trees, HPE 390, QCOM 402, NTR 407, NVDA 531/611 and AMZN 1,188 sit against a p10 of 2,694. Raising the floor to catch them reclassifies ~9% of sentiment runs, so it is left to the owner.

**The Benzinga firehose miss was reported as a definitive absence after scanning a bounded window (2026-09-21).** `get_insider_transactions_benzinga` and `get_congress_trades_benzinga` scan the market-wide Form 4 and congressional firehoses and filter locally, because both endpoints ignore every ticker parameter. The scan was budgeted in **pages** - four pages of `pagesize=100` - and on a miss raised `NoMarketDataError(detail="no Form 4 transactions for this symbol in the most recent filings")`. That claimed the symbol *has* no filings when the reader had only looked at a prefix of the newest filing day. Both phases of the recorded plan are now landed: the budget is **rows**, and the miss names the window it actually covered.

**Phase 0 - probing the endpoints first, as the plan required, changed the fix.** The two routes DO honour `date_from`/`date_to` (2020 and 2024 windows both returned 0 rows), but they take the **plain** spelling; the bracketed `parameters[date_from]` form the calendars use is ignored there. Three further measurements decided the design: (1) `pagesize` is **not a cap** on these routes - `pagesize=100` returned 233 / 100 / 275 / 289 rows on consecutive pages and `pagesize=1000` returned 2,425 in one response; (2) the page cursor is **stable** (identical row sets across back-to-back walks) but pages 1-4 of a trailing window all cover the **same newest filing day** as different slices, so the walk does not step back in filing time **[SUPERSEDED - see the Phase 3 entry above: the UNFILTERED walk these readers make does step back monotonically, 12 pages spanning 2026-08-18..2026-09-17. The observation holds only of a windowed walk, and only at the top of the window]**; (3) a date window therefore *bounds* the query but does not *extend* the scan **[SUPERSEDED - a date window would extend it. The row budget still stands, on the request count rather than on this]**. So the fix is not a date window - it is an honest row budget, which is the plan's step (1).

**Phase 1 - the budget is rows inspected, not pages.** `_filtered_pages` now takes `page_size` and `max_rows`, inspects every row a page returns, and ends the walk on an empty page **or** the row budget - never on a short page. The old `if len(batch) < page_size: break` was unsound given the ragged slices: a 3-row page ended the scan with rows still arriving. It returns a `_Scan` record (`rows`, `inspected`, `pages`, `truncated`, `lo`, `hi`), so truncation is observable rather than inferred. Defaults `_FIREHOSE_PAGE_SIZE = 1000` / `_FIREHOSE_MAX_ROWS = 3000` resolve at call time so they are patchable, and a page guard (`max_rows // page_size + 2`) bounds the request count.

**Phase 2 - the miss names the scanned window.** `_Scan.describe()` renders e.g. `first 3,425 rows of the newest filings (2026-09-14..2026-09-17), a bounded sample - an older filing may sit beyond it`, and both readers embed it: `no Form 4 transactions for this symbol in the {scan.describe()}`. A scan that reached the end of the stream omits the "bounded sample" hedge, because then it is complete *within* its window. The reported date axis is the **served** one - the filing date for Form 4 (never `date_transaction`, which spans a year inside a single filing day) and the disclosure `report_date` for congress - so the window is not overstated.

**Measured live (2026-09-21).** A symbol genuinely absent (`AAPL`, `SPY`) yields the honest bounded-sample message naming 3,425 rows and the window. A symbol present (`DELL`, the most Form 4 rows in the live window at 194) is reached in **1 request inspecting 2,425 rows**, where the old budget used 4 requests for 400 nominal rows; congress (`AAPL`, 24 rows present) renders `4 buys / 4 sells (net +0)`.

**Tests.** `tests/test_benzinga_surface.py`, 27 -> 30. Two are the failing-first proofs, each **RED** under a faithful mutation with a byte-identical sha256 restore: reinstating the short-page break (reddens `test_a_ragged_short_page_is_not_the_end_of_the_stream`) and reverting the miss detail to the flat absence claim (reddens `test_a_budget_limited_miss_names_the_row_window_not_absence`). `test_the_two_firehose_readers_page_with_the_lowercase_spelling` was repointed to the new keyword signature.

**Web impact**: none. Both readers' render shape is unchanged and no tool, CLI flag, env var or JSON key moves; the change is the internal scan budget and a `NoMarketDataError` detail string.

**Every finished report now carries its own TypeSafe verdict - `enable_jev_verdict` writes `jev_verdict.json` into the report tree (2026-09-21).** Owner instruction: run jev each time a report has finished generating, and put the verdict JSON in the corresponding report folder.

**The jev core MOVED out of `scripts/` into the package.** The verdict must be callable from the pipeline (`batch.py`, which trading_web also calls), and production code importing from `scripts/` inverts the dependency while duplicating the logic would create a second authoritative producer of the same numbers. So `scripts/jev_decide.py` is now only the argument surface, and the model client, the neutraliser, the batteries and the tree readers live in `tradingagents/jev.py`. The 353-line core was **sliced, never retyped** - verified byte-identical after the move - and the 37 existing tests were repointed by changing one import line plus the three `main`/`_parse_args` call sites.

**New entry point `judge_tree(tree, key=...)`** = the `--verdict` recipe (the four analyst reports, positions neutralised, buy/hold/sell) plus `write_verdict` into the tree. One call, so a caller cannot judge without storing - the point of the step is that the verdict travels with the report it is about. `verdict_for_tree` returns a payload carrying `ratings` (rating / confidence / probabilities / evidence / horizon per stem), `neutralized` counts, the raw `results`, and `failures`/`cost`.

**The hook is best-effort, like `enable_pre_market_review`** - a judge that annotates a finished run must never fail it. A missing `OPENROUTER_API_KEY` skips with a log line rather than erroring (the engine does not require that key), and a vendor failure or a transport exception is recorded in the payload with the vendor's own message. Both are asserted.

**New gate `enable_jev_verdict`** (`TRADINGAGENTS_ENABLE_JV_VERDICT`), registered in the five places. It **ships off** - matching `enable_pre_market_review`, and so the web app's runs do not silently start paying - and is set `true` in the owner's `.env`, which is what makes it fire on every run here.

**A new registry section was needed: §7c "Post-run output gates".** This gate is not §6/§7b (it adds no source the model reads), not §7 (it changes nothing the model reads) and not §1/§2 (it cannot stop, shrink or reroute anything). By the time it runs the decision is already saved, so it can only add a file to the tree. The registry now says so, rather than filing it under a heading that would be a lie.

`batch.py`'s two post-save hooks were extracted into `_post_save_annotations`, so the gate is testable without running the graph. New `tests/test_jev_verdict_hook.py` (12): the gate OFF means the judge is **never called** and ON means it is (the §8 "fails when the gate is ignored" requirement), the two post-save gates are independent, the JSON **lands in the report tree** at the asserted path, the hook sends the analyst reports neutralised with the rating battery, a research report never reaches the judge, and the three quiet-failure paths. **Proven failing-first by mutation:** making the hook call unconditionally turns `test_gate_off_never_calls_the_judge` red ("the gate was ignored - the judge ran while switched off"), sha256 restore MATCH.

One test had to be written differently. `assert DEFAULT_CONFIG["enable_jev_verdict"] is False` passed before the `.env` flip and failed after it - `DEFAULT_CONFIG` is the default **after** `_apply_env_overrides`, so it asserts the developer's `.env`, not the shipped default. The test now reads the `DEFAULT_CONFIG` **literal** structurally via AST, which is env-independent and is what "ships off" actually means.

**Measured live:** `_batch_jev_verdict` run against the real vendor on `NVDA_20260921_114014` and `MSFT_20260921_114625` wrote a 5.8 KB `jev_verdict.json` into each (NVDA `fundamentals=hold, market=hold, news=buy, sentiment=hold`; MSFT all four `hold`), 0 failures.

**Web impact**: a behaviour change the app must know about. `analyze()` now calls the verdict hook, and trading_web's `run_batch` calls `analyze` directly - so a web-triggered run writes `jev_verdict.json` into its tree when `enable_jev_verdict` is on. It is off by default and best-effort, so an app that does not set the flag is unaffected; the file is additive and no existing field moves.

**The jev judgement recipe is now `--verdict`: analyst reports only, de-biased, judged buy/hold/sell (2026-09-21).** Owner instruction: when asked to send a set of reports for a stock symbol, send **only the analyst reports**, **strip position language** ("buy", "hold", "sell", ...) first so the judge is not handed the conclusion, and ask it for a **three-way buy/hold/sell rating**.

The three parts are one flag because they are one intent. `--verdict` = the four analyst reports only (never the research/risk/portfolio documents, which already carry a conclusion), their position language neutralised, and the buy/hold/sell battery. `--verdict` and `--all` are mutually exclusive - `--all` contradicts "analyst reports only" - and the pieces are available alone as `--neutralize` and `--rating`, so the composition is testable rather than implied.

**Neutralising is the substantive part.** The analyst reports *state* a rating, and a rating is what is being asked for: without stripping it, the judge measures agreement with the document rather than with the evidence. Words are replaced by `[POSITION]`, never deleted, so the substitution stays visible and is counted per report. The word list is deliberately **narrow**: `buy`/`sell`/`hold`, `strong buy`/`strong sell`, the weight and perform scales, `accumulate`/`distribute`. It **excludes** `long`, `short`, `add`, `reduce`, `trim` and `neutral` - each is ordinary English in a market report (`long-term`, `short interest`, `adds to risk`, `reduces margin`), so stripping them removes *evidence* rather than bias. Matching is case-insensitive on word boundaries, so `buy` does not touch `buyback` and `hold` does not touch `shareholders` or `holdings`; multi-word positions are consumed whole so `strong buy` does not leave `strong` beside a marker.

**Measured live on `NVDA_20260921_114014`:** 20 position terms replaced across the four analyst reports (fundamentals 9, market 4, news 5, sentiment 2); ratings came back `fundamentals hold (0.85)`, `market hold (0.67)`, `news buy (0.25)`, `sentiment hold (0.90)` for $0.000909. **The de-biasing changed an answer:** under the old stance battery without neutralisation the sentiment report read `bullish` at confidence 1.00; with its stated position stripped it reads `hold` at 0.90 on evidence 2.01 - the bullish call was the document's own, not the evidence's. Worth recording that the analyst evidence, judged blind, gives a **hold** consensus while the run's own decision was `Underweight`: the bearish conclusion is produced by the research/risk/portfolio stages, not by the analyst evidence.

`tests/test_jev_decide.py` **27 -> 37**: the case-insensitive replacement and its count, multi-word consumption, the word-boundary guards (`buyback`, `shareholders`, `holdings`, `seller` untouched), the deliberate non-stripping of ambiguous market English, the no-op case, the battery satisfying the endpoint's discriminator, `--verdict` end-to-end through `main()` asserting all three requirements at once (4 states for a 6-report tree, `[POSITION]` in the sent state, `rating` asked with buy/hold/sell), `--rating` alone *not* neutralising, `--neutralize` alone keeping the default battery, and `--verdict`/`--all` rejecting each other. Docs: `docs/developer/06-entrypoints.md` + `docs/api_reference.md`.

**Web impact**: none. A `scripts/` utility's judgement options; no tool, engine flag, env var or JSON shape changed.
...
**One documented knob did not exist.** `TRADINGAGENTS_ENABLE_ALPHA_PROFILE` -> `enable_alpha_profile` appeared in the table with a note; the env var is read by **no file**, the config key is in neither `DEFAULT_CONFIG` nor `_ENV_OVERRIDES`, and the name that does exist - `alpha_profile` - is a **report output key** in `scripts/strategy_quality_report.py:211`, not a gate. The row conflated a report field with an env-overridable config gate and was removed.

**Two mechanisms reach the config, and the doc now covers the union.** `_ENV_OVERRIDES` (258 rows) is the bulk, but `DEFAULT_CONFIG` also reads a literal `os.getenv("TRADINGAGENTS_...", fallback)` for six keys - `results_dir`, `data_cache_dir`, `llm_failure_journal_dir`, `report_verify_model`, `report_verify_max_calls`, `memory_log_path`. Those were previously documented ad hoc; the generated table carries all 264 pairs, ordered by the code's own declaration order so it groups by topic.

**What keeps it honest.** New `scripts/gen_api_reference_table.py` regenerates §1.1 from `_ENV_OVERRIDES` + the `os.getenv` reads, preserving every existing human note verbatim (they carry domain knowledge the code cannot) and adding a code-derived `default X` note for new rows - never an invented semantic one, and never a value for a credential-shaped key. New `tests/test_api_reference_env_table.py` (4 tests) re-derives the surface and asserts the table lists **exactly** it, keys every row to the key the code maps it to, uses one cell count, and lists no env var twice. **Proven failing-first against the pre-fix document: 3 of 4 red - 102 omitted env vars, cell counts `{23: 2, 43: 5}`, and the duplicate set.**

**Web impact**: none. A documentation correction plus a generator script and a test; no tool, CLI flag, env var, or JSON shape changed. `trading_web`'s `test_doc_claims.py` / `test_engine_contract.py` read neither file.

**The Benzinga surface: five new capabilities, six backup registrations, and three transport defects that made two thirds of the API unreachable (2026-09-20).** Owner instruction: put Benzinga in the `news_data` chain, implement the five capabilities identified in the entitlement assessment, wire the vendor as a backup wherever an alternative exists, and re-rank every multi-vendor chain by payload richness then quota.

**The chain.** `news_data` is now `eodhd,benzinga,moomoo,yfinance,alpha_vantage,stockdata,newsapi` — Benzinga is **second**, because it is the rare free feed that is both ticker-filtered and financial-first, so it precedes the generic keyword feeds. The exclusion it replaces was justified in the docs by "no real key registered yet", a reason that lapsed the moment the key was exercised.

**Five capabilities, four of which had no producer at all in this repo:**

| Tool | Endpoint | What it adds |
| --- | --- | --- |
| `get_guidance_revisions` | `/v2.1/calendar/guidance` | management's own forward revenue/EPS range per future period, with the prior range and the narrative note. The only forward **growth** producer in the data layer |
...
| `get_analyst_actions` | `/v2.1/calendar/ratings` | one row per action: firm, analyst, rating change, price-target move |
| `get_news_removed` | `/v2/news-removed` | retracted article ids — the only feed here that can **invalidate** evidence already collected |

All five sit behind one new gate, `enable_benzinga_surface` (**default OFF**): one vendor's surface, one risk profile, one opt-in. While it is off each tool returns a `DATA_DISABLED` sentinel and never fetches, so a gate-off run is byte-identical to the run before the surface existed.

**Six backup registrations on existing routes:** `get_stock_data`, `get_earnings_calendar`, `get_analyst_ratings`, `get_insider_transactions`, `get_congress_trades` and `get_corporate_actions`. The `get_analyst_ratings` backup renders a consensus **derived from the individual actions** in its trailing window and labels it as such rather than presenting it as a published consensus.

**Re-ranking.** Every multi-vendor chain is now ordered richest-payload-first, then by quota. One ordering was **against that criterion** and is corrected: `get_insider_transactions` ran `alpha_vantage,yfinance,moomoo` — a 25 req/day keyed vendor in front of `moomoo` (local OpenD, no quota, richest rows) and `yfinance` (keyless). It is now `moomoo,yfinance,alpha_vantage,benzinga`. Because that method lives in the `news_data` **category**, it needs a `tool_vendors` override; without one it would inherit the news chain and put the Benzinga Form 4 firehose first on every lookup.

### Three transport defects, each of which broke something

1. **`BASE` hardcoded the `/v2` version segment.** `_benzinga_get("v2.1/calendar/ratings")` was requested as `/api/v2/v2.1/calendar/ratings` and answered with the gateway's own `no Route matched with those values` 404. The module could only ever reach the `/v2` routes, so **no calendar was implementable** — news lives at `/api/v2`, the calendars at `/api/v2.1` and the alt-data surface at `/api/v1`. `BASE` is now the API root and the version segment belongs to each caller's path.
2. **Every dict-wrapped 200 was collapsed to `None`.** Benzinga is not consistent: `/v2/news` is a bare list, every calendar is a dict keyed by its own family (`{"ratings": [...]}`), the alt-data surface is `{"data": [...]}`, `/v2.1/fundamentals` is `{"result": [...]}` and `/v1/logos` is `{"logos": [...]}`. Returning `None` for any dict reported each of those routes as **"no data" on a 200 that carried rows**; only the bare-list news route ever worked. The unwrapper now returns the envelope's sole list-valued member, and `{"ok": "true", "errors": [...]}` — the `/v3/fundamentals` failure shape, whose `errors` is itself a list — is raised as a typed error rather than handed back as records.
3. **`congress.get_congress_trades` could not fail, so it could not fall back.** `route_to_vendor` treats *any* returned string as a successful result and returns it, so a vendor answering the prose `"congress trades unavailable for X: ..."` stopped the chain: **every later vendor configured for `get_congress_trades` was unreachable**, and the Benzinga backup added here would have been dead code. It now raises the typed `NoMarketDataError` (reason as detail), and a partial result — one chamber answering — still renders rather than raising.

**Four parameters that do nothing or do something else, so none was trusted.** Each was proven against the live API before it was used: `/v2/news-removed` **ignores every date parameter** (an empty filter, a one-day window and a three-month window all return identical rows) while `pageSize`/`page` work, so that reader takes a limit and a page and **states that it offers no window**; the two alt-data endpoints **ignore every ticker parameter** (`tickers`, `symbols`, `parameters[tickers]`, `company_symbol` all returned the identical market-wide stream — and the published docs name `tickers` as the filter, so the documented parameter is the ignored one), so those readers page the stream and filter locally and say so; `/v2/bars` **requires `interval`**, without which the vendor answers 200 with a stub row carrying no candles; and **the paging parameter is spelled differently per route family** — the `/v1/*` endpoints page on lowercase `pagesize` while `/v2/news-removed` honours camelCase `pageSize`. `/v2.1/fundamentals` takes `symbols=`, not `tickers=`.

**The paging spelling was a defect in this change's own first version, found by running it.** `_filtered_pages` used `pageSize` on the Form 4 stream. The vendor does not error on the wrong spelling — it silently returns a *different slice* (pages came back ragged at 50/150/141 rows), so the reader walked four "pages", found no AAPL row, and reported a typed no-data. AAPL is on the fourth *real* page. Corrected to `pagesize` for the `/v1/*` readers only, with `test_the_two_firehose_readers_page_with_the_lowercase_spelling` and `test_news_removed_still_uses_the_camelcase_spelling` pinning both spellings so they are not "unified" later. Verified live after: real AAPL Form 4 rows (Jennifer Newstead, SVP — disposed 30,104 shares under a 10b5-1 plan) and real congressional trades (Cleo Fields, D LA 6).

**Tests.** `tests/test_benzinga_surface.py`, 27 tests. The two transport tests are the failing-first proofs: each was re-run against a faithful reconstruction of the pre-fix behaviour — `BASE` re-pinned to `/api/v2`, and `_unwrap_records` reverted to collapsing every dict — and each **turned red**, with a byte-identical sha256 restore of the mutated file. `tests/test_gate_env_toggles.py` gains the `enable_benzinga_surface` registry entry and the `_feature_gate(` access idiom.

**A pre-existing registration gap, fixed in the same pass.** `enable_options_surface` and `enable_risk_free_curve` shipped with a `DEFAULT_CONFIG` key, an `_ENV_OVERRIDES` row and a `.env.example` line but **no `docs/gate_registry.md` row and no registry entry** — three of the five places. They were also *unregisterable*: their only read site is `_feature_gate("enable_x", ...)`, an idiom the Status-enforcing scan did not match, so the scan found no read site and the registry could only have called a live gate `inert`. The scan now matches `_feature_gate(`, and both gates are declared `wired` with a doc row.

**Web impact**: additive. `run_card.json` and `complete_report.md` are unchanged in shape; no existing tool, CLI flag or JSON field is removed or renamed. Five new tool names exist (`get_guidance_revisions`, `get_fda_calendar`, `get_offerings_calendar`, `get_analyst_actions`, `get_news_removed`), all returning `DATA_DISABLED` while `enable_benzinga_surface` is off (it is off in `.env`). Two existing chains change order — `news_data` (Benzinga second) and `get_insider_transactions` (moomoo first) — so a run's served vendor for those methods can differ from a previous run's, and the news analyst may receive Benzinga headlines where it previously received another vendor's.

**All four phases of the score context contract — the engines become mandatory and their results are SUPPLIED, and every engine-score tool binding is dropped (2026-09-19).** Implements `docs/scores/ScoreContextContract.md` §11 in full, under the owner's four decisions (§13).

- **The contract in one sentence:** *the LLM must not decide whether a mandatory scoring calculation happens, but it must receive that score, its coverage, and its supporting measurements before forming its thesis.* The engines already ran in code (`trading_graph.py:651`, no LLM in the path); what was discretionary was their **delivery**.
- **§13.1 — every analyst now receives the FULL scorecard, not only the engine it owns.** New `report_hygiene.scorecard_context_block` renders all eight engines from the run's **one snapshot** (`state["quant_scorecard"]`), so it is **read, never recomputed** and the prompt cannot drift from the report. Previously the market analyst saw `Technical 62` without `Regime 81` or `Risk 79`, and so still had to infer the relative weight of regime and risk — the exact implicit weighting this contract exists to remove.
- **§13.2 — `TradeScore` is included, labelled downstream and non-instructional, and printed LAST.** `ENGINE_GATES` already orders `trade` last, so the ordering falls out of the table rather than a second list. The block states it is *NOT a trading instruction, NOT a position size and NOT a gate*, so the composite is a reference point rather than an anchor.
- **§13.3 — all eight engine-score tool bindings are DROPPED.** *Engine-score tools are application-internal calculation mechanisms, not LLM-facing analytical tools.* Keeping both a supplied number and a callable route invites *"is the supplied score authoritative, or should I call the tool again?"* — and it was a live duplicate: the owned engine reached its report **twice**, once supplied and once bound. `toolsets.engine_score_tools` and its `_SCORE_TOOL_BY_NAME` map are deleted, the four `tools.extend(...)` calls removed, the now-dead `_enabled` helper removed, and all eight leaves declared in `TOOL_LEGACY_BINDING` with their real consumer (the supplied block). The **exploratory** tools are untouched — only the eight engine leaves go.
- **§13.4 — every engine prints score, coverage, required floor and status.** The floor already existed and was already **enforced** (`combine` withholds below it, `score_engine.py:206`) and already **printed in prose** (`withheld` `:208`, `basis` `:216`) — but it was not a **field**. `combine` now returns `"floor"` beside `"coverage"`, and the eight engines propagate it from their own top-level `combine` call. Resolved through the one implementation `coverage_floor` (`:45`), never a second constant.
- **Two units that must not be conflated, recorded in the code and the doc:** the floor is a component **COUNT** (`len(present) < floor`) while `coverage` is a weight **FRACTION**. The owner's illustrative `Coverage: 45% / Required: 50%` mixed the two; a *printed* score always meets its floor (below it the score is withheld, never `0`, never `50`), so the honest rendering is the count form the kernel already emits.
- **The canonical prompt contract (text2) is `report_hygiene.MANDATORY_ENGINE_RULES`** — one constant read by every analyst, so the wording cannot drift between them. It states the eight engines by name, forbids selective evaluation, forbids substituting `0` for a missing measurement, and requires the coverage floor to be respected.
- **Phase 3's mapping half was already built:** `format_engine_detail` renders the non-monotonic triple (`RSI raw 53.26 -> aligned 85`), so the delta this work adds is the **floor/status rows** and the coverage normalisation, not the mapping.

**A `floor` key added to a return shape, and the three consequences found by running the suite rather than reading it.**
- **`combine` gained one key** and the eight engines propagate it; `test_event_state.py`'s key-set assertion (which pins that the event state carries **no gate or size key**) gained `floor` — the token check below it still governs, and `floor` is neither.
- **Three producer line-pins went stale** because inserting one line shifts every `def` after it: `regime_score.py`'s own component table pointed `market_trend` at `:343` (now `:344`) and `vix_term_structure` at `:400` (now `:401`), and `test_regime_score.py` hardcoded the first. **A pre-existing fourth was found by sweeping rather than by the failure**: `regime_score.py` pinned `analysis_tools._vix_percentile_read` at `:6988`, which had drifted to `:7976` — the test tolerates a stale pin outside `strategies/`, so nothing had ever reported it. All four corrected, and a sweep now reports **0 stale pins**.
- **Five tests asserted the removed binding contract.** None was deleted: each was re-pointed to the stronger statement it now defends — the engine leaf is bound to **no** analyst *whatever the gate*, which is §6 acceptance (a) satisfied by construction rather than by a branch. `test_gate_off_binds_no_score_leaf_at_all` became a tautology and was folded into the new assertion rather than kept as a test that can no longer fail.

Tests: `tests/test_engine_ownership_map.py` re-pointed and extended (7 new: full scorecard to every analyst, TradeScore last and labelled, floor printed, `NA` never `0`, empty on the master gate, empty without a snapshot, a gated-off engine omitted from the rows); `tests/test_score_engine.py` +2 (the floor is exposed and agrees with `coverage_floor`). **Six behaviour mutations each turn their test red**, then byte-identical restore across all three source files: the master gate not honoured, the floor dropped from the row, an unmeasurable engine rendered as a number, the non-instructional label removed, `combine` no longer exposing the floor, and an engine leaf bound to an analyst again. Engine suite **4973 passed / 6 skipped**.

**Web impact**: none. No tool name, CLI flag or JSON shape is added, removed or reshaped — `run_card.json`, `complete_report.md` and the decision artifacts are unchanged in shape, and `quant_scorecard`'s snapshot keys are untouched. With `enable_quant_scorecard` on (it is on in `.env`), each analyst prompt now carries the full scorecard in addition to its own engine block; with it off, every prompt stays byte-identical to a pre-scorecard one, and the four toolsets are byte-identical **whatever the engine gates say**, since no engine leaf is bound at all.
...
**The FundamentalScore appendix was arithmetically impossible, and the card's number was a second producer (2026-09-20).** Found by reading `reports/MSFT_20260920_145644/1_analysts/fundamentals.md`, which prints a composite of **67.71/100** above four sub-scores that are **all zero**:

```
### FundamentalScore — 67.71/100
- FQS: 0/100
- FGS: 0/100 over 1 components
    withheld: {'FTNT': 'coverage 0 of 2 factors < floor 2', 'GEN': ..., 'NOW': ..., 'RBRK': ..., 'ZS': ...}
- VS: 0/100 over 2 components
- FRS: 0/100
```

No weighted average of four zeroes is 67.71, and the `withheld` dict names **peer-panel tickers**, not MSFT. Both defects were reproduced before being fixed, and both fixes were verified against live engine output.

- **The renderer read a panel-keyed structure as a category-keyed one.** `format_engine_detail` was written for `TechnicalScore` (and regime/risk/sentiment), whose categories carry `score`/`present`/`withheld` as scalars. `FundamentalScore` is a **panel** engine: each sub-score keys its `scores`/`bands`/`coverage`/`withheld` **by ticker**. Three misreads followed, and all three are visible in the block above:
  - the score came from `next(iter(scores.values()))`. `factors.category_scores` inserts names in **ascending percentile order** (`sector_rank._pct_rank` walks the sorted values), so the first entry is always the **worst peer** — whose percentile is `0.0`. That is why all four printed zero: not a coincidence, a structural certainty;
  - `withheld` was printed verbatim, and it is keyed by ticker, so the block named the peer panel's names;
  - `present` fell through to `factors_NA`, which is the set of factors with **no supplier** — so `over 1 components` counted a *missing* factor (`rev_cagr5`), and `over 2 components` counted `fcf_yield`/`val_z`. The lookup against `components` could never match anyway, because the panel engine's `components` is keyed by ticker too.
  The renderer now resolves the analysed ticker's own row (from the snapshot's own `ticker`), prints the sub-score's **band** and its **coverage over its own factor set**, and names the NA factors as the gap they are (`factors NA: rev_cagr5`) rather than counting them as present. **Verified on live MSFT output:** FQS 75 (elite) + FGS 40 (stalling) + VS 75 (below-market) + FRS 87.5 (fortress) = 277.5, ÷ 4 = **69.375 ≈ 69.38** — the printed composite now recomputes from the rows beneath it.
- **The run card recomputed the fundamental score instead of reading the run's snapshot — defect D-8, recurring.** `_run_card_fundamental_score` called `fundamental_score_for_ticker` again at write time, so one run had **two producers** of the same number: the prompt's scorecard said **67.71** and the card's own block said **55.21**. `_run_card_trade_score` already carries the fix for this (`P12-5`: *"it reads the run's own score snapshot when there is one … so all three surfaces print one number rather than three assemblies (defect D-8)"*); the fundamental block never received it. It now reads `final_state["quant_scorecard"]["engines"]["fundamental"]["result"]` and recomputes **only** as the fallback for a legacy state written without a snapshot. The docstring's old claim that the score *"is recomputed deterministically at write time"* was false and is replaced by the measurement below.
- **Why the recompute was not merely redundant — the panel is not reproducible.** `peer_universe.resolve_peer_universe` re-fetches statements and sectors **live on every call with no cache**, and a transient vendor failure returns `None` metrics that silently drop names from the cross-sectional scored set. Measured on `(MSFT, 2026-09-20)`: **113 metric diffs between two identical calls** (CRWD `accruals`, `eps_yoy`, `f`, `m`, `noa`, `price_to_cash_flow`, `price_to_free_cash_flow`, `revenue_yoy` all flipping to `None`), and **three consecutive composites — 61.25 / 69.38 / 79.17**. That is the mechanism behind the 67.71-vs-55.21 split, and it is why the card must read the snapshot: the snapshot is the one number the analyst was actually handed.

**Web impact**: each analyst report's FundamentalScore appendix now prints the analysed ticker's own sub-scores with their bands and coverage, and the block's rows recompute to the printed composite instead of contradicting it. `run_card.json`'s `fundamental_score` block now matches the run's `quant_scorecard` block, so the two can no longer disagree within one run. Trees already written are unaffected — they carry the old numbers and would need a re-run to gain the corrected appendix.

**Nine standing defects fixed, and one of the recorded defects was WRONG — the signald finding was a false premise (2026-09-20).** Every fix below was found by RUNNING the path, and each carries a failing-first proof by mutation.

- **The signald "no supervisor exists" claim was false, and the real gap is different.** The record said *"the daemon is DEAD and nothing restarts it — no supervising restart process exists."* Verified against Task Scheduler: `Signald_Daemon` **does** exist (logon + weekly MON–FRI 08:00, `RestartOnFailure` PT1M ×10, `ExecutionTimeLimit` PT0S) and so does `Signald_Watchdog` (weekdays 08:00, every 5 min for 7 h 30 m). Measured on the 2026-09-18 death: the daemon died **15:15 CT** (`^C`; `Last Result` `-1073741510` = `0xC000013A` `STATUS_CONTROL_C_EXIT` — a deliberate `schtasks /end`, not a crash); `RestartOnFailure` **did not and cannot fire** (it retries a *failed* run, and a deliberate end is not one); the watchdog **did** fire at **15:30** with `Last Result: 1` (the `heartbeat_loss` page). So **detection works and recovery is manual**, and because the watchdog window is weekdays 08:00–15:30 a Friday-afternoon death is paged **exactly once** and then silent all weekend. `TradingExecution/docs/RUNBOOK.md:36` said *"Nothing runs the watchdog for you"* while line 56 of the same section documented the scheduled task — **stale and self-contradicting, corrected** (`TradingExecution 3423f2b`). This is the D-6 failure mode (*a stale line hiding a wire*) in our own record.
- **The ATR window is now part of the basis, and the ledger's `defect` class drops by 27.** `_basis_of` recorded only the unit class, so ATR quoted at several **windows** (ATR-14, the chandelier's, the swing set's) collapsed into one basis and `classify_conflict` read two windows as one metric printing two values. New `_windowed_basis` binds the window the report actually prints beside the figure (`ATR(14)`, `ATR-14`, `atr_14`, `14-bar ATR`, `ATR 14` → `atr:14`; `ATR(14, simple mean of TR)` → `atr:14-simple`), and does NOT read the `(n x ATR)` multiple leg or a `22-bar` anchor as a window. **Re-measured over the same 56 trees in one run, before and after:** `defect` **59 → 32** (−27, exactly the ATR rows), `basis_difference` **186 → 215**, `unresolved` **18 → 16**, **TOTAL 263 → 263**. The total is unchanged because nothing was created or suppressed — it is a pure **reclassification**, which is the strongest available evidence the fix is a classification fix rather than a quiet excuse.
- **The kill-switch state is now PRODUCED, so `EXIT` / `NO_TRADE` can finally fire.** Three call sites read `state["kill_switch_state"]["active"]` (`prompt_metrics.py`, `reporting.py` ×2) and **nothing wrote the key**, so every read took a silent `False` default and `portfolio_action_from_gate`'s `kill_switch` leg was dead. The producer is measured — the one realized book drawdown this run already resolved (`risk_ctx["book_drawdown"]`, peak-to-trough, the same number the governor gates on) against the configured HWM hard tier — and the session-p&l leg, which has **no in-engine producer** (the executor owns live p&l), is passed `None` and **named** in `unmeasured_legs` rather than guessed.
- **`TRADINGAGENTS_ENABLE_COMPUTED_CONTEXT` was INERT.** Documented in `.env`/`.env.example` since 2026-09-04 and read by **nothing** — the Phase A-E block was built unconditionally at `trading_graph.py:688`, so setting the variable to `false` silently did nothing. It is a real gate now, registered in all five places. **Its default is `True`, the only context gate that is** — the shipped behaviour was "always on", and defaulting it `False` would have silently dropped a shipped prompt block from every run without `.env`; §7 of the gate registry records why.
- **The structured debate's consensus exit was dead (D-10).** `_complete_round` reads the independent pre-debate agreement off the section **channel**, but the sampled stances live in the graph **state**, which only `l1_node` sees — so the key was never written, `termination_check` always received `None`, and **every debate ran to its round cap**. `l1_node` now joins the two. `independent_agreement` gained a `roles` parameter so the research debate scores its **own** bull/bear pair rather than the risk trio's — parameterised, not reimplemented (one producer). `None` keeps the contour off: a fabricated `1.0` would terminate a debate that never converged.
- **A forward `PEG` had no caller.** `value_dip_setup` accepted `forward_peg`, built a re-rating evidence row from it, and **no caller in the repo passed it**. Wired to a real producer — `dataflows/yfinance_sector.fetch_estimate_trend` (the reader the screener's RevEC leg already uses), giving both legs of the definition from one source: forward P/E = price / next-FY consensus EPS, expected growth = next-FY / current-FY − 1 — behind the **existing** `enable_analyst_estimates` gate, with the basis printed on the rendered row. Trailing `eps_yoy`/`revenue_yoy` were **rejected** as proxies that do not measure forward growth, and yfinance's `pegRatio` is a *trailing* P/E PEG (a mislabel).
- **`value_dip_setup`'s `catalyst_window: bool = False` was the last false assertion of the D-11 class.** `regime_gate_read` is tri-state; this caller still handed it an explicit `False` it never measured, asserting a catalyst read nobody made. Now `bool | None = None`, forwarded unchanged. No veto reinstated.
- **The sentiment baseline was written TWICE per run.** `compute_social_scores` **appends** to the rolling baseline that `surprise_velocity` z-scores against, and it had **three callers in one run** — the sentiment analyst's prefetch plus two tool-side readers (`get_sentiment_computed` on the market toolset, and the SentimentScore component assembler). So one run wrote today's score two or three times, and each later read's velocity was computed against a history already containing today's value. The second reader also passed **no `cache_dir`**, so it could write into a *different* baseline file than the prefetch reads. New `record: bool = True` makes the prefetch the single **RECORDER** and every other caller a **READER**.
- **A provider failure was counted as a verifier SUCCESS and lost its reason.** The exception branch fell straight through to the shared entry assembly, so a rejected call produced `overall: UNKNOWN` with **no `reason`** and still incremented `stem_succeeded`, consuming the `max_calls` budget. A run whose calls were all rejected therefore read `UNKNOWN` on every stem with nothing to tell it apart from a verifier that never started — which is exactly what Alibaba's *"Output data may contain inappropriate content"* rejection produced. The reason is now recorded (`verify call failed: <type>: <msg>`, the same shape the `max_calls` / unusable branches use) and the stem no longer consumes budget.
- **The recorded `sentiment_tools()` "defect" is NOT a defect.** It was carried as *"bound to nothing in production"*; the sentiment analyst binding no tools is **documented deliberate design** in four places (`analyst_execution.py:39` *"Binds no tools: its data is prefetched into the prompt"*, `toolsets.py:496`, `docs/scores/README.md` D-9, `ResearchLayerWiring.md`), and the toolset exists precisely so the **engine** can reach those leaves. The real defect in that cluster was the double-append above, and it is fixed.

**Failing-first proofs, by mutation, with byte-identical restore (sha256).** The kill-switch producer removed → `test_kill_switch_state_is_produced_and_can_fire` fails (`assert None is True`). The D-10 join removed → `test_d10_consensus_exit_reads_the_section_agreement` fails (`KeyError: 'independent_agreement'`). Both then pass unmutated. **The mutation harness itself had to be fixed**: it round-tripped through `str` with `newline=""`, and this repo is `core.autocrlf=true` on a mixed CRLF/LF tree, so the "restore" silently rewrote two source files' line endings. It now reads and writes **bytes**; both files were restored to CRLF and `git diff` confirms the content is unchanged (96 insertions / 4 deletions, identical before and after).

**Also closed on sight:** the 47 pre-existing ruff findings in `tests/` (all behaviour-preserving), so `ruff check tradingagents/ tests/` is **clean for the first time**. And `docs/scores/MEASUREMENT_FINDINGS.md:262-263`'s stale `news_score … **0** measured` claim was re-measured rather than assumed: the **panel** axis is still genuinely `0` and the reason is structural (none of the engine's 11 declared component names is in the panel's 37-name metric vocabulary — a bulk cross-section has no per-article leg), but the **live** path now measures **2 of 11** (`relevance`, `novelty`): of the 8 `run_card.json` cards carrying a `news_score` key, 3 carry a real score (MSFT 57.6 twice, QCOM 58.8) and 5 carry `"score": null, "unavailable": "no news producer measured"` — that `null` was the `ccd0c5b` defect.

**Web impact**: none. No tool name, CLI flag or JSON shape changed. `kill_switch_state` and `compute_social_scores`'s new `record` parameter are additive; the ledger's `basis` strings gain a window suffix (`atr:14`); the verifier's `verify_flags.json` gains a `reason` key on a failed stem; and `enable_computed_context` defaults `True`, which is the behaviour every existing run already had.
...
**Design study: Dream-RSI (arXiv:2609.14858), and what a replay simulator would have to be built from here (2026-09-20).** `docs/design_dream_rsi_replay_simulator.md`, new. No code changes, no gate, no behaviour touched.

A Google/UMD/DeepMind paper that optimises the *exploration policy* of a long-horizon program-discovery loop by turning completed discovery trees into a **replay simulator** — candidate policies are scored offline against recorded outcomes at zero execution cost, and the best is redeployed. Read against this repo, which already has three of its four ingredients:

- **The mechanism does not transfer, for two independently checked reasons.** (1) **A recorded tree here is a chain, not a tree.** Dream-RSI replays because its tree holds outcomes for branches a different policy would have taken; this repo runs **one trajectory per analyst**, so replay can only evaluate *truncations* of what actually ran — you can ask "what if the debate stopped a round earlier", never "what if we opened a different branch". (2) **The round structure is not recorded at all.** Verified on `reports/WDC_20260915_120300`: `tool_evidence.json` leaves carry `ts` = **`time.monotonic()`** (`evidence_gather._leaf`), a process-relative float and not a round index; those leaves come from the deterministic gather's **single pass**, not the LLM's rounds; `run_card.json` has **no round counts**; and the LLM's own calls are journaled by `tool_call_log.log_tool_call` to `<data_cache_dir>/tool_calls/`, **outside the report tree**, with no round index either. So `N_im` and `k_i*` in the paper's objective have **no producer here**.
- **The headline numbers are not comparable.** "162× fewer agent calls" is measured against SimpleTES at **51,200 generations**; a run here is ~12 LLM calls. There is no two-orders-of-magnitude budget to recover.
- **What does transfer, ranked.** (a) **The selection rule must not be the paper's.** Its `V^{m*} ≥ V^0` guarantee is **in-sample** — selecting the argmax of M candidates on the same t worlds is the winner's curse, and this repo already owns the correction in `strategies/evaluate.py`: `deflated_sharpe:103` ("penalize multi-trial tuning"), `pbo_flag:235` ("best-trial in-sample picks fail out-of-sample"), `reality_check:311`, `spa:351`, `purged_cpcv_splits:181` — **all unused on this question**. (b) The **prefix-only** constraint from Appendix B.2: a policy may read only what it could have observed at decision time — the repo's allocation is currently scattered across `research_depth`, `MAX_TOOL_ROUNDS` and a dozen gates, with nothing stating what those knobs may read. (c) A **cost term**: quality (`verify_flags.json`) and cost (`reporting._run_card_llm_cost_est:980`) are both measured and **never joined**, so "was the 5-round debate worth it" is unanswerable per run. (d) **§5.1 as a falsifiable experiment**: the paper measures explicit directional guidance as *consistently underperforming* its unguided counterpart — and this repo injects exactly that on by default (`enable_reflection` `default_config.py:819`; `reflection_hint` `strategies/reflection.py:83`; `build_reflection_context:109`; `past_context` `trading_graph.py:619-625` → `portfolio_manager.py:134-139`). It is a cheap ablation of an **always-on** feature, runnable with `context_ab.mcnemar_exact:309` over the 56 recorded trees.
- **Prior work credited, not duplicated:** `scripts/context_ab.py` **is** a replay simulator over recorded trees already (`decision_items_from_tree:1189`, `ARMS:396`, `compact_prose:454`) — for **context** policies. It does not replay *how much research to buy*.
- **A pre-existing discrepancy recorded:** `deflated_sharpe`'s docstring says "Euler-Mascheroni-based" but its body is the simplified `observed − sqrt(2·ln n_trials)`. Any adoption should not assume more rigour than the function has.

The plan is ordered so nothing depends on the loop, and Phase E (replay + corrected selection) is explicitly **not** where to start.

**Web impact**: none — a design document only.
...
- **The proposal is directionally right and mostly already built.** The deterministic gates (`risk_governor.govern:37`, `risk_hierarchy.evaluate_hierarchy`), the deterministic decision context (`_compiled_decision_context:1282`, built unconditionally at `trading_graph.py:688`), the deterministic synthesis (`consensus.weighted_consensus:32`, `should_hold:57`), the pre-debate independent reads (`independent_vote:156`) and the engine ownership map (`quant_scorecard.ENGINE_SECTIONS:119`) all exist. The document maps each proposal item to BUILT / PARTIAL / NOT PRESENT with a verified `file:line`.
- **Two measured findings.** (1) The forced-evidence block rendered into each analyst's **system prompt** is **139,372 chars (~34,843 tokens)** for one QCOM run — fundamentals 64,443, market 38,953, news 35,976 — against ~6,044 tokens of analyst report produced. (2) **The PM never reads the analyst reports**: grepping `portfolio_manager.py` for the four report keys returns nothing; the reports reach the decision only through the RM's `investment_plan` and the Trader's plan.
- **It does NOT claim the premise is proven.** The literature establishes context degradation (Chroma *Context Rot*, 18 models, 2025-07-14) and LLM trading conservatism (FINSABER, arXiv:2505.07078 — *"overly conservative in bull markets"*) — but FINSABER attributes that to **regime miscalibration**, not context volume. No general law "long context → risk aversion" exists. The document says so and designs the falsification first.
- **The genuinely missing pieces**, in value order: any per-run measurement of prompt size; the **uncertainty vs negative-evidence** distinction (an `NA` is not a bearish fact); a **challenge pass with a closed output vocabulary** that can only invalidate; and a bounded Decision Packet.
- **Prior work credited, not duplicated.** `docs/implementation_plan_openrouter_large_prompt_ttft.md` W5 already frames this and left items 1-3 open; its debate-digest A/B is measured (8,204 → 4,530 tokens, −45%; grounded citations 39 → 28, fabricated figures 21 → 10). `scripts/context_ab.py` already A/Bs context size against accuracy with an exact McNemar test — the document specifies **extending** it with a decision-outcome item type rather than building a parallel harness.

- **Revised same day from owner feedback (v2).** Five corrections, each recorded in the document rather than silently applied. The load-bearing one is methodological: the original four arms **confounded context volume with representation**, so a 2x2 factorial now separates them (`context-size effect = C2 - C1`; `representation effect = P1 - C1`). The others: the experiment reports the **full decision distribution** (BUY/HOLD/SELL, entropies, confidence) rather than CCI alone, because two very different behaviours can share a CCI; the packet no longer prints an `AGREEMENT 7/8` ratio (it becomes a **directional distribution**, and no `DECISION` line exists — *the packet constrains the information channel, not the decision*); **uncertainty counters move into Packet v1** so the first experiment can test H1b at all; and the challenge pass gains a **materiality test**, because "the packet contains a contradiction" would otherwise let `fundamental bullish` vs `regime bearish` invalidate — recreating the conservatism the design exists to remove. Phasing reordered so the challenge pass is **last**, not first. The conclusion is softened to what is established: *"the repository already contains most of the primitives … the remaining work is primarily deterministic assembly, explicit uncertainty/conflict representation, and experimental validation."*

- **Revised again (v3) from a second round of owner feedback - six technical corrections.** (1) **The volume claim was overstated**: `C2 - C1` is *not* pure context volume (a digest changes fact count, ordering, redundancy, placement), so C1 must be built by a **deterministic information-preserving transformation** and the harness must report a **retention table** (unique metrics/figures, bullish/bearish/uncertainty facts, source sections) - without it the factorial's first contrast is uninterpretable. (2) **Paired decision flip rates and transition matrices** added; aggregates can hide `BUY -> HOLD` on identical snapshots, so a transition matrix is more informative than CCI. (3) **Raw decision separated from gated action** - decision conservatism is measured *before* deterministic risk gating, because a `HOLD` may be `LLM = BUY` + `risk_gate = REJECT`, a different phenomenon entirely. (4) `PERMISSION` reclassified from evidence to **`RISK CONSTRAINTS`** - it is a decision-domain constraint, not a fact to weigh. (5) **H1b renamed to an uncertainty-*framing* effect** - a packet that separates uncertainty also signals that the architecture considers it important, so the miscounting claim is not isolable. (6) The challenge invariant restated as **"may not create a new caution rationale"** - "cannot add caution" is false, since invalidation *is* increased caution. Also: the architecture now distinguishes the packet as an **information** boundary from the **decision** boundary (`RAW DECISION`) and the **execution** boundary (deterministic gates), with the Phase 0 telemetry block expanded into a decision-context observability layer.

- **v4 - a third round of owner feedback, four corrections, and the document is now the frozen contract.** (1) **The telemetry boundary is named, not implied**: `raw_rating` was the wrong label - it says "raw" without saying *raw relative to what*, and could later be populated from an already-normalised `PortfolioDecision`. Replaced with `llm_output` (the PM's structured emit, before ANY transformation) / `deterministic_postprocess` / `execution`. (2) **The retention claim was too absolute**: retention below 100% does not invalidate the contrast, it *renames* it - the experiment reports a **context-volume effect**, a **content-retention effect**, or a **representation effect** depending on what actually changed. (3) **Snapshot identity promoted to an invariant** - all arms for a paired observation must consume the same deterministic evidence snapshot, asserted via `data_snapshot_hash` / `engine_output_hash` / `model_parameters_hash`; a mismatch invalidates the pair rather than becoming a footnote. (4) **Independent reads are diagnostics, not evidence** - otherwise `independent_agreement` silently becomes a consensus the PM defers to; it is a *measurement of disagreement*, which is what §11's conditional expansion needs as a trigger and nothing else. Also added: the per-layer **may / may not** contract table, the paired transition output format, and the eight-item Phase 0 target (zero behavioural change).

**Web impact**: none — documentation only. No code, config, gate, tool or artifact shape changes.

**The engine section is now rendered into each analyst report deterministically — the placement no longer depends on the model (2026-09-19).** Found by running the QCOM report after the score context contract landed: `sentiment.md` came back at **402 characters with no engine in it**, where the earlier run had carried it.

- **The cause was structural, not model variance.** `SentimentReport` (`agents/schemas.py:444`) has exactly `overall_band / overall_score / confidence / computed_score / computed_velocity / sample_size / narrative` — **no field for an engine result**. The supplied block reached that analyst's *prompt* but had nowhere to land in its *response*, so whether the number appeared at all was model discretion. Measured both ways on the same symbol and date: the earlier QCOM run's model volunteered it into `narrative` (*"the engine composite reads 80.3/100 (bullish-positioned) at 50% coverage"*), the next run's did not. That is precisely what the owner's invariant forbids — *"the model may interpret an engine result, but does not decide whether or where the authoritative engine result appears."*
- **New `report_hygiene.engine_report_section(analyst_key, ticker, trade_date, cfg, snapshot)`**, and `write_report_tree` appends it to each `1_analysts/*.md`. **Renders, never recomputes** — it reads the run's one snapshot (`trading_graph.py:651`, handed down as `state["quant_scorecard"]`), so the report's number and the prompt's number cannot disagree. It renders only the engines that analyst **owns**, read from `ENGINE_SECTIONS`, so placement stays the ownership map's decision.
- **The section carries its own evidence chain**, not a pointer to section V: the headline (score, band, coverage, required floor, status) plus that engine's category → measurement rows via `format_engine_detail` on a single-engine snapshot — including the non-monotonic triple (`rsi=54.42 rsi_aligned=85 mapping=producer-defined non-monotonic band`). Live: 1118 chars for sentiment, 1679 for market.
- **The prompt changed with it.** `engine_score_block` said *"Report it under a subsection headed `## XScore (engine score)`, quoting the score, its COVERAGE and its band VERBATIM"*; it now says the section is written **by the system** and the analyst's job is to **interpret** it. One producer for the section, so code-rendered and model-written copies cannot both appear.
- **Gate off, or no snapshot, renders nothing** — a gate-off tree stays byte-identical.
- **A stale claim corrected in the design set.** `ResearchLayerWiring.md` §4.2 describes Level 2 as *"an addition to `write_report_tree` … still NOT built"*. It **is** built — the `## V. Engine score detail` section appends `format_engine_detail(snapshot)` to `complete_report.md` (`reporting.py:~1807`, `P12-10`/`P12-12`). What was genuinely missing is the **per-analyst** section, which is what this adds; the doc now says so.

Tests: `tests/test_engine_ownership_map.py` +6 — sentiment gets its section deterministically (the schema gap), an analyst gets only the engines it owns, empty on the master gate, empty without a snapshot, an unmeasurable engine named `NA` never `0`, and the render is byte-identical for the same snapshot.

**Web impact**: none. No tool name, CLI flag or JSON shape changed. `1_analysts/*.md` gains an engine section for each analyst whose engine measured, with the gate on — which is a rendered-markdown addition, not a schema change; `run_card.json`, `research_decision.json` and `complete_report.md` are untouched by it.
...
- **Nothing in this tree read a real yield before this**, while `strategies/dcf.py::wacc_from_beta:28` already consumed a **nominal** 10y Treasury as `rf` and **assumed** the premium (`erp: float = 0.05`, its own docstring calling 0.05 *"the assumed equity risk premium"*). `dataflows/eodhd.real_yield_points_eodhd` / `::get_real_yield_rates_eodhd` read EODHD's `/ust/real-yield-rates` — one call, **895 rows / 179 dates**, tenors 5Y/7Y/10Y/20Y/30Y.
- **`inflation_expectation_eodhd` is the ONE producer of `nominal − real`** (master rule 15). Every result carries **both legs' dates, the gap between them, and the basis string** — a nominal/real subtraction across a stale leg is a different number from one across an aligned pair, and the caller has to be able to see which they got. Measured live at 2026-09-17: 5Y 2.32, 7Y 2.34, 10Y 2.33, 20Y 2.45, 30Y 2.25.
- **`federal_reserve.treasury_curve_points` is new**, and `get_treasury_curve` now renders **from it** — one producer, two presentations. The pairing needed a structured nominal leg, and a second parser would have been a second producer for the same read. The rendered output is byte-identical; `tests/test_federal_reserve.py` passes unchanged.
- **Three corrections the build forced, each measured rather than assumed.**
  - **`year` is not a parameter this endpoint has.** The design's signature sketch was `(tenor=None, year=None)`, but the vendor **ignores every query parameter** on this path — `year=2026`, `filter[date]`, `filter[tenor]` and `from`/`to` each returned the identical 895-row body. The filtering is client-side, so a `year` argument would have been a parameter that **silently did nothing** — worse than a named gap. Dropped, with the reason in the module.
  - **A tenor the vendor does not publish is named, never interpolated.** `real_yield_points_eodhd("3M")` raises with the published set in the message; TIPS start at 5Y and there is no bill-end real yield to interpolate from.
  - **A rate that will not parse is dropped, never carried as `0.0`.** A 0% risk-free rate silently inflates every DCF it touches.
- **The first consumer was wrong twice, and only running it showed that.** The screener's `--rates` block initially called the pairing once per tenor — six times — each re-reading **both** whole-surface legs: twelve network reads where two suffice, and a **158-second** run. It also printed the entire **179-row** series into the report head. Both fixed: the pairing accepts pre-fetched legs (`real_points=` / `nominal_curve=`) so the subtraction still happens in exactly one place while each read is paid for once, and `get_real_yield_rates_eodhd(tenor, tail=N)` bounds the rendered rows while the header still states the full coverage and the withheld count. Re-run: **60 s, 10 rendered rows**.
- **Gate off makes no call**: the block prints `unavailable` with its reason, and a gate-off run is byte-identical.
- **`dcf.wacc_from_beta` still carries `erp = 0.05`.** P0 makes the premium *measurable*; promoting the measurement into the valuation would silently rewrite every DCF the tree produces, which is a decision contract rather than a defect. Left for the owner.
- Tests: `tests/test_eodhd_rates.py`, 20 tests, offline (both vendor seams mocked). **Seven behaviour mutations each turn their test red** — aligned-flag forced true, unparseable rate carried as `0.0`, unpublished tenor returning every row, a dead leg becoming `0.0`, the lookahead guard removed, the tail bound ignored, supplied legs ignored — then byte-identical restore.

**Web impact**: one new advisory block. `scripts/value_screener.py --rates` prints the real-yield tail and the five-tenor nominal/real/expectation table at the head of the report. It never gates a row, never changes a valuation, and with the gate off it prints `unavailable` and makes no call. New env var `TRADINGAGENTS_ENABLE_EODHD_RATES` (documented in `.env.example`).

**The unused EODHD surface is enumerated, and the supplied catalog is corrected against the live key (2026-09-19; `docs/design_eodhd_unused_surface.md`, new).** Documentation only - no code, no gate, no test.

- **The premise was wrong in two directions.** EODHD is **not** a removed vendor: the repo calls **8 of its paths** today (`eodhd.get_stock_data_eodhd:104`, `_sentiment_points_eodhd:153`, `get_news_eodhd:240`, `get_corporate_actions_eodhd:275` for both `/div` and `/splits`, `get_exchange_symbols_eodhd:328`, `get_market_snapshot_eodhd:358`, `get_top_movers_eodhd:383`, `get_top_movers_symbols_eodhd:408`). All 8 returned 200. And the supplied 68-endpoint catalog is **materially wrong about what the key can reach**: probed across five rounds (~100 URLs including path variants), the set splits into **9 reachable-and-unused**, **20 plan-gated (403)** and **~9 not found at any probed path**.
- **The `/ust/*` Treasury family is not in the catalog as a family and is fully open.** Found by guessing paths: `/ust/real-yield-rates` (895 rows), `/ust/bill-rates` (1,253), `/ust/long-term-rates` (537), `/ust/yield-rates` (2,506) - a year of rows per call, 42-187 KB each. The catalog's own Treasury section names only `GBOND` and `MONEY`, and **both of those are 403** (`This data is not available for your subscription plan.`).
- **A 422 probe separated "plan-gated" from "wrong path".** `/calendar/dividends` returns **422** *"filter.date eq field is required"* without params and **403** with them - so the route is authorized and the *plan* is the gate. Without that probe it would have been indistinguishable from a mistyped URL. This is why the doc records every failure with its exact message: an entitlement wall, a wrong path and an absent feature close differently.
- **Six of the nine reachable endpoints are REJECTED under master rule 15.** Reachability is not a reason to adopt. `/ust/yield-rates` and `/ust/long-term-rates` duplicate `federal_reserve.get_treasury_curve:116`; `/us-quote-delayed`'s sector/industry duplicate `yfinance_sector.fetch_sector:64` and its quote duplicates the already-used `/real-time`; `/id-mapping`'s `cik` duplicates `sec_edgar._cik_for:175`; `/eod-bulk-last-day` (45,009 rows / 6.6 MB in ONE call) is the same producer as `get_stock_data_eodhd`, batched - transport, not a source.
- **P0 is `/ust/real-yield-rates`, and it is the one endpoint here that changes what an existing number means.** Nothing in the tree reads TIPS or real yields (`grep -rn "real_yield\|TIPS"` returns nothing), while `strategies/dcf.py::wacc_from_beta:28` already consumes a *nominal* 10y Treasury as `rf` and **assumes** the equity risk premium (`erp: float = 0.05`, docstring: *"the usual central bank range is 4.5-6%"*). A real yield is the missing half of the pair that turns that assumption into a reading.
- **The adopted set is three endpoints, and the plan says why the other six are not.** P0 `/ust/real-yield-rates`, P1 `/ust/bill-rates` (auction `discount`/`coupon`/`maturity_date`/`cusip` - no existing producer), P2 `/id-mapping`'s **FIGI / LEI / CUSIP** (new to this repo) with **CIK explicitly demoted to a fallback**, since SEC is the authoritative registrar for its own identifier. P3 records `/eod-bulk-last-day` as a batched transport only.

**Web impact**: none. Documentation only - no key, CLI flag, JSON shape, prompt or rendered output changed, and no gate was added.

**P0+P1 of both vendor-surface designs are built: the K-line quota pre-flight, the batched market snapshot, and the estimate leg that could never fire (2026-09-18).** Two new gates, both default off, each with its registry row, `.env.example` line and test.

**`enable_moomoo_snapshot` - `docs/design_moomoo_unused_api_surface.md` P0+P1.**
- **P0 - the historical-K-line quota, read before the scan.** `moomoo.get_kl_quota_moomoo` renders `used / total / remaining` plus the recent request log; `kl_quota_remaining_moomoo` is the numeric form for a caller that must decide. `scripts/value_screener.py` reads it as a pre-flight: it logs the remaining count, and **refuses cleanly at zero** rather than discovering the exhaustion mid-run and leaving a partial watchlist. Live: `Used 26 of 100 - 74 remaining (74%)`.
- **`None` is not `0`.** An unreadable quota (OpenD down, no permission) returns `None`, which the pre-flight reports as unreadable - never as exhausted. Collapsing the two would refuse runs for a gateway outage.
- **P1 - the batched market snapshot.** `moomoo.get_market_snapshot_moomoo` renders 142 columns per name from one call: the valuation block, the pre/after/overnight session families, ETF NAV/premium and option greeks. Live: **122 symbols requested, 120 returned, 2 dropped, 0 failed** from one batched path.
- **The all-or-nothing endpoint is survived by a bisect.** `_batched_snapshot` chunks the request and, on the refusal, drops the named symbol and retries the chunk - bounded, so a chunk of entirely bad symbols terminates. Live against three deliberate bad symbols (a renamed `US.SQ`, a nonsense `US.ZZZZZ`, an OTC `US.SSLZY`): all three dropped **by name**, the three good rows still returned.
- **The design's per-symbol pre-validation is deliberately NOT implemented.** Pre-validating through `get_exchange_moomoo` is one `get_stock_basicinfo` call per name - N calls to save N calls, which destroys the point of batching. The bisect finds the same symbols for one retry each, and every one is named in `dropped`. The deviation is recorded in the docstring and in the doc.
- **Coverage travels with the number**: the render prints requested / returned / dropped / failed, names every dropped symbol, and states that these are **live snapshots, not dated closes** (a read taken after the close is an after-hours read, and the pre/after/overnight columns say which session each price belongs to). An absent cell renders `n/a` - a live `US.SPY` row shows `n/a` for the equity valuation block rather than `0.00`.

**`enable_analyst_estimates` - `docs/design_finnhub_yfinance_unused_surface.md` P0+P1.**
- **The leg that could never fire is now measured.** `analyst_revisions.estimate_change_index` documented its own gap (`:14-18`): *"the engine has current consensus and price targets only, never a 3-4 quarter estimate history, so this leg returns `unavailable`"*. `revision_index(history, levels=None, ...)` defaults `levels` to `None` and **the only caller that ever passed a value was a test** - so in production the leg was structurally dead. New `yfinance_sector.fetch_estimate_trend` supplies the five levels `yfinance.Ticker.eps_trend` carries, and `fetch_eps_revisions` supplies the vendor's own up/down counts.
- **The 90-day window is NOT four quarters, and the output says so.** This is the open question the design doc raised, and the answer decides the implementation: `eps_trend`'s columns are four *lagged reads of one quarter's consensus*, not four quarters. Feeding them in unlabelled would have made the leg print a **false basis** - its success string hardcoded the word "quarterly". `estimate_change_index` and `revision_index` gained an optional `level_basis` that names the series; the default is unchanged, so every existing caller's basis is byte-identical. The vendor path prints `over 4 vendor 90-day estimate-trend change(s)`.
- **The arithmetic is untouched** - only the label moves, and a test asserts the index is independent of `level_basis`. A measured leg with an honest label beats both a dead leg and a mislabelled one.
- **`RevEC` is a new watchlist column**, and `--revision-index` help says so. With the gate off, `RevIdx` is `revision_ratio` alone, exactly as before.

**A pre-existing display defect fixed on sight: a format spec rendered as a value.**
- `cell(v, fmt)` uses `fmt.format(v)`, so the two **printf** specs on the watchlist row (`"%+d"` for `RevUp`, `"%+.1f"` for `Inst`) rendered as the literal text `%+d` in every row. Because that literal is non-empty, **the column then survived the empty-column pruning** - an unmeasured column looked measured. Both are now `{:+d}` / `{:+.1f}`. Demonstrated: with the fix, a run whose `rev_net` is genuinely `None` now correctly **prunes** the `RevUp` column instead of printing `%+d` in it.

Tests: `tests/test_moomoo_snapshot.py` (14, new - the bisect, the offender resolver against both live message shapes, the bounded retry, the `None`-not-`0` quota, the coverage render, the gate both ways), `tests/test_analyst_estimates_wiring.py` (12, new - the five-level read and its row selection, a short series refused rather than trimmed, the labelled vs default basis, and the unavailable-to-measured transition asserted as the transition it is), `tests/test_value_screener.py` +2 (the format-string and prune-halves of the display defect), `tests/test_gate_env_toggles.py` +2 registry rows.

**Web impact**: additive and off by default - two new `run_card`-free surfaces, one new watchlist column (`RevEC`, present only when `--revision-index` is passed and the gate is on), and `RevUp`/`Inst` now render their values instead of `%+d`/`%+.1f`. No key, CLI flag meaning, JSON shape or prompt changed.

**The unused finnhub and yfinance surfaces are enumerated, and one finding closes a leg that could never fire (2026-09-18; `docs/design_finnhub_yfinance_unused_surface.md`, new).** Documentation only - no code, no gate, no test.

- **The scan.** `finnhub.Client` exposes **117** methods (this repo calls **9**); `yfinance` 1.5.2 exposes 5 module functions, 15 classes and a `Ticker` with **54 properties + 44 methods** (~23 attributes used). **108 finnhub** methods and **37** yfinance properties are unused. The used sets were extracted from the tree, not from memory. Live-probed: **61 finnhub** calls (paced 1.4 s) and **32 yfinance** calls.
- **The headline: `yfinance.Ticker.eps_trend` supplies the input `analyst_revisions.estimate_change_index` documents itself as never having.** That leg needs `len(weights)+1 = 5` levels (`DEFAULT_ESTIMATE_WEIGHTS = (9,7,5,3)`, `analyst_revisions.py:43`) and its own docstring (`:14-18`) says *"the engine has current consensus and price targets only, never a 3-4 quarter estimate history, so this leg returns `unavailable`"*. `revision_index(history, levels=None, ...)` (`:279`) defaults `levels` to `None`, and **the only call site that ever passes a value is a test** - so the leg is structurally `unavailable` in production. `eps_trend` returns `current`/`7daysAgo`/`30daysAgo`/`60daysAgo`/`90daysAgo` - five levels. `eps_revisions` gives the direct up/down counts that `revision_ratio` currently reaches through the `upgrades_downgrades` proxy (`yfinance_sector.py:106`, whose own docstring calls it *"a cheap proxy"*).
- **The finnhub free-tier wall is now enumerated: 12 of 61 probed endpoints work, 48 return 403.** Working: `financials_reported`, `filings`, `fda_calendar` (593 rows), `company_earnings`, `stock_insider_transactions`, `ipo_calendar`, `market_holiday`, `market_status`, `quote`, `symbol_lookup`, `country` (249 rows with `equityRiskPremium`), `stock_lobbying`. Appendix A lists all 48 refused. `exchange` returned HTML, not JSON.
- **Two caveats that decide how the working ones are used.** (1) `fda_calendar` is the **advisory-committee meeting** calendar, not PDUFA action dates - a complement to `event_calendars.py`'s pdufa.bio source, never a replacement. (2) `AGENT_ONBOARDING.md` records `earnings-surprises` as 403; it is, but **`company_earnings` works** and returns `surprise`/`surprisePercent` - different routes, so the recorded note needs a refinement rather than a correction.
- **Rule 15 blocks two of the tempting adoptions.** `statement_parsing.trailing_twelve_months` (`:1270`) opens with *"One producer for a trailing-twelve-month total"*, so yfinance's `ttm_income_stmt`/`ttm_cash_flow` cannot be a contributor - cross-check only. Same for finnhub `financials_reported` beside `sec_edgar`.
- **Also worth taking:** finnhub `market_holiday` returns `tradingHour` (`09:30-13:00` on half-days) - the session logic has no holiday source today; and `shares_full` is a share-count **time series**, which is what would have caught the SIMO market-cap defect mechanically rather than by inspection.
- **Plan P0-P5** behind the new `enable_analyst_estimates` (verified absent from the tree), gate off => byte-identical. **P0/P1 is the one worth doing**; P2-P5 are independent.

Tests: none (documentation only). Every code citation was verified at the definition site and the doc's tables validate.

**Web impact**: none - no key, CLI flag, tool, JSON shape or prompt changed.
...
- **Why the source changed rather than the plan.** The EODHD key is live (`/eod` → 200, `/exchange-symbol-list` → 200) but `/bulk-fundamentals/NASDAQ` and `/fundamentals/AAPL.US` both return **403**, and the plan they need — **Extended Fundamentals** — is priced **"By request"** on the vendor's own page and excluded from every published tier (EOD $19.99 / EOD+Intraday $29.99 / Fundamentals $59.99 / All-in-One $99.99). So **spending more at EODHD could not fix it**: the 403 is plan-gating, not spend-more. FMP was the only vendor with a publicly priced bulk tier ($49/mo Premium, $99/mo Ultimate for "Bulk and Batch Delivery") and was not needed — SEC EDGAR XBRL is **free, keyless, and already in the tree** (P0-1).
- **Cost.** One `companyfacts` request per filer, paced to SEC's published **10 requests/second** ceiling. The payload is per-**filer** and date-independent, so it is fetched once and re-read for every date: a 30-date panel over 464 names costs **464 requests, not 13,920**.
- **Two properties the vendor leg would not have had, and neither may be "simplified" away.**
  - **Point-in-time.** Every fact carries the date its annual report was *filed*, and only facts filed on or before the panel date are eligible. Using the newest fiscal year regardless of filing date would let a 2026-09-17 panel read a 10-K filed in October — look-ahead bias, and the one error that would make every measured IC in this layer too good to be true.
  - **One fiscal year for every leg.** The reference year is the newest eligible end among the core statement lines; a tag with no value at that end is **absent**, never substituted from another year. Without it, a filer that stopped tagging inventory in 2013 would contribute a 2013 inventory beside a 2025 balance sheet — a mixed-vintage row that looks measured and is not.
- **Coverage is recorded per name, never dropped silently.** `_meta.fundamentals_gaps` names every name the leg could not supply (no CIK, pre-XBRL, IFRS) with its reason, and `render_text` prints the count. `_meta.vendor_gate` became `_meta.fundamentals_error` — the "gate" it named no longer exists.
- **Market cap is derived, because EDGAR has no price**: the panel's own close × the EDGAR cover-page share count (`dei:EntityCommonStockSharesOutstanding`, read at its own newest eligible period under the same point-in-time rule). Without the join, every price-based ratio — EV/EBITDA, P/E, P/B, earnings yield — would be permanently `NA` for no reason.
- **Three defects in the SEC reader itself, found by running it against live EDGAR.**
  - **20-F filers were invisible.** `_annual_rows` matched `form == "10-K"` only, so a foreign private issuer contributed **0 of 19 tags** — SIMO files nothing but 20-F. The annual-form list now accepts 10-K / 20-F / 40-F (before any `/A` amendment suffix); SIMO goes from 0 to 15 of 15.
  - **Candidate tags were not merged by period end.** "The first tag with any data wins" returned a series that stopped the year the filer changed concept — NVDA's `CostOfGoodsAndServicesSold` ends in 2021 while `CostOfRevenue` carries on, so GP/A was `n/a` while the value sat in the next candidate. Candidates are now merged by fiscal end, earliest winning.
  - **A filer with no us-gaap namespace triggered the per-tag fallback.** TSM reports under IFRS (`ifrs-full`); the payload arrived, the namespace was absent, and the fallback spent 23 requests collecting 404s to find nothing. The fallback is now reserved for the failure it exists for — the payload itself not arriving.
- **The tag vocabulary was widened against live payloads**, not guessed: six balance-sheet rows (current assets / liabilities, inventory, short-term investments, long-term debt and its current portion), `Retained earnings`, `Cost of revenue` and `Liabilities and equity`. Three assemblies were added where EDGAR has no single concept — `total_debt` from the long-term row plus its current portion, `total_liabilities` from the balance-sheet identity when `Liabilities` is not filed (AMZN files only `LiabilitiesAndStockholdersEquity`, and its absence cost Altman Z, Ohlson O, NOA and net-net), and `working_capital` from the two current legs.
- **Live, 2026-09-18, 7 names** (MSFT LULU WDC SIMO TSM NVDA AMZN): **17-25 metrics each**, including the whole valuation block (P/E, P/B, EV/EBIT, earnings yield), Piotroski F, Altman Z, GP/A, accruals, Ohlson O, NOA and Zmijewski. **One named gap (TSM, IFRS).** Beneish M is absent for a recorded reason — EDGAR has no consistent marketable-securities concept, and a partial one would produce a *wrong* number rather than a named gap.

**The forward company-event calendars: one family made measurable, one shown unanswerable, one dropped.**
- **New module `dataflows/event_calendars.py`**, gated by **`enable_event_calendars`** (default **False**, `default_config.py:1088`) — deliberately **separate** from `enable_event_state`, because this is the one part of the event state that leaves the machine. Enabling the engine must not silently acquire a third-party fetch, and with the gate off every family keeps the answer it had before the adapters existed.
- **FDA / clinical → pdufa.bio** (`/api/v1/events`, free and keyless, 1,000 req/day anonymous). This family is now **`SCORABLE`**: `FAMILY_AVAILABILITY["product_clinical"]` moved off `ABSENT`, and its `ABSENT_REASONS` entry was deleted rather than left as a dead claim.
- **The announced-day rule is load-bearing.** pdufa.bio changed `date` on 2026-09-09 to mean *an announced day, or `null`*: for `month`/`quarter`/`year` precision the field is null and only `date_month` holds the granularity, because the site used to serve a month midpoint as if a sponsor had announced it (~7 rows in 10). **Only `date_precision == "day"` rows become forward events.** A month midpoint turned into a day-count would be exactly the fabrication this set refuses, and it would land in a scored imminence. Live 2026-09-18: **456 catalysts tracked, 120 carrying an announced day** — so `calendar_coverage` prints `rows_held` beside `rows_with_announced_day`, and a family that measures nothing says *"of 6 rows, 5 undated"* instead of reading as an absence of catalysts.
- **Court → CourtListener cannot answer, and the code says so.** It is a filing archive, not a forward calendar: `/dockets/` and `/docket-entries/` require a token even for `OPTIONS`, and the anonymously accessible `/search/?type=r` exposes exactly three date fields — `dateArgued`, `dateFiled`, `dateTerminated` — all backward-looking. Probed live across three result sets (144,748 / 4,563 / 61,937,018 matches): **zero rows carried a future date.** The adapter returns `None`, which makes the family **`missing`** — **not** `not_applicable`, because "this source cannot carry a forward date" is not the claim "no court event is scheduled", and only the second one would be a false assertion. `event_calendars.court_probe` keeps the finding re-verifiable as a callable rather than as prose that could quietly go stale.
- **Investor day → DROPPED by decision.** No free source exists; Wall Street Horizon via Interactive Brokers ($49/mo retail) was the only priced source and was declined; EODHD's Corporate Events Calendar ($19.99/mo) does not carry investor days at all — its own page scopes it to earnings, IPOs, splits, dividends and news. The design permits dropping it: the families are independent and coverage prints per family.
- **`None` is never `[]`.** `None` means the question was not answered (`missing`); `[]` means it was answered and holds nothing (`not_applicable`). `event_state.calendar_answers` is the one place that turns adapter rows into the three-status answer, so the leaf and the run card cannot drift into collapsing them — the single mistake that would turn "this source cannot answer" into "no event is scheduled".
- **Live:** `RARE` 2026-09-18 reads `product_clinical` **`available`** with a PDUFA the next day (`imminence 0.989`, family coverage 2 of 7); `MRNA` reads `not_applicable` because its only tracked row is month-precision.

**Two defects found while building this, both fixed with a failing-first regression test.**
- **A scorable family asked with no rows crashed.** `forward_calendar` reached for `ABSENT_REASONS`, which no longer carries `product_clinical` once it gained a producer — so `forward_calendar("product")` raised `KeyError`. It now falls back to the family's `NA_REASONS` text, which is the honest answer for a family that *has* a producer but was not asked.
- **A knife-edge test fixture.** `test_reflection_hint_states` recorded one win and one loss and asserted the hint at a `score >= 0.5` boundary; the two decay weights differ in their last bits because the entries are recorded microseconds apart, so the ratio lands a floating-point ULP below 0.5 in **~1 run in 200** (measured: 1 of 200) and the hint flips to `critique`. The producer is correct — a 50% hit rate *is* 0.5, and `>= baseline` is the declared contract — so the fixture was moved off the boundary. This is a **test-assumption defect**, the owner's fourth category.

Tests: `tests/test_event_calendars.py` new (12 + 1 opt-in live probe, skipped by default behind `TRADINGAGENTS_LIVE_NETWORK_TESTS`); `tests/test_fundamentals_enhancements.py` gains 4 SEC-reader cases (candidate merge by period end, the 20-F form list, the cover-page share count, and the no-us-gaap fallback guard); `tests/test_score_panel.py` reworked around the new transport contract and gains the point-in-time, one-fiscal-year, market-cap-join and named-gap cases; `tests/test_event_state.py` gains the `None`-vs-`[]` case.

**Web impact**: none — `scripts/score_panel.py` is a manual script, the calendars are off by default, and no tool name, CLI flag or existing JSON shape changed. `run_card.json`'s `event_state` block already existed and is populated only when its gates are on.
...
- **The reporting defect is separate, and it is real.** `catalyst_window=False` was a **false assertion**, not an `NA`. The corrected contract: the pre-graph context prints the value actually available pre-graph, or `unavailable_pre_graph` when there is none; the post-graph `strategy_overlays.catalyst` stays an output/observation.
- **Fixed — `strategies/regime.py::regime_gate_read`.** `catalyst_window` is now **tri-state**, `bool | None = None`. `None` means the caller supplied no event fact: the axis is reported **unmeasured**, the gate neither vetoes nor claims "no catalyst", and the value is never coerced to `False`. The trailer becomes *"volatility contained + no fast downtrend (catalyst window not measured)"*.
- **Fixed — `graph/trading_graph.py::_compiled_decision_context`.** It stops reading `strategy_overlays` altogether, supplies no event fact, and prints `catalyst_window=unavailable_pre_graph`.
- **Fixed — `agents/utils/value_dip_tools.py::get_value_dip_setup`.** The same defect one layer down: it passed an explicit `catalyst_window=False` it never measured. It now supplies none and prints `catalyst_window=unavailable`.
- **The veto is not reinstated anywhere**, and the axis is deliberately not measured on the pre-graph path — that is what keeps the 2026-09-17 `RegimeScore → EventScore` boundary intact.
- **Fails before, passes after.** Against the pre-fix code (reconstructed by stashing the two source files) both new tests in `tests/test_strategies_regime.py` fail with the exact false string from the corpus: `catalyst_window=False reasons=volatility contained + no fast downtrend + no catalyst`.
- **The ground-truth parser is unaffected:** `structured_debate._KEY_VALUE_RE` requires a **numeric** value, so `unavailable_pre_graph` registers no key — exactly as `False` did not.

**Also closed on sight in the same pass.** `sec_edgar._REFERENCE_TAGS` was declared with a docstring pointing at `annual_facts` and read by **nothing** — the reference-year rule lives with the canonical assembly at `scripts/score_panel.py::SEC_REFERENCE_LABELS`; removed, with a note recording where the rule actually lives. `statement_parsing.annual_series`'s docstring described only the vendor stacking path and now names its SEC XBRL sibling `sec_annual_series`, the per-key longest-series merge rule (never a splice), and why a non-USD filer skips the SEC leg.

Tests: `tests/test_strategies_regime.py` +2 (both fail before the fix).

**Web impact**: none — no tool name, CLI flag or JSON shape changed. The `computed_decision_context` string changes (`catalyst_window=unavailable_pre_graph`), which is the point of the fix; its consumers are the ten prompt sites, the L1 registry and report section `IVa`, and none of them parses that token.
