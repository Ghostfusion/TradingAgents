...
**Web impact**: none - no key, CLI flag or JSON shape changed; the `2_research` markdown gains headings, which any markdown renderer already handles (the app only names the folder, in `HelpGuide.jsx`).
...
Tests: `test_report_verify.py` 180 passed (+1: the word-boundary regression, which fails against the previous regex); full suite 4245 passed, 5 skipped (the same five as before).
