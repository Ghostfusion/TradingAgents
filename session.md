...
- **Earnings quality wired to consensus verdict** (user option 1):
  ...
  negative-FCF red flag) + keeps the forensic trap; old 6%/2% accrual band
  (backwards label) removed; verdict capex is sign-robust (abs). Live AAPL
  smoke: MEDIUM concern, cc 0.995, trap LOW. Tests 3 updated + 3 new; wiring
  gate green; ruff clean.
- **Quant-engine v2 audited** (per user): fixed the DuPont driver metric (was argmax|factor| — now log-DuPont attribution vs neutral 1.0; margin/turnover/leverage/mixed labels), scenario DCF g_base=0 coercion + added market-price band/mos, earnings-quality n/a-on-no-inputs + concern wording; wired the trio into the fundamentals analyst (graph ToolNode already had them). Tests 19 + 7 renders; suite green (minus pre-existing broken test_strategies_calibration.py); ruff clean.
