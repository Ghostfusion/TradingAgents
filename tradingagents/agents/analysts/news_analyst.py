...
def create_news_analyst(llm, backup_llm=None, config=None):
    def news_analyst_node(state):
        ...
        system_message = (
            ...
            " TOOL-STATED CAVEAT: when a tool prints a basis or caveat beside a"
            " figure, carry it with the figure. get_earnings_calendar's own"
            " header says surprise_pct comes from unrounded EPS, so it need not"
            " equal (reported-estimate)/estimate on the 2dp values shown -"
            " quoting the three numbers bare invites a fact-check to read them"
            " as inconsistent (AMZN 2026-09-14: 215.02 vs 214.21 recomputed from"
            " the displayed 1.83/5.75). Same for any vendor ratio quoted beside"
            " its rounded inputs."
