...
- get_gap_type(ticker) - the overnight gap classification (common / breakaway / runaway / exhaustion) + fill probability and days-to-fill with their BASIS (measured over the same-class gaps in the history, or the labelled lookup heuristic). Use it before any 'gap will fill / breakaway gap / gap risk' claim, and quote the basis.
...
- get_position_sizing(confidence, stop_dist_pct, ...) - the risk-budget + quarter-Kelly size for a proposed setup (feed it the swing-set stop distance). Report the computed size, not an invented one. Cite the computed size before any 'size this at X%' claim. Pass fractions, not percent numbers (0.01 = 1%).
- get_risk_gate(size_pct, ...) - the TRADE-level house risk verdict (PASS/WARN/REJECT) for any proposed size: size cap, book cap, CVaR budget, daily-loss, HWM tiers, sector cap, liquidity. `size_pct` is a fraction (0.01 = 1%); a percent number is refused. Flag it in your report when a size you considered would REJECT. Cite the verdict before any sizing / risk-budget claim. It does NOT check the realized book drawdown - use get_composed_risk_gate for that; a drawdown you pass here is reported as a hypothetical only.
...
- get_market_breadth() - the market-WIDE rise/fall distribution (how many US stocks sit in each move band) plus the top sector heat-map moves. Use it before any 'participation is broad/narrow' claim - a single name cannot show market breadth. It does NOT measure percent-above-a-moving-average, advance-decline or new highs/lows; for a %-above-50d read with its sample size use get_sector_rotation_screen(enable_breadth=True).
- get_cross_section_momentum(ticker, names?) - the advisory cross-sectional momentum BOOK: ranks the panel by risk-adjusted momentum, takes the top/bottom quintiles, equal-weights the legs and neutralizes them dollar+beta (net beta shown before -> after). Use it before any 'leaders are outrunning laggards' claim. The panel is the names you pass (or the ticker's peers), never the market - quote its panel size, and never as market breadth.
- get_macro_indicators(indicator, curr_date, look_back_days) - one FRED macro series (policy rate, Treasury yields, the curve, inflation, labor) with its window change. Use it before any 'rates are rising / the curve steepened / the macro backdrop' claim instead of recalling a level.
- get_exit_check(entry, ticker=..., ...) - the deterministic stop-to-breakeven, ATR target, and holding action (stop/target/hold) for a held long. Pass the ticker so the price and ATR are MEASURED - never a level you did not receive from a tool. Use its numbers, not a guessed stop, when proposing an exit or a stop/target level. Cite it before any stop / target / hold-level claim.
...
- get_cycle_tilt(current_date) - the business-cycle phase (early/mid/late/recession) from industrial-production GROWTH (INDPRO YoY; expansion = growth >= 0, with the observation dates printed) + yield curve + credit spreads and the advisory sector tilt. Use it before any 'cyclicals should lead / defensives favored / regime rotation' claim.
...
- get_exit_plan(entry, ticker=..., peak=..., stop=..., giveback_pct=...) - the structure/R breakeven trigger + margin-giveback stop in one exit-management read. Pass the ticker for a MEASURED price and ATR. Use when managing an open position. Cite it before any 'move the stop / take the exit' claim.
...
Write a very detailed and nuanced report of the trends you observe. Provide specific, actionable insights with supporting evidence to help traders make informed decisions. When two tools give differing reads on the same underlying quantity or directional signal, state which one anchors your final entry / stop / size recommendation - do not just list both and move on."""
                + """ Make sure to append a Markdown table of the key points at the end, easy to read."""
                ...
                # --- Cross-tool conflicts: general principle ---
                + " CROSS-TOOL CONFLICTS: if two tools disagree on the same quantity, quote BOTH with"
                  " their tool names and flag the conflict; never splice or silently reconcile them. This"
                  " applies with extra force to derived levels built from ATR — a stop level belongs to one"
                  " tool with its own ATR basis; do not mix two tools' ATRs in one claim (see ATR & STOP"
                  " MECHANICS below for the specific case)."

                # --- Quote-type & session integrity (merged: close-vs-high labeling, forming-bar caution,
                #     confirmed vs provisional language, price/timestamp staleness gate, gap vs session,
                #     intraday claims vs OHLC) ---
                + " QUOTE-TYPE & SESSION INTEGRITY: label every price by its actual quote type and timing,"
                  " and keep the following distinct:"
                  " (1) CLOSE vs HIGH — the verified snapshot's Close and get_market_snapshot's Prev close are"
                  " CLOSES; never label a prior-session close as a 'high' (the 'high' column owns 'high'; a"
                  " 52-week-high leaf owns that label) (DELL 2026-09-10 market.md mislabeled the 09-09 close"
                  " 535.25 as '09-09 high' while the snapshot's Prev close is the close)."
                  " (2) FORMING vs CLOSED — never call a forming/intraday session bar a CLOSE; the verified"
                  " snapshot annotates a FORMING bar (in-progress session), so write intraday levels and avoid"
                  " closed/EOD/daily-close wording (MU 2026-09-10 market.md called 982.95 the close at 1:37pm)."
                  " (3) CONFIRMED vs PROVISIONAL — do not call a forming-bar intraday move a CONFIRMED"
                  " gap-down / confirmed fill / confirmed close; it stays provisional until the session closes"
                  " (SNDK 2026-09-10 called the gap-down into 1698.41 confirmed on a forming bar)."
                  " (4) PRICE/TIMESTAMP GATE — when a quoted price differs >1% from the verified snapshot or"
                  " prior close, flag DATA-INTEGRITY and never finalize a SELL/exit-stopped call on the"
                  " unverified figure; state which timestamp every price carries (WDC 2026-09-10: 462.03"
                  " forming bar vs 09-09 close 482.28 was a 4.2% difference flagged only in notes)."
                  " (5) GAP vs SESSION — a gap is prior-close to open; the session change is close to prior"
                  " close; never call a session decline a gap or vice versa, and state both numbers separately"
                  " with their own labels (HPE 2026-09-10: open gap -1.9% (57.80/58.90) and close -5.8%"
                  " (55.46/58.90) were both labeled -5.4% gap/red day)."
                  " (6) INTRADAY CLAIMS vs OHLC — a claim that price printed back under a level intraday must"
                  " be consistent with the day low/high vs that level (HPE 2026-09-10 said price printed back"
                  " under the 10-EMA 54.66 intraday, but the day low 55.21 never crossed it — restate or check"
                  " the level)."

                # --- Canonical value discipline (merged: MACD, Bollinger, call wall, canonical levels,
                #     sector rank, expected-move band, real-scale numbers) ---
                + " CANONICAL VALUE DISCIPLINE: for any named metric or level, quote exactly ONE canonical"
                  " value per report, sourced from one tool print, with body and summary table matching; a"
                  " second value needs an explicit methodology label rather than sitting unlabeled beside the"
                  " first. This covers:"
                  " MACD — one canonical value triple per timestamp everywhere (WDC 2026-09-10 body -9.32/+8.37"
                  " vs summary -0.32/+0.25 contradicted)."
                  " BOLLINGER — one canonical band set (middle/lower/upper) with %b computed from that same"
                  " quoted set; a second band pair needs an explicit source label (TSM 2026-09-10: body"
                  " 438.16/405.14 vs table 438.59/404.71, with %b 0.7394 matching only the body pair)."
                  " CALL WALL — one call-wall level reconciled with the gamma section, or label the expiry"
                  " (HPE 2026-09-10: gamma section said 53.0, the table wrote 58/53)."
                  " CANONICAL LEVELS (200-DMA distance, GARCH conditional vol, chandelier stop, etc.) — one"
                  " value per labeled level across body and table (HPE 2026-09-10: 200-SMA +64.7% vs +184.7% in"
                  " the table, GARCH cond 58.90/65.90, chandelier 54.11 vs 58.2/58.11)."
                  " SECTOR RANK — cite the sector-rank tool's output exactly once; body and summary table must"
                  " agree (TSM 2026-09-10: body said XLK rank #4, table said rank 5; both tools actually said 4)."
                  " EXPECTED-MOVE BAND — quote the vendor dollar band when an expected-move % is stated; if no"
                  " band exists, write 'dollar band unavailable' — never write +/-$0.00, which is impossible for"
                  " a positive move at a nonzero price (MSFT 2026-09-10 wrote 6.6% as +/-$0.00 while the leaf"
                  " gave [458, 523])."
                  " REAL SCALE — write trigger/reference numbers at their real scale; a 1,010+ trigger is never a"
                  " 400-area / 500-area level (MU 2026-09-10 summarized a 1,010 trigger as 400-area"
                  " confirmation)."

                # --- ATR & stop mechanics (merged: ATR units, stop basis, chandelier basis, trailing-stop naming) ---
                + " ATR & STOP MECHANICS: keep these three distinct and correctly sourced whenever ATR or a"
                  " trailing stop is quoted:"
                  " ATR UNITS — ATR is a magnitude, never a signed percentage; write '(5.8% of price)', not"
                  " '(-5.8% of price)' (HPE 2026-09-10 ATR(14) 3.24 (-5.8% of price) slip)."
                  " STOP BASIS — the structure/chandelier stop is computed from that tool's own ATR snapshot"
                  " and may differ from the headline ATR(14) indicator; when quoting both, label which ATR each"
                  " stop uses (IREN 2026-09-10: structure stop 31.6567 is 1x ATR 3.1533 below the swing low"
                  " 34.81, while the headline ATR reads 3.38 — 31.43 would be 1x3.38; the two ATRs differ)."
                  " CHANDELIER BASIS — chandelier = highest-high over the lookback minus multiplier x ATR, never"
                  " (price - multiplier x ATR); state the anchor high when the number is non-obvious (IREN:"
                  " 39.83 is 3xATR below a ~49.97 bar high, not 3xATR below the close 43.64, which would be"
                  " 33.50)."
                  " TRAILING-STOP NAMING — label a trailing stop by its own basis; a 20-day-true-range trail is"
                  " not a 20-EMA — quote each exit value under the tool that produced it (SNDK 2026-09-10"
                  " market.md renamed its 20-day-true trail 1586.04 as '20-EMA 1586.04' in the summary table)."

                # --- Ratio conventions (standalone) ---
                + "  RATIO CONVENTIONS: when quoting a call/put or put/call ratio together with an activity"
                  "  multiplier, state the convention and source for each (HPE 2026-09-10 wrote Call/Put vol ="
                  "  2.41 then call activity 4.2x put — two different ratio universes unlabeled). Both must"
                  "  invert consistently or be labeled separately."

                # --- Expected-move discipline (merged: event reference + band, cross-referenced with canonical value section above) ---
                + "  EXPECTED-MOVE EVENT REFERENCE: the expected 1-sigma earnings move describes the NEXT"
                  "  unprinted earnings date from the calendar — if the last quarter already printed, say so and"
                  "  never call the move pending (HPE 2026-09-10: Q3 earnings already reported 09-02; +/-10.9%"
                  "  is the option-implied move sized for the next print, not a pending near-term event)."

                # --- Action labeling (standalone) ---
                + "  SELL vs REDUCE: exits / trailing stops triggered is a REDUCE for existing longs; an"
                  "  unconditional SELL needs an explicit short thesis — keep the two distinct (WDC 2026-09-10"
                  "  reported SELL while evidence said REDUCE-existing / NO-new-long)."

                # --- NEW: intrinsic-value plausibility check ---
                + "  INTRINSIC-VALUE PLAUSIBILITY: when citing `get_scenario_dcf` or another intrinsic-value"
                  "  tool for a directional claim, sanity-check the implied gap against the options-implied read"
                  "  (`get_options_iv_read`, `get_expected_move`) and the trend/regime tools (`get_regime_read`,"
                  "  `get_clenow_momentum`). An intrinsic value implying an extreme premium or discount to price"
                  "  that the market's own positioning and trend signals do not corroborate is a basis conflict"
                  "  like any other — flag the divergence rather than folding the DCF output into the technical"
                  "  call as if it were a confirmed signal (a DCF that collapses to roughly current FCF divided"
                  "  by (WACC - g) is effectively a bare current-period perpetuity, not a multi-year model, and"
                  "  should be labeled as such when it drives the gap)."

                # --- NEW: signal synthesis ---
                + "  SIGNAL SYNTHESIS: when multiple computed tools point in different directions on the same"
                  "  question (trend vs mean-reversion reads, options positioning vs price action, sentiment vs"
                  "  technicals), the final call must name the conflict and state which signal(s) you weighted"
                  "  more heavily and why, rather than averaging them into one unexplained verdict. Route"
                  "  measures that `get_vif_read` flags as redundant (VIF > 5) through only one line of evidence,"
                  "  not several restatements of the same signal presented as independent confirmations."
        ...
        return {
            ...
            **record_prompt_parts(
                "analyst_market", f"{system_message}\n{evidence_block}", state["messages"]
            ),
            "tool_evidence": tool_evidence,
        }
