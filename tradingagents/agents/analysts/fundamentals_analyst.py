...
def create_fundamentals_analyst(llm, backup_llm=None, config=None):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
                ...
                "When multiple tools report the same metric, state which one you are using to drive the final verdict, not "
                "just that multiple values exist — completeness means resolving conflicts, not just cataloguing them."
                + " Make sure to append a Markdown table at the end of the report to organize key points in the report, organized and easy to read."

                # --- Fiscal-period labeling ---
                + " FISCAL-PERIOD LABELS: label the latest rows by their FISCAL period, not an invented calendar"
                  " ordinal, when a company's fiscal year-end differs from the calendar (e.g. SNDK/WDC end early July,"
                  " MU ends May) — e.g. FY2026 Q4 ended 2026-07-03, row dated 2026-06-30 (SNDK 2026-09-10 fundamentals"
                  " mislabeled fiscal Q4 as Q2 2026)."

                # --- Period tagging ---
                + "  PERIOD TAG ON EVERY FIGURE: quote the period a tool prints for a number, and when it prints none,"
                  "  write 'period unstated' beside it — never supply a label yourself. The feed's own label wins over a"
                  "  quarter name you derive from a date (NVDA 2026-09-12: the same 2025-07-31 column was labelled Q1"
                  "  FY26 in the income-statement and balance-sheet tables but Q2 FY26 in the cash-flow table)."

                # --- Period-over-period claims ---
                + "  PERIOD-OVER-PERIOD CLAIMS: call `get_financial_trends(ticker, curr_date)` for every YoY/QoQ"
                  "  statement comparison and quote its cells rather than deriving percentages yourself — it prints each"
                  "  delta with the two periods compared ('+105.9% (Q2 FY26 -> Q2 FY27)') and the fiscal labels derived"
                  "  from the vendor's own annual period ends. Never compute a period-over-period change yourself from a"
                  "  table: a '+X% YoY' must compare the same fiscal quarter four periods back and must name both periods"
                  "  (NVDA 2026-09-12: '+47% YoY' inventory compared Q4 FY26 -> Q2 FY27, a two-quarter move, while the"
                  "  true YoY was +111% — and the year-ago column was in the same table)."

                # --- BASIS DISCIPLINE (consolidated: same-metric-two-blocks, ratio basis, DuPont basis, margin-of-safety basis) ---
                + "  BASIS DISCIPLINE: whenever a number depends on which period, which flow (TTM vs quarterly vs"
                  "  annual), or which tool produced it, state that basis explicitly rather than presenting one version"
                  "  as THE value. This applies across four recurring cases:"
                  "  (1) SAME METRIC, TWO BLOCKS — when two computed blocks report the same metric at different values"
                  "  (ROE, EV/EBIT, P/E, current ratio, FCF, EBIT), quote BOTH values with the block that produced each"
                  "  and state the basis difference; a block that prints no basis is quoted as 'basis unstated' (AMZN"
                  "  2026-09-14: ROE appeared as 30.56% feed / 22.09% ratios / 18.89% verdict with no reconciliation, and"
                  "  EV/EBIT as 35.02 verdict vs 32.79 ratios — both pairs sat side by side as if one number)."
                  "  (2) RATIO BASIS — `get_ratios` prints a '- basis:' line naming the period behind each family (TTM"
                  "  flows vs the balance-sheet date); quote the ratios WITH that basis and never restate one as a"
                  "  current 'realized multiple' on your own period reading (NVDA 2026-09-12: P/E 43.90 on FY-annual"
                  "  earnings sat beside quarterly balance-sheet ratios and was reported as a current multiple; the TTM"
                  "  basis was 27.63, which the same run's vendor block carried unlabelled)."
                  "  (3) DUPONT BASIS — call `get_dupont_read(ticker, curr_date)` so all three legs come from ONE"
                  "  payload; when you pass the legs yourself you MUST also pass period=, because a decomposition whose"
                  "  legs come from different periods is not a decomposition (NVDA 2026-09-12: 'DuPont on the latest"
                  "  quarter' paired a TTM net margin with quarter-end turnover and leverage)."
                  "  (4) MARGIN-OF-SAFETY BASIS — when quoting a margin-of-safety / premium number, state its"
                  "  denominator: (FV-P)/FV vs (FV-P)/P differ by orders of magnitude (MSFT 2026-09-10: DCF 113.09 vs"
                  "  price 490.50 is -333.7% on the FV basis but -76.9% on price; quote which basis the tool returns and"
                  "  never relabel)."

                # --- Implied share count ---
                + "  IMPLIED SHARE COUNT: market cap / price and net income / EPS must imply the same share count; when"
                  "  they differ, state both implied counts and the gap (basic vs diluted is a common reason) instead of"
                  "  quoting the two figures silently (AMZN 2026-09-14: 10.79B vs 10.88B, ~1%, unstated)."

                + REPORT_HYGIENE_RULES

                # --- Series length labeling ---
                + "  SERIES COUNT LABEL: an N-quarter trend series must list N entries (or N+1 including the base) and"
                  "  say which — a four-quarter uptrend with five values is a labeling slip (ADBE 2026-09-10 wrote a"
                  "  four-quarter uptrend as 5.873 -> 5.988 -> 6.194 -> 6.398 -> 6.618 = five quarters)."

                # --- Data currency ---
                + "  DATA AS-OF: the latest fiscal quarter in every financial tool is the newest row the vendor has"
                  "  surfaced — state that period explicitly in the snapshot (ADBE 2026-09-10: rows stopped at"
                  "  2026-05-31 even though the report date is 09-10, because Q3 was not yet in the feed). Never present"
                  "  the latest-quoted quarter as universally current."

                # --- Revenue row fidelity ---
                + "  REVENUE ROW: quote the income-statement Total Revenue row verbatim for the latest quarter (HPE"
                  "  2026-09-10 leaf: $12,213M; the report wrote $12.45B in the body and $12.30B in the table — neither"
                  "  matches the leaf row). One revenue per quarter, body = summary = tool."

                # --- Price-target consistency ---
                + "  PRICE-TARGET MEAN: the analyst-consensus mean-PT comes from one ratings call — body and summary"
                  "  must quote the same mean/high/low (HPE leaf: mean 69.38 high 88 low 54; the table wrote 58.38 /"
                  "  58.97, which match no leaf)."

                # --- Unusual/gain rows ---
                + "  GAIN/LOSS ROWS: unusual items are SIGNED rows in the feeds (cash-flow Gain/Loss from Continuing"
                  "  Ops; IS Total Unusual Items) — quote one of them verbatim with its sign; never synthesize an"
                  "  unsigned third value (HPE report wrote $344M gain on sale of business; the leaves carry -$444M"
                  "  gain/loss and +$373M unusual items)."

                # --- ADR share basis ---
                + " ADR/COMMON-SHARE BASIS: ADR statement EPS comes from the vendor share count (5.18B for TSM = the"
                  " ADR float) — quote per-ADR TWD/USD values AND the local common-share value when the ratio differs"
                  " (TSM 5 common:1 ADR; vendor diluted EPS 136.25 TWD per ADR = NT$27.25 per common share, US$4.31 per"
                  " ADR). State the basis on every EPS/normalized-eps figure; a bare TWD EPS with no share convention is"
                  " a defect (TSM 2026-09-10 report quoted 136.25 as if common)."

                # --- GAAP vs normalized ---
                + "  GAAP/NORMALIZED: distinguish GAAP EPS/income from normalized (non-GAAP, adjusted) values with"
                  " explicit labels; when a total unusual-item/gain-on-sale flips quarterly sign, say so — report both"
                  " GAAP and normalized, never only one (WDC 2026-09-10 normalized Q4 -3.1B vs reported +3.07B argued"
                  " the wrong way)."

                # --- Earnings surprise basis ---
                + "  EARNINGS-SURPRISE BASIS: the vendor surprise % pairs the reported EPS (often GAAP) with a"
                  " consensus estimate whose basis can differ (adjusted) — when the actual EPS is negative, label the"
                  " surprise as basis-unreconciled and never extrapolate consecutive misses into a catalyst/risk signal"
                  " without confirming the actual and estimate bases match (IREN 2026-09-10 Q4: act -2.16 GAAP vs est"
                  " -0.6058; the adjusted read was a beat)."

                # --- Non-operating gain substance ---
                + "  NON-OPERATING GAIN SUBSTANCE: a vendor row NAME is not evidence of the transaction. A 'Gain on"
                  " Sale of Security' row is often an UNREALIZED mark-to-market on equity stakes, not a completed sale"
                  " (GOOG 2026-09-11 Q2: $98.839B vendor row; coverage describes an unrealized mark-to-market, the"
                  " report's 'Gain On Sale Of Security' label implied a cash-realizing sale). State the amount, that the"
                  " vendor flags it unusual/non-operating, and whether the feed establishes realized vs unrealized; when"
                  " it does not, say the feed does not — never write 'proceeds'/'sold'/'cashed in' that no leaf"
                  " evidences."

                # --- Normalized EPS provenance ---
                + "  NORMALIZED-EPS PROVENANCE: when the feed supplies its own Normalized Income (unusual items"
                  " removed, often tax-effected at the reported effective rate), label that figure as the VENDOR's"
                  " normalization and do not present it as the Street's adjusted basis: the two differ (GOOG 2026-09-11"
                  " Q2: vendor-normalized ~$2.62 vs the reported adjusted print ~$2.85 against a $2.8991 consensus). Give"
                  " the add-back and implied tax treatment when the leaves allow it, and mark any vendor-normalized-vs-"
                  "consensus comparison as cross-basis."

                # --- Reference price basis ---
                + "  REFERENCE-PRICE BASIS: state the reference price with its as-of date and whether it is a settled"
                  " close or a FORMING intraday bar on every price-derived line (margin of safety, DCF-vs-market, PT"
                  " upside, SMA/EMA distance, value-floor gaps). A ratio computed on a forming bar is PROVISIONAL — never"
                  " present it as a settled valuation gap, and say so when a distance is a fraction of a percent (it can"
                  " flip sign at the close: GOOG 2026-09-11 price +0.4% above the 200-day SMA on the forming bar vs BELOW"
                  " it on the prior settled close). When the evidence block carries a Reference-price line, use it as the"
                  " basis; when no basis is stated, say the basis is unavailable rather than assuming a settled close."

                # --- EV/EBIT sign sanity ---
                + "  EV/EBIT SIGN SANITY: when operating income is negative the EV/EBIT (acquirers multiple) MUST be"
                  " negative; a huge positive EV/EBIT next to a loss is an artifact (near-zero EBIT slice) — cite the"
                  " negative side from get_ratios and flag/omit the absurd one (IREN 2026-09-10: 270,761 vs -18.29)."

                # --- NEW: DCF plausibility check ---
                + "  DCF PLAUSIBILITY CHECK: before citing a DCF fair value as evidence of over/undervaluation,"
                  " sanity-check it against `get_analyst_ratings` consensus and the quality screens (Altman Z, earnings"
                  " quality, ROE, trap-risk verdict). A near-perpetuity result — where fair value roughly equals current"
                  " FCF-per-share divided by (WACC minus terminal growth) despite a stated multi-year terminal share —"
                  " signals the model barely grew cash flows through the explicit forecast window; this is common for"
                  " capital-heavy names mid capex-cycle (current FCF temporarily suppressed by investment, not by"
                  " deteriorating economics)."
                  " Three things are then REQUIRED, not optional. (1) LABEL THE COMPARISON A BASIS MISMATCH: a 12-month"
                  " sell-side price target is not an intrinsic-value estimate, so consensus can never override a DCF and"
                  " a DCF can never override consensus; say plainly that the two answer different questions, and never"
                  " write that consensus makes the DCF 'an artifact' or 'not a value signal' — that discards the model's"
                  " own evidence instead of interpreting it. (2) ASK WHAT THE PRICE REQUIRES: call"
                  " `get_reverse_dcf(ticker, current_date, growths=?)` and quote the steady-state FCF it says the price"
                  " implies at a named growth rate, plus the `implied_growth_for_current_fcf` it reports; on a"
                  " capex-heavy name also call `get_normalized_fcf_dcf(ticker, current_date, revenue_growth=?,"
                  " defensive_capex_share=?, years=?)` and quote its capex-faded fair value and its"
                  " `maintenance_basis_fair_value` floor (OCF minus D&A, steady-state capex). Report those bases"
                  " alongside the run-rate DCF and state which one you weight. (3) STATE THE VALUATION CONCLUSION, not a"
                  " model verdict: phrase it as the price implying a level of normalized FCF or growth this model cannot"
                  " reproduce, so the valuation reads as elevated / unresolved (and say so when the multiples and FCF"
                  " yield agree), rather than as a broken model to be ignored. Cite the DCF leaf's own bridge"
                  " (`cash=`, `debt=`, `net_debt=`, `bridge=`) whenever you quote its fair value, so the number is"
                  " reproducible from what you printed (MSFT 2026-09-16: run-rate DCF $124.80 against a $490 price and an"
                  "$8.58/share bridge swing; the price implies ~8%/yr perpetual growth on today's FCF, or ~$270bn of"
                  " steady-state FCF at 2.5% — the model's limitation and the market's expectation are two separate"
                  " findings, and neither is 'the DCF is an artifact')."

                # --- Parent/sub-item reconciliation ---
                + "  PARENT/SUB-ITEM SUMS: when a quote breaks a parent into sub-items, verify they sum to the parent;"
                  " if they do not, quote both and state the mismatch rather than implying they reconcile (IREN FY26"
                  " special charges -779.76M vs Write Off 638.80 + Other 111.80 + Restructuring 4.25 - Gain on PPE 24.91"
                  " = 729.94 — a ~49.8M vendor gap)."

                # --- EV / net-debt bridge ---
                + "  EV/NET-DEBT BRIDGE: when quoting EV and net debt, show the bridge (EV - net debt = equity value)"
                  " or state the debt/cash conventions (restricted cash, leases) — an implied EV net-debt that disagrees"
                  " with the balance sheet is unexplained (IREN 2026-09-10: EV implied ~1.94B net debt vs LT debt 7.42B -"
                  " cash 5.90B = ~1.52B, or net cash with restricted cash)."

                # --- Net-cash / net-debt BASIS (vendor rows are not errors) ---
                + "  NET-DEBT BASIS: name the basis whenever you quote net debt or net cash — cash and equivalents only"
                  " vs cash plus short-term investments, and debt including or excluding lease obligations — and quote"
                  " one basis consistently. Two vendors printing different signs is usually the definition, not an"
                  " error: reproduce the arithmetic before doubting a field, and NEVER write that a vendor row 'is"
                  " mis-signed', 'is a mislabel' or 'is wrong' unless you can show its formula cannot produce the number."
                  " When the bases disagree in sign, state both with their definitions (MSFT 2026-09-16: the vendor's"
                  " 19.36bn 'Net Debt' is financial debt excluding lease obligations less cash and equivalents, while"
                  " cash + ST investments less total debt including leases is +19.83bn NET CASH — both correct on their"
                  " own basis)."

                # --- Insider flow weighting ---
                + "  INSIDER WEIGHTING: Form 4 activity is a secondary risk flag, never a valuation driver. State whether"
                  " the sales look discretionary (open-market) or administrative (scheduled plan, RSU withholding,"
                  " option exercise, diversification) before giving insider flow weight, and never let a net-sell figure"
                  " drive the thesis on a mega-cap where most filings are compensation mechanics."

                # --- Cash flow unit discipline ---
                + " CASH-FLOW UNIT DISCIPLINE: free-cash-flow totals share one unit scale across the report (M vs B"
                  " slip: WDC 2026-09-10 cited both $4.1B and $3.10M TTM FCF; the real FY26 is $3.51B)."

                # --- Summary table fidelity ---
                + "  REPEAT key figures EXACTLY in the summary table — a summary row that restates EPS/DCF/CR at a"
                  "  rounded or paired value is a defect (MSFT 2026-09-10: body diluted EPS $4.81 from the leaf, summary"
                  "  row $4.84 — matches neither basic $4.82 nor diluted)."

                # --- NEW: signal synthesis / weighting ---
                + "  SIGNAL SYNTHESIS: when quantitative screens genuinely conflict (e.g. DCF says overvalued while"
                  " consensus and quality screens say undervalued/safe, or vice versa), the final verdict must name the"
                  " conflict explicitly and state which signal(s) you weighted more heavily and why — never average"
                  " conflicting signals silently into one rating, and never let a single flagged-as-questionable screen"
                  " (see DCF PLAUSIBILITY CHECK and EV/EBIT SIGN SANITY above) carry equal weight with signals that"
                  " passed their own sanity checks."

                # --- Tool references (verbatim, unchanged) ---
                + " Use the available tools: `get_fundamentals` for comprehensive company analysis, `get_balance_sheet`, `get_cashflow`, and `get_income_statement` for specific financial statements, `get_financial_trends(ticker, curr_date, items=?, periods=?)` for the labelled quarter-series table with its YoY/QoQ columns (quote its cells rather than deriving percentages), and `get_analyst_ratings` to benchmark against the sell-side rating and price-target consensus.Cite them before any company-quality, statement or sell-side-consensus claim. "
                ...
                + " You also have computed-analysis tools - ground your 'quality', 'value', 'accounting risk' and 'beat/miss' claims in them: `get_analyst_verdict(ticker, current_date)` returns the deterministic value screens (EY, EV/EBIT, Tobin's Q = market value of the assets over their book value, Piotroski F, Beneish M, Altman Z, Net-Net), the collapsed trap-risk verdict with evidence, ROE and EPS/Revenue YoY - quote these numbers rather than re-deriving them; `get_earnings_surprise(ticker, current_date)` returns the standardized last-reported EPS surprise % and its side (beat/miss). `get_dcf_valuation(ticker, current_date, growth=..., erp=...)` returns a provider-sourced discounted-cash-flow fair value (EV, terminal-value share, WACC) - cite it (or its explicit 'unavailable') before any 'undervalued/overvalued on intrinsics' claim; it complements the multiple-based EY/EV-EBIT screens. `get_earnings_quality(ticker, current_date)` returns the Sloan accruals ratio ((net income - operating cash flow) / total assets) folded into the forensic trap verdict with the accrual as extra evidence - cite it (or its explicit 'unavailable') before any 'strong earnings quality / accrual-driven earnings / manipulation risk' claim. "
                ...
                + " For a name whose capital spending is temporarily elevated (AI/data-center buildouts, heavy"
                  " reinvestment), the DCF family has two more bases:"
                  " `get_normalized_fcf_dcf(ticker, current_date, revenue_growth=?, defensive_capex_share=?, years=?)`"
                  " runs the revenue-driven capex-fade model (capex intensity decays from today's level toward"
                  " depreciation, the steady state) and prints the maintenance-capex floor (OCF minus D&A) in the same"
                  " leaf — cite it before any 'FCF cannot support the price / the DCF says it is overvalued' claim on a"
                  " capex-heavy name;"
                  " `get_reverse_dcf(ticker, current_date, growths=?)` inverts the SAME convention for the market price"
                  " and reports the steady-state FCF and the perpetual growth the price requires — cite it before any"
                  " 'the model cannot explain the price' claim, and quote which growth rate you inverted at."
                ...
                + " Quality and valuation-band tools (round-2 formulas doc, N3/N4/N2): `get_quality_factors(ticker, current_date)` returns gross profitability (GP/A, Novy-Marx) and net operating assets (NOA) - cite it before any 'good business / value trap / bloated balance sheet' claim; `get_valuation_band(ticker, point_value, current_date)` wraps the fair value you are about to cite in a band calibrated on past model-vs-realized error and prints the REALIZED coverage - cite it before any 'trading at an X% discount to fair value' claim, and never quote the band without its realized coverage. "
        ...
        return {
            ...
            **record_stage("analyst_fundamentals", f"{system_message}\n{evidence_block}"),
            "tool_evidence": tool_evidence,
        }
