...
def create_fundamentals_analyst(llm, backup_llm=None, config=None):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            + " ETF/FUND TOOLING (fund engine enabled): `get_etf_valuation(ticker)`"
            " is the weighted-constituent valuation - call it before any fund"
            " 'cheap / expensive / no anchor' claim (company-statement tools are"
            " expected unavailable for a wrapper); `get_etf_decline_driver` for"
            " the decline-cause classification; `get_etf_relative_strength` for"
            " relative strength vs S&P 500; `get_etf_risk` for beta / downside-"
            " capture / vol / ATR / max-DD; `get_etf_mechanics` for NAV premium/"
