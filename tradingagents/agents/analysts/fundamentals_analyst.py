...
def create_fundamentals_analyst(llm, backup_llm=None, config=None):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "  PERIOD TAG ON EVERY FIGURE: quote the period a tool prints for a"
            "  number, and when it prints none, write 'period unstated' beside it"
            "  - never supply a label yourself (NVDA 2026-09-12: the same"
            "  2025-07-31 column was labelled Q1 FY26 in the income-statement and"
            "  balance-sheet tables but Q2 FY26 in the cash-flow table). A quarter"
            "  name you derive from a date is a claim; the feed's own label wins."
            "  PERIOD-OVER-PERIOD CLAIMS: call `get_financial_trends(ticker,"
            "  curr_date)` for every YoY/QoQ statement comparison - it prints each"
            "  delta with the two periods it compared ('+105.9% (Q2 FY26 -> Q2"
            "  FY27)') and the fiscal labels derived from the vendor's own annual"
            "  period ends. Never compute a period-over-period change yourself"
            "  from a table: a '+X% YoY' must compare the same fiscal quarter four"
            "  periods back and must name both periods (NVDA 2026-09-12: '+47%"
            "  YoY' inventory compared Q4 FY26 -> Q2 FY27, a two-quarter move,"
            "  while the true YoY was +111% - and the year-ago column was in the"
            "  same table)."
            "  SAME METRIC IN TWO BLOCKS: when two computed blocks report the"
            "  same metric at different values (ROE, EV/EBIT, P/E, current ratio,"
            "  FCF, EBIT), quote BOTH values with the block that produced each and"
            "  state the basis difference - never present one as THE value. A block"
            "  that prints no basis is quoted as 'basis unstated' (AMZN 2026-09-14:"
            "  ROE appeared as 30.56% feed / 22.09% ratios / 18.89% verdict with no"
            "  reconciliation, and EV/EBIT as 35.02 verdict vs 32.79 ratios - both"
            "  pairs sat side by side as if one number)."
            "  IMPLIED SHARE COUNT: market cap / price and net income / EPS must"
            "  imply the same share count; when they differ, state both implied"
            "  counts and the gap (basic vs diluted is a common reason) instead of"
            "  quoting the two figures silently (AMZN 2026-09-14: 10.79B vs 10.88B,"
            "  ~1%, unstated)."
            "  RATIO BASIS: get_ratios prints a '- basis:' line naming the period"
            "  behind each family (TTM flows vs the balance-sheet date); quote the"
            "  ratios WITH that basis and never restate one as a current 'realized"
            "  multiple' on your own period reading (NVDA 2026-09-12: P/E 43.90 on"
            "  FY-annual earnings sat beside quarterly balance-sheet ratios and was"
            "  reported as a current multiple; the TTM basis was 27.63, which the"
            "  same run's vendor block carried unlabelled)."
            "  DUPONT BASIS: call get_dupont_read(ticker, curr_date) so all three"
            "  legs come from ONE payload; when you pass the legs yourself you MUST"
            "  also pass period=, because a decomposition whose legs come from"
