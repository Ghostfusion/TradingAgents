...
def get_intraday(symbols: list) -> "dict | None":
    ...
    for sym, info in data.items():
        ...
        volume = minute.get("v") if minute is not None and minute.get("v") is not None else daily.get("v")
        out[sym] = {
            "price": price, "vwap": vwap, "volume": volume,
...
__all__ = ["get_bars", "get_bars_batch", "get_latest_snapshot",
           "latest_snapshot", "get_intraday", "get_calendar", "get_clock"]