...
def test_composite_scores_ranks_momentum_higher():
    closes = {"AAPL": [100.0 + 0.5 * i for i in range(140)],
              "MSFT": [100.0] * 140}
    scores = vs.composite_scores(
        [{"ticker": "AAPL", "earnings_yield": 0.05},
         {"ticker": "MSFT", "earnings_yield": None}],
        closes,
    )
...
def test_allocation_block_sums_to_one_when_uncapped():
    text = allocation_block({"A": 0.5, "B": 0.3, "C": 0.2},
...
def test_contract_exits_fields_when_enabled():
    ...
    c = build_position_contract(cfg={"enable_exits": True, "atr_mult": 2.0,
                                     "breakeven_atr": 1.0, "target_atr": 4.0},
                                closes=closes)
