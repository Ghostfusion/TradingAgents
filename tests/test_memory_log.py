...
class TestTradingMemoryLogCore:
    ...
    def test_load_entries_cached_until_write(self, tmp_path):
        log = make_log(tmp_path)
        log.store_decision("NVDA", "2026-01-10", DECISION_BUY)
        first = log.load_entries()
        second = log.load_entries()
        # Same mtime -> same cached list object (no re-parse).
        assert first is second
        assert len(first) == 1
        # A write invalidates the cache: a new entry must appear.
        log.store_decision("AAPL", "2026-01-11", DECISION_OVERWEIGHT)
        third = log.load_entries()
        assert third is not first
        assert len(third) == 2
        assert [e["ticker"] for e in third] == ["NVDA", "AAPL"]
