...
from tradingagents.dataflows.date_window import in_window as _in_news_window
...
def test_window_excludes_future_and_undated_in_backtest():
    ...
    assert _in_news_window(inside, start, end) is True
    assert _in_news_window(future, start, end) is False     # look-ahead blocked
    assert _in_news_window(None, start, end) is False        # undated -> excluded in backtest
...
def test_window_keeps_undated_in_live_window():
    ...
    assert _in_news_window(None, now, now) is True
...
def test_upper_bound_is_exclusive():
    ...
    assert _in_news_window(midnight_after, start, end) is False
    assert _in_news_window(last_moment, start, end) is True
...
def test_offset_aware_timestamp_is_converted_not_truncated():
    ...
    assert _in_news_window(aware, start, end) is True
