...
def compute_metrics(entries: list) -> dict:
    ...
    if not entries:
        return {"total": 0, "wins": 0, "win_rate": None, "mean_alpha": None,
                "by_analyst": {}}
    ...
    return {
        ...
        "by_analyst": {
            k: {"total": v["total"], "wins": v["wins"],
                ...
                "mean_alpha": (sum(v["alphas"]) / len(v["alphas"]))
...
def print_report(metrics: dict, ledger_path) -> int:
    ...
    if metrics["total"] == 0:
        ...
        print("\n".join(lines))
        return 0
    lines.append(f"samples: {metrics['total']}  wins: {metrics['wins']}  "
                 f"win_rate: {metrics['win_rate']:.0%}" if metrics['win_rate'] is not None else "")
    ...
    for analyst, b in metrics["by_analyst"].items():
        ...
        lines.append(f"  {analyst}: n={b['total']} wins={b['wins']} "
                     f"win_rate={wr} mean_alpha={ma}")
    lines.append("NOTE: flow tags attach to new ledger entries when "
