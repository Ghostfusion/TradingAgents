...
| Run-file front-end | `pipeline.py` args (verified): `tickers`, `-f/--file`, `-u/--universe`, `--market`, `-n --movers-count`, `--movers-direction`, `--top`, `--limit`, `-d --date`, `--min-mcap`, `--price-min`, `--pe-max`, `--workers`, `--analysts`, `--depth`, `--vendor` — runfile maps 1:1 to these. NOTE (investigated 2026-09-02): design docs say `pipeline.py --alloc` — that is a doc slip; the flag is `scripts/value_screener.py --alloc` (line 1282, `action="store_true"`) → `tradingagents/strategies/portfolio.py` `allocation_block(scores, cfg, sector_map, returns_by_name)` (line 59). `pipeline.py` and `batch.py` have NO alloc machinery. No new CLI flag needed — Phase 1 extends `allocation_block`'s strategy options |
...
- Tools → `analysis_tools.py` (PM toolset): `get_topk_drop_plan(scores_by_name, topk, n_drop)`, `get_enhanced_index_tilt(scores, benchmark_weights, w0=None, turnover_cap=0.2)`; re-export; bind PM. **Correction (2026-09-11):** the PM runs `NO_EXTERNAL_TOOLS`, so no PM toolset exists to bind into; these two are declared in `TOOL_LEGACY_BINDING` and their real consumer is the screener alloc block above. The alloc math also feeds the existing screener seam `scripts/value_screener.py --alloc` → `portfolio.allocation_block` (investigated: that is the real flag the docs call "pipeline.py --alloc") — new strategy options behind the existing flag, gated by `enable_topk_drop`/`enable_enhanced_index` (False); rendered into the allocation-plan block. `portfolio_contract` (FinRL doc §6.3) is a later wrapper — no-op here.
...
- New tools in `agent_utils.__all__` + market/PM prompts + graph ToolNode; `portfolio.allocation_block` alloc-strategy options (behind the existing `value_screener --alloc` flag); `default_config.py` keys + `.env.example`; trading_web allowlist + scripts screen rows; docs set; `ruff check` + full hermetic suite; commit. Suggested commit: `feat: qlib phase 1 pure calculators (factor expressions, signal analysis, tradability, alloc)`.
...
- **Alloc seam is the screener, not pipeline (investigated).** The design docs' `pipeline.py --alloc` misnames the flag: it lives as `scripts/value_screener.py --alloc` (line 1282) → `portfolio.allocation_block` (`portfolio.py:59`). No CLI addition — new Topk-Drop/enhanced-index options slot into `allocation_block` behind `enable_topk_drop`/`enable_enhanced_index` (False), rendering into the existing allocation-plan block.
