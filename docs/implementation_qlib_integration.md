...
- Tools → `analysis_tools.py` (PM toolset): `get_topk_drop_plan(scores_by_name, topk, n_drop)`, `get_enhanced_index_tilt(scores, benchmark_weights, w0=None, turnover_cap=0.2)`; re-export; bind PM. The alloc math also feeds the existing screener seam `scripts/value_screener.py --alloc` → `portfolio.allocation_block` (investigated: that is the real flag the docs call "pipeline.py --alloc") — new strategy options behind the existing flag, gated by `enable_topk_drop`/`enable_enhanced_index` (False); rendered into the allocation-plan block. `portfolio_contract` (FinRL doc §6.3) is a later wrapper — no-op here.
