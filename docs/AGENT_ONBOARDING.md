...
- 2026-09-23 `(working tree)` - **The corpus survey gained its adoption plan, and grounding it found an undeclared dependency.** `docs/implementation_plan_paper_survey_26.md` phases the six themed docs into P0-P4 with a gate, a failing-first test and acceptance criteria per item, and all 44 anchors it reuses were re-verified in the tree. Measured rather than assumed: **sklearn is absent** (X2, X5 and V1 need it or a hand-rolled equivalent), **torch is installed but undeclared and imported nowhere**, and `tsbootstrap` is a real public library. Separately fixed: **numpy and scipy were undeclared** while `strategies/statistical.py:18` imports scipy directly - every scipy edge from a declared dependency is extras-gated, so a clean install resolved none. Both manifests now declare `numpy>=1.26.0` and `scipy>=1.11.2`.
