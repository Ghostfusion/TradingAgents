...
- 2026-09-25 `(working tree)` - **Owner decision: `get_cycle_tilt` reads industrial-production GROWTH; the executor's order guard gains the closing-auction TIF.** The manufacturing leg of the cycle-phase classifier used the PMI diffusion level via FRED's `NAPM`, which is discontinued - the new sector-driver mapping exposed it (0 observations measured today, against 13 for `industrial_production`), so the leg had been reading `None` since the series went away and the classifier silently fell back to the curve/credit legs. New `cycle_tilt.industrial_production_growth(observations)` takes FRED's own `[(date, value)]` shape and returns year-over-year growth WITH the months it compared (INDPRO is published with a lag - the newest observation on 2026-09-25 was `2026-08-01`), and the expansion boundary moved with the input: `MANUFACTURING_EXPANSION_PCT = 0.0`, the analogue of PMI's 50. `cycle_phase`/`cycle_tilt`/`macro_stance`'s first argument is now `manufacturing_growth_pct` (clean rename, every caller and test migrated, no shim), `get_cycle_tilt` fetches the series itself over a 900-day window so the pipeline stays automated on free FRED data, and the market analyst's prompt names the new input and boundary. The discontinued `pmi`/`manufacturing_pmi` aliases stay in `fred.py` with a DISCONTINUED marker - removing an owner-facing name is a separate decision. Live: INDPRO `2026-08-01` 103.07 vs `2025-08-01` 101.62 = `+1.42%` growth, phase `mid`. In the executor: `ALLOWED_TIFS` is now `("day", "gtc", "cls")` (owner decision) so the flatten planner's own `CLOSING_AUCTION_TIF` can pass the guard's legality check; `opg`/IOC/FOK stay out and the extended-hours rule still demands `day` (execute `9eee3d6`).

- 2026-09-25 `(working tree)` - **Playbook items 2, 4 and 5 built (reported rows only), and one dead FRED alias found.** (2) `sector_screener.sector_dip_reads` composes the SAME single-name producers (`atr_compression_read`, `closing_range_read`, `base_priming_read`, `volume_dry_up`, `trigger_candle`) per SECTOR ETF, and `get_sector_rotation_screen` renders a `## Sector dip reads` table - the rotation screen could rank a sector and read its relative strength while being unable to say whether its dip was quiet or violent. Nothing enters a rank, a candidate or a size. (4) `sector_screener.etf_volume_provenance` labels what an ETF's volume contains (creation/redemption baskets + market-maker hedging; no vendor here publishes either, so the ratio is labelled and never netted), naming the ETF universe (SPDR sectors, RSP* twins, `INDUSTRY_ETFS`, broad benchmarks); a non-ETF reads `is_etf=False` = "not known to be an ETF here", never "is a stock". It renders under the dip table and beside the O'Neil distribution-day count, whose volume is the benchmark ETF's. (5) New `strategies/sector_drivers.py` maps each SPDR group to one FRED driver and a direction, and `get_sector_rotation_screen` renders a `## Sector macro drivers` table with the level, the 1m/3m change (counted in OBSERVATIONS, so the monthly series read n/a rather than pretending to be 21-day moves) and whether the move helps; a flat move reads False, an unread series reads None, and `XLV`/`XLP` are deliberately unmapped with the reason printed. Found on the way, NOT changed: `dataflows/fred.py`'s `pmi`/`manufacturing_pmi` -> FRED `NAPM` is discontinued (0 observations measured 2026-09-25, vs 13 for `industrial_production`), so `get_cycle_tilt`'s PMI leg has been reading None (None-safe, so nothing fabricated); the new mapping uses `industrial_production` instead and the alias itself was left for the owner since repointing it changes `cycle_tilt`'s behaviour [SUPERSEDED 2026-09-25: the owner chose to move `get_cycle_tilt` onto INDPRO YoY GROWTH with 0% as the expansion boundary - see the Changed entry in `CHANGELOG.md` and the newest log entry below; the aliases stay in the table with a DISCONTINUED marker.] Also in this round: item 6 in the executor repo - `calendar.close_window` + `Config.close_window_minutes` label the last 15 minutes of the session (`rth_close`) so a close-adjacent pickup is visible in the log, still scanning (execute `88d9d9e`), and the executor's pre-existing red lint step was fixed (`3a9e6ef`).

- 2026-09-25 `(working tree)` - **The owner's two answers: X3's dead gate is wired and rendered, and the two reported-only reads are built.** (a) `enable_mp_lower_spectrum` was a switch that could not be on and whose key nothing read. Now `market_breadth` receives the run's config from `_market_breadth_read` (which also caches the panel it built), `regime_score` attaches every gated read through one `_attach_printed_reads` (R5's block and X3's spectrum, each under its own key - `MP_SPECTRUM_KEY`), and `_render_regime_score` prints them BESIDE the component table, since both are backdrops rather than 0-100 legs. Gate-off runs stay byte-identical (test-pinned). (b) `relative_strength.rs_breakdown` - the lower-side twin of `rs_position` over `lookback=63`: `new_low` (strict), `near_low` (within 3%), and `lower_high` (recent half's peak vs the earlier half's, both peaks reported). It is carried by `relative_strength_report`, printed by `get_relative_strength` and `get_swing_set`, and referenced from `swing.py` - which is also what satisfies the `test_calc_agent_wiring` public-calc gate - with `candidate` provably unchanged under a hard-coded `freefall` label. (c) `compression.base_priming_read` - the T-1 two-bar state the other compression reads cannot express: prior bar tight (<=1%) OR inside, today's range >= 1.5x it, close above the prior high, rvol >= 1.4x over the prior 50 sessions (divisor = the slice's own length), close in the top quarter (>=0.75). `primed` is True only when every leg is measured and true, `None` when any leg is unmeasured. Reported on `vdu_entry_setup` and as a `value_dip_setup` row, rendered by `get_vdu_entry_setup`, outside `measured_gates` (pinned by test). Nothing gates on any of the three, and none of them was measured against an unfiltered baseline yet - gating is a separate, measured decision.

- 2026-09-25 `(working tree)` - **An owner-requested survey of a pasted "buy the sector dip / VDU + volatility-contraction" playbook became five defect fixes.** Nothing was added to the engine (the playbook's items are mostly already shipped, in the single-name value-dip path rather than the sector path - see the round report), but auditing them turned up four real defects and two false doc claims: (1) `multi_breadth` divided each window's above-count by EVERY usable member instead of the members carrying that window, so a member below the 200-bar floor counted as "not above its 200d" and each 200d column was depressed by construction, while a column with no eligible member printed a fabricated `0.0%` - both fixed (per-column `n_20d`/`n_50d`/`n_200d`, `None` when empty), and the matrix + `get_sector_rank` now apply the `small_sample` gate their own comments claimed. (2) `trigger_candle` averaged a short volume slice by a fixed `window`, so three volume bars turned an ordinary 1.2x day into `rvol=12.0` and fired a trigger. (3) `setup_a` read a "52-week high" off two bars and could emit `ready`. (4) `regime_gate_read` reported `verdict="tradable"` beside `pass=False` when only the index-stress leg blocked, its short-series return omitted `thresholds` (a documented consumer, `KeyError` under 60 bars), and its 60-bar guard let a 60-199-bar series report `above_sma200=False` / `fast_downtrend=False` as measured facts rather than `None`. Doc claims corrected: `docs/design_sector_rotation_screener.md`'s `enable_sector_rotation_screen` gate (never written - the tool registers unconditionally), `docs/gate_registry.md`'s `enable_mp_lower_spectrum` row ("visible as a count" - nothing renders it, and `_market_breadth_read` never passes `cfg`, so the gate cannot even be on from a run), and `compression._atr_series`'s "skipped rather than coerced" (it refuses the whole series). Receipts: four failing-first provers with restored sha256s in `CHANGELOG.md`'s Fixed section; engine suite re-run green.

- 2026-09-25 `(working tree)` - **Six sector/dip enhancements landed, and three defects turned up on the way.** Defects: (1) the moomoo K-line path returned ONE SDK page - `request_history_kline` caps at `max_count=1000` and hands back a `page_req_key`, which three call sites discarded, so a 5-year fallback described a ~1,000-session history while the report printed its own record count; `moomoo._fetch_kline_pages` now follows the key (bounded at 50 pages) and keeps one row per session, ascending (order MEASURED: one page comes back ascending, so pages are re-sorted, not reversed). (2) The value-dip regime row read the FUNCTION defaults (vol cap 0.8 / band 8%) because both measurement sites passed `cfg=None`, so `value_dip_regime_vol_cap` / `_downtrend_band` did nothing on the very path `require_regime` hard-gates on. (3) `docs/design_sector_rotation_screener.md` opened with "proposal - no code changed" while line 4 of the same file named the shipped modules. Then the six enhancements the owner picked, one commit each: **E1** `strategies/compression.py` (ATR/close percentile inside its own trailing ~6-month window + closing-range tightness) as REPORTED rows on the VDU ladder - they cannot flip a candidate, and a test patches both producers to prove it; **E2** `strategies/distribution_days.py` (O'Neil count: 25-session expiry, +5% rally cancel, and the follow-through reset the swing literature usually omits) as the sector screen's `market distribution (…)` line, positional over the caller's own session list so no holiday calendar can disagree with the bars; **E3** `strategies/volume_flags.py` (OPEX-week / witching flags + `rvol_ex_mechanical`) wired into `trigger_candle` / `volume_dry_up` behind the new `enable_mechanical_volume_discount` (off by default - turning it on CHANGES which triggers fire), with the trigger suppressed on a session that is itself mechanically huge; **E4** the rotation backtest's absolute-momentum arm with a risk-off leg (`cash_closes`), reported under `abs_gate` BESIDE the shipped series (bit-identical with and without the parameters - a test pins that), live at `--cash-symbol SPY --abs-window 200`: Sharpe 0.699 vs 0.603 with 5 proxy slots; **E5** the rank-hysteresis arm, which in that same window RAISED turnover (12.33 vs 11.00) rather than cutting it - the arm measures the trade-off instead of assuming it; **E6** the OHLCV cache now tails off the newest same-symbol file instead of re-downloading five years every time the window-derived file name changes (live: ZYME 1,254 -> 1,256 rows in 0.42 s, 5-year history kept). Every change carries a failing-first proof with a byte-identical restore, a CHANGELOG entry with its web-impact line, and the new gate is registered in `docs/gate_registry.md`, `tests/test_gate_env_toggles.REGISTRY`, `.env.example` and the regenerated `docs/api_reference.md` env table.

- 2026-09-24 `(working tree)` - **A prompt edit becomes attributable: the prompt is versioned and the condition recorded per run.** N5 of the survey's adoption set (T1 batch), behind `enable_prompt_condition_harness` (default off). Adopt the harness, not the framing - the module states that 2606.00061's n = 72 per cell, its sign flips across context windows and its missing multiple-comparison control make it weak evidence for its own framing, so nothing asserts an effect exists. What lands: `prompt_version(prompt)` = sha256(text)[:16], `condition_descriptor(label, {stage: prompt})`, and every ledger row carrying BOTH `prompt_condition` (arm label) and `prompt_version` (version id), so a label spanning two prompt texts is detectable (`condition_accuracy` sets `version_conflict`). No pooling: `condition_accuracy` groups scored rows by condition, reports rows with no condition or no scored outcome under `unattributed` with counts and EXCLUDES them from `n_scored`, and an arm below `CONDITION_MIN_N = 5` reports accuracy None with `below_min_n` rather than a rate over a handful of rows - so a run whose condition was not recorded is `unavailable`, never pooled into the total. `prediction_ledger.log_decision` gains a first-class `prompt_condition` parameter written as an explicit null when absent (additive; no existing caller changes). No prompt string changes anywhere in the engine, and the harness is offline by construction - the live decision-ledger writer `graph/trading_graph.py::_finalize_run_body` was deliberately NOT touched (ground rule 8), because the comparison doubles LLM spend. Failing-first, re-proved by the integrator: replacing `prompt_condition=str(label)` with `prompt_condition=None` in `record_condition_run` fails `test_condition_recorded_per_run` by name (assert None == 'arm_a'), source restored byte-identical, sha256 258b3341... Web impact: none.

- 2026-09-24 `(working tree)` - **A cross-sectional fragility read carries the calibration that makes it a probability.** R5 of the survey's adoption set (T1 batch), behind `enable_forward_stress_probability` (default off). `market_breadth.forward_stress_probability(panel, *, gate)` aggregates monthly fragility - return dispersion, the downside share below mean - 1sd, and INVERTED skew - into a one-month-ahead probability of a high-stress regime, the outcome being the equal-weight panel's next month below `FS_STRESS_QUANTILE = 0.2`. `p_stress` IS the current band's realized hit rate from `calibration.calibration_table` (the engine's ONE bin table, called directly and label-mapped, so no second definition exists), and `calibration` is present whenever `p_stress` is a number and None on every refusal - dropping it is the card's mutation, re-proved by the integrator: `test_the_probability_carries_the_calibration_report` red by name, source restored byte-identical, sha256 ad2cb652... The denominator rule is READ, not restated: a panel below `FS_MIN_NAMES = 20` is refused through `sector_screener.breadth_with_gate` with its own reason (and an empty panel is refused first, since n = 0 would pass that gate's `if n and n < min_n`); a band below `FS_MIN_BIN_N = 5` rows is refused, as is a history with no observed stress months. Honest labelling per R8: the `caveat` says the bands are fitted on the same history they score, so it is a FITTED read, not a walk-forward-tested detector - hence PRINTED in `regime_score` under `out["printed"]`, present only while the gate is on, never in COMPONENTS/COMPONENT_ORDER/align_components, so every pinned count and the coverage denominator are untouched. The regime_score edit was kept line-neutral above the line test_regime_score.py pins. Web impact: none.

- 2026-09-24 `(working tree)` - **Look-ahead is inexpressible in the factor DSL, not merely discouraged.** H2 of the survey's adoption set (T1 batch), behind `enable_factor_availability_gate` (default off). `factor_schema` gains `observability_for` and a `FactorSpec.observability` property - the record stays at exactly nine fields (Q3) and `availability` keeps its `present`/`NA` data-presence meaning rather than being repurposed - and `factor_expressions.availability_gate(expr, ...)` rejects at REGISTRATION time: any negative literal shift in `ref`/`delta`/`pct_change`, and any field whose declared class is later than the decision date. Zero per-run cost (static analysis at admission). Only what this repo can honestly state is declared: market columns are `SESSION_CLOSE`, every `FACTOR_SCHEMA` factor is `FILING_DATE`, and `NEXT_SESSION`/`MACRO_RELEASE_LAG` are legal caller-declared classes applied to NO field here - inventing a vendor or macro publication lag is exactly the open H8 decision. An undeclared field is refused with its reason, never assumed safe. Dated classes bind to `data_quality.fundamentals_pit_ok`, corroborated by `dataflows.pit_registry.read_as_of` when a symbol is named (both read, neither edited). Gate off returns the unchanged `purity_gate` verdict; `purity_gate` gained an optional `extra_fields` whose default leaves every existing caller byte-for-byte unchanged. Failing-first, re-proved by the integrator: replacing the forward-shift branch with `shifts = []` fails `test_late_availability_field_rejected` by name (its assertion reads "a negative shift reads the future and must be refused"), source restored byte-identical, sha256 10c0b0aa... The gate's only caller is the offline admission point in `scripts/factor_bench.py` - `alpha_zoo.evaluate_expr` was deliberately not touched, so nothing pays the cost per expression per run. Web impact: none.
- 2026-09-24 `(working tree)` - **The candidate zoo has a redundancy screen, with the L1 selection hand-rolled.** X2 of the survey's adoption set (T1 batch), no gate - it is an offline scheduled screen and nothing is added to the in-run surface. `alpha_zoo.redundancy_screen(candidates, controls, target, *, selection_window, evaluation_window)` does post-double-selection per candidate (LASSO the controls, LASSO the candidate on those controls, OLS the target on the candidate plus the union of survivors, re-fit on the EVALUATION window); survival needs `|t| >= 1.96` in selection AND a same-sign non-zero out-of-sample coefficient. The L1 solver is ~40 lines of fixed-order coordinate descent with the theoretical Belloni penalty `c * sd(y) * sqrt(2 log p / n)` - deterministic, no CV, no randomness, no new dependency, because sklearn is absent and adding it is an open owner decision. The two windows MUST be disjoint (an overlap raises ValueError) and means/scales are fitted on the selection window only. Consumption is a decision: a non-survivor becomes a documented component, and every dropped component gets one refusal-ledger row via the landed H7 writer `refusal_ledger.log_refusal`, naming the screen and both windows - deleting that write is the card's mutation, re-proved by the integrator: `test_every_dropped_component_is_logged` red by name, source restored byte-identical (sha256 asserted). Reachable only offline (`scripts/redundancy_screen.py`, with a static test asserting no other module under `tradingagents/**` mentions it); a window too short to measure drops every candidate WITH its reason, so `kept + dropped` still partitions the input. Web impact: none.

- 2026-09-24 `(working tree)` - **A drawdown envelope now says how deep and how long a drawdown of a given Sharpe should run - and it is report-only.** K2 of the survey's adoption set (T1 batch), behind `enable_drawdown_envelope` (default off). `book_risk.drawdown_envelope(sharpe, horizon, skew, kurtosis, hurst)` returns `mdd_median`, `mdd_p90`, `max_loss`, `time_under_water` and `longest_recovery` from a seeded Monte-Carlo over the book's own annualized volatility, and carries the Sharpe's estimation uncertainty as a field (Lo 2002, `sqrt((1 + SR^2/2)/n)`) because the card's own caveat is that Sharpe-estimation uncertainty changes the answer. The `T^(H-1/2)` rescaling applies to the DEPTH measures only and only when a Hurst estimate exists (`mean_reversion.hurst_exponent`, read not re-estimated); without one it uses square-root-of-time and says so (`hurst: "assumed"`, `hurst_scale: 1.0`). Failing-first proof, re-proved by the integrator: pinning the exponent to `0.5` takes the H=0.72, T=252 p90 ratio from 3.3752668884 to 1.0 against the closed form 252^0.22 = 3.3752668884, and `tests/test_drawdown_envelope.py::test_hurst_rescales_mdd` goes red by name, source restored byte-identical (sha256 asserted). Consumed by the strategy-evaluation row in `strategies/evaluate.py` as ONE additive gated key beside the realized `max_drawdown`. **The governor leg is deliberately NOT wired** - `risk_governor.govern`, `drawdown_gate` and `book_context.measured_book_drawdown` are untouched - because the mandate answer on letting the rescaling reach the drawdown governor is still open (plan decision 3); this is the card's own documented fallback. The K lane's focused set - the three new files with `test_strategies_regime.py`, `test_strategies_book_risk.py` and `test_risk_score.py` - is 92 passed; ruff clean. Web impact: none.
- 2026-09-24 `(working tree)` - **A regime VaR is judged by Kupiec and Christoffersen instead of asserted.** R2 of the survey's adoption set (T1 batch), behind `enable_hmm_heavy_tails` (default off). `regime.hmm_filtered_regime` gains an `emission` parameter (`gaussian` default, plus `student_t`, `laplace`, `ged`) with selectable `K`, and `regime_conditional_var(posteriors, emissions, q)` inverts the posterior-weighted mixture CDF for the running state's VaR. ONE shared forward-backward kernel - only the emission density and the per-state M-step swap - so the gaussian branch is arithmetically the pre-R2 code, verified numerically (loglik/means/covars/A/pi all array_equal) as well as by the existing `test_strategies_regime.py` pins. The coverage instrument is shared with K1: `book_risk.var_coverage_test(returns, *, alpha, var_series)` = Kupiec POF + Christoffersen joint conditional coverage, returning the verdict with its coverage level, sample size and held-out window, and `unavailable` when that window is too thin. Measured: on a manufactured fat-tailed two-regime series the Gaussian-emission VaR fails (hit 0.0375, joint p 0.00616) while Student-t passes (0.0565, p 0.2478); forcing the Gaussian CDF regardless of `emission` turns Student-t into fail (hit 0.1030) - the card's mutation, re-proved by the integrator, `test_the_coverage_test_rejects_the_gaussian_var` red by name, source restored byte-identical (sha256 asserted). Heavy-family shape parameters are FIXED and reported (`EMISSION_NU`, `EMISSION_BETA`) rather than fitted, and a `K != 2` is never unlabelled (the output carries loglik/BIC/n_obs/n_params and its selection criterion). K lane focused set: 92 passed; ruff clean. Web impact: none.
- 2026-09-24 `(working tree)` - **A tail number carries its data quality and its estimation uncertainty, and no input can narrow it.** K1 of the survey's adoption set (T1 batch), behind `enable_tail_risk_layer` (default off). `strategies/tail_risk.py` (new) COMPOSES the existing producers rather than re-deriving them (rule 2): `book_risk.simple_var/cvar/portfolio_cvar` for the number, `data_quality.aggregate_quality` for the quality score, `conformal.quantile_band/rolling_band` for the band, and the shared `book_risk.var_coverage_test` for the verdict - returning `{var, cvar, q_score, uncertainty, band, status}` with `status in {ok, widened, unavailable}` and `None` for the number whenever the status is unavailable. One-directional by construction: the widening term is `max(0.0, half - raw_half)`, so a monotonically worsening quality input widens the band monotonically (0.08448 to 0.19365 across six levels) and walks ok -> widened -> unavailable, the VaR moving outward until it refuses; flipping that guard to `(1.0 - total_widen)` narrows the band and can drive it negative, which is the card's mutation (re-proved by the integrator: `test_quality_only_widens` red by name, source restored byte-identical, sha256 asserted). A red quality verdict is `unavailable`, never a number, and the breach rate plus the full Kupiec/Christoffersen record travel with it. `risk_score` declares `tail_risk` as a PRINTED component with its producer label (the R3/R7 precedent), so no pinned scored count moves and COMPOSITE_ENGINES is untouched. Uncertainty is the mean of the measurable terms (ensemble dispersion over at least two members, an out-of-distribution distance, a trailing breach-drift term), and an unmeasurable term is excluded rather than counted as zero. K lane focused set: 92 passed; ruff clean. Web impact: none.

- 2026-09-24 `(working tree)` - **A subdominant eigenspace that moves inside the Marchenko-Pastur bulk is noise, not a rotation.** V5 of the survey's adoption set (T1 batch), behind `enable_eigen_rotation` (default off). `strategies/eigen_rotation.py` (new) removes the market mode from a rolling correlation matrix, keeps the subdominant eigenvectors, and measures rotation as the mean squared sine of the three principal angles - `mean(1 - sigma_i^2)` of the singular values of `U^T V`, invariant to eigenvector sign by construction (the test monkeypatches `np.linalg.eigh` to flip half the signs, so the property is tested against a hostile input). The MP check is **mandatory** and the bound is imported, not re-derived: `market_breadth.mp_below_count` is the engine's one producer of `mp_lower`, so the read computes only when EVERY subdominant eigenvalue sits below that edge (the sufficient condition) and refuses otherwise - `market_breadth.py` was not edited. Measured: on 6 names x 252 sessions, dropping the check yields rotation `0.700456`, angles `[0.504735, 1.219557, 1.451782]` from eigenvalues `[1.0847, 0.9861, 0.9511]` against `mp_lower = 0.715203` - all inside the bulk. That is the failing-first proof (mutation -> `test_rotation_refuses_inside_the_mp_edge` red by name, source restored byte-identical, sha256 asserted). Reached in-run through `covariance_models.spectral_functionals(..., *, cfg)`'s ONE gated `rotation` key, from the production path `regime.spectral_change_read`, so the read is not dormant on the day it lands; with the gate off every previously returned key is byte-identical, including the one-eigendecomposition-per-window property R3's test pins. The window must exceed the panel, and every refusal carries the window it was computed over. Focused set with the covariance, spectral, options-surface, regime and wiring suites: 85 passed, 1 skipped; ruff clean. Web impact: none.

- 2026-09-24 `(working tree)` - **A risk-neutral density is recovered only when the quotes can identify one.** V3 of the survey's adoption set (T1 batch), behind `enable_rnd_recovery` (default off). `strategies/rnd_recovery.py` (new) fits a two-component lognormal mixture per expiry under mass and forward constraints (the high component's forward fixed by the mixture mean equalling the forward, so the constraint is exact rather than a penalty), and returns the 1% quantile, the risk-neutral variance and the tail probability with its threshold printed (`RND_TAIL_THRESHOLD`). Deterministic: bounded `scipy.optimize.least_squares` from a fixed initial guess, no seed, no wall clock. **The deliverable is the refusal:** identifiability comes from the conditioning spectrum of the covered strikes - the SVD of the price-sensitivity matrix of the mixture's OTM quotes w.r.t. its free parameters at the fitted point - and a rank below the parameter count, or a condition number above `RND_MAX_CONDITION = 1000`, returns `unavailable` with `density=None` and BOTH numbers reported. On the card's four-strike chain the condition number is 4031.5, so it refuses; with the guard dropped the same chain returns `quantile_01 = 50.4969`, `risk_neutral_variance = 0.288167`, `tail_probability = 0.359443` - the failing-first proof (mutation -> `test_sparse_chain_returns_unavailable_not_a_density` red by name, source restored byte-identical, sha256 asserted). The learned recovery operator is **not** built (the card records that per-expiry classical fits beat the adapted learned operator on 524 held-out NIFTY calls); `model_free_implied_variance` seeds the guess and `variance_swap_strike` is a reported cross-check, both read and neither re-derived (rule 2). Reached in-run from `options_surface.expected_move_from_chain(rows, *, cfg)` - called by `analysis_tools.get_options_iv_read` - as ONE additive key present only while the gate is on, so a gate-off run is byte-identical to the run before it existed. Focused set with the options-surface, covariance, spectral, regime and wiring suites: 85 passed, 1 skipped; ruff clean. Web impact: none.

- 2026-09-23 `(working tree)` - **A regime change is declared only against calibrated noise.** R3 of the survey's adoption set, behind `enable_spectral_null_band` (default off). `covariance_models.spectral_null_band(n_obs, shrinkage, *, z)` = `z*(1-shrinkage)/sqrt(n_obs)` - a PURE function of window length and shrinkage intensity, so the same length and intensity always carry the same band; `panel_spectrum` does ONE eigendecomposition per window over `statistical.correlation_matrix`'s correlation and refuses per rule 4 (thin window, unresolved pair, unmeasurable intensity) as `unavailable`; `eigen_projector_distance` = `||P_k - P_k'||_F` carrying move / normalized / band / geometry / threshold / exceeds / reading; `spectral_functionals` reports the absorption ratio (top max(1, round(n/5)) modes) and the leading share, each with its own band and flag. `regime.spectral_change_read` is the gated per-PANEL read (two eigendecompositions total) and a non-rejection reads "**not detectable**", never "no change" - the paper's own framing is that non-rejections partly reflect wide bands. WHAT ENTERS THE SCORE IS THE FLAG, not the functional: `regime_score` registers `spectral_change` as a SEVENTH component declared only while the gate is on, and a functional on any other scale aligns to None, so the absorption ratio or the projector distance cannot drift into the score (rule 2 kept: the read is calibrated WITH the engine's intensity, not a second authority beside it; COMPOSITE_ENGINES untouched). THE FINDING THAT SHAPED IT: the engine's `ledoit_wolf_shrink` intensity saturates at 1.0 on a narrow panel, so a band scaled by (1-shrinkage) collapses to zero and would flag EVERY move; the read therefore calibrates at an EXPLICIT intensity (default 0.0, the raw correlation read), refuses a zero band with its reason, and reports the engine's intensity as `engine_shrinkage` - a saturated input cannot silently read as "everything moved". STATED GAP: the component and its glue are in place but no in-run caller supplies a panel yet, so it is dormant in-run - queued in this round, and a gate-off run's declared set is unchanged. Three stale file:LINE anchors in regime_score.py repaired. Failing-first: forcing the flag True fails `test_spectral_null_band.py::test_a_stable_covariance_is_not_flagged` by name, restored byte-identical; focused set 78 passed; ruff clean.

- 2026-09-23 `(working tree)` - **The engine can now flag an arithmetically impossible accuracy claim - and building it exposed a look-ahead in its own sigma recursion.** H4 of the survey's adoption set, behind `enable_accuracy_ceiling` (default off). `alpha_eval.ceiling_ratio` fits `volatility_models.garch11_fit` (READ, rule 2) on the leading `train_frac` share, builds each hold-out row's `sigma_hat` from a one-step-ahead recursion, and reports `{kappa_hat, da_squared, r2_ratio, r2_oos, da, n, train_n, flagged, basis, unavailable}` with `flagged = r2_ratio > da_squared`; `calibration.excess_accuracy` reports the model hit rate minus the always-up base rate on identical rows per walk-forward fold, carrying H11's `block_bootstrap_interval` (imported, not reimplemented); both refuse to `unavailable` WITH the reason, never a zero. Read as a FALSIFIER, never a validator. THE DEFECT: the delivered recursion consumed a row's return BEFORE emitting that row's sigma, making `eps_t` the model's own standardized residual and the ceiling circular - the docstring already said the return must be consumed only after its row, so the order is fixed here (emit, then update). It was found by verifying the failing-first claim rather than accepting it, and the FIRST PROOF ATTEMPT FAILED and is recorded: on the card's fixture the GARCH fit returns `alpha = 0` (iid data has no clustering to fit), the recursion stops depending on the current return, and the two orders are numerically identical - so neither the ordering mutation nor a fit-window mutation turns the card's test red, i.e. that test did not guard the property it names. The guard was rebuilt on a GARCH-clustered fixture (fit alpha ~ 0.2) where the one-step-ahead order gives `kappa_hat` -0.343 and consuming the row's own return first gives -0.255, and the new `test_sigma_hat_never_consumes_its_own_row` asserts `< -0.30` - the property with ~0.045 margin each side, not a magic number. The fixture's docstring records why it must be clustered. The card's named test is kept as it was. Three interpretations recorded: the return is a record whose core triple is exactly the card's plus the verdict and reason rule 4 and its own acceptance test require; `kappa <= 0` by Jensen flips the legal region, so the implemented rule is the card's stated inequality; and the resolved decisions carry a direction, not a magnitude, so the ceiling reads a unit-scaled sign forecast with Hold/REVIEW dropped rather than scored as misses. Rule-7 caller: `trading_graph._maybe_report_accuracy_ceiling` from `_resolve_pending_entries`.

- 2026-09-23 `(working tree)` - **"Not proven" and "disproven" are now different answers.** H6 of the survey's adoption set, behind `enable_materiality_verdict` (default off). `evaluate.materiality_verdict(stat, ci_low, ci_high, delta)` returns SUPPORTED iff `L > delta`, REFUTED iff `U < delta`, INCONCLUSIVE on a straddle, `unavailable` on missing/non-finite input - and **INCONCLUSIVE is never collapsed into REFUTED**: a straddling interval is unresolved, not evidence of no edge, and a reader taking "we could not show it" for "we showed it is not there" is the failure this vocabulary exists to prevent. The card's mutation (collapse the verdict) fails `test_materiality_verdict.py::test_straddling_interval_is_inconclusive` by name, restored byte-identical. `family_materiality` is the ONE producer of the family adjustment: it reuses the `reality_check`/`spa` bootstrap machinery, applies Benjamini-Yekutieli step-up, attaches each candidate's H11 interval, classifies the R axis at `delta_r` and reports the S bar at `delta_s` as a named necessary condition - and a SUPPORTED candidate failing FDR survival is demoted to INCONCLUSIVE, never REFUTED. `benchmark_table` gains an exposure-time-MATCHED row (benchmark on held bars, cash elsewhere, cost-free) when an exposure series is aligned; dropped otherwise, default path byte-unchanged. The vocabulary reaches `report_verifier`: `ReportVerification.verdict` (SUPPORTED/REFUTED/INCONCLUSIVE/UNKNOWN, `overall` untouched), REFUTED only on contradicted/misquoted/internal-conflict claims, and a tree-level `family` key. The thresholds `materiality_delta_s = 0.20` and `materiality_delta_r = 0.01` are plain DEFAULT_CONFIG VALUES (no enable_ prefix, no env row, no gate) read by literal key, so they are declared before any result is seen; the card's caveat travels - they are non-canonical by the author's own admission and the unit is a catalogue rule, not a portfolio. Web impact: the app renders `verify_flags.json` generically; gate off is the pre-H6 payload exactly, gate on adds the per-stem `verdict` and tree-level `family` key, and the contract pair is run against this change.

- 2026-09-23 `(working tree)` - **The engine now records the candidates it REFUSED.** H7 of the survey's adoption set, behind `enable_refusal_ledger` (default off). Every guardrail recorded what it did; none recorded what it stopped, so a guardrail's precision was unfalsifiable - a refusal with no ledger is a claim with no denominator. `strategies/refusal_ledger.py` (new) appends one immutable row per refused candidate (`symbol`, `as_of`, `gate`, `reason`, `snapshot_hash`, `ts`, `written`), `classify_refusal` scores it against its own forward path into five tiers (`saved-windowed` / `missed-moon` / `saved-early-death` / `flat` / `unclassifiable`), `score_all` joins on the row's own `(symbol, as_of)`, and `save_to_miss_ratio(min_events=30)` publishes the per-gate taxonomy with **`events` always beside the ratio** plus explicit `underpowered` / `unavailable` reasons - a ratio without its denominator is what this file exists to prevent. THE tie-break is the load-bearing branch: a refusal that both saved capital and missed a run is a MISS, as an explicit branch, because without it saved-first ordering calls every such refusal saved - and that is the card's mutation, proven by name by the integrator. Five of five writer sites reached, all additive: `risk_governor.govern`, `knife_guard.knife_factor` (only the block band), `market_tradability` (suspended / limit up-down / volume truncation), `news_relevance.admit_article`, `value_dip.value_dip_setup`; each gained keyword-only optional `symbol`/`as_of`, its verdict is unchanged, and every write sits under `contextlib.suppress` - a ledger that can break the refusal path it observes is worse than no ledger. `scripts/refusal_scorecard.py` is the rule-7 caller. Card caveat carried: the paper's generalisation is weak (13-22 days, one chain, one deployment, one regime) and its headline tier was refuted by its own test, so the taxonomy and discipline are adopted and the ratio is never a constant. Focused set 90 passed; ruff clean.

- 2026-09-23 `(working tree)` - **A confirmed divergence from Ledoit-Wolf (2004) in `ledoit_wolf_shrink`, measured and deliberately NOT fixed.** `covariance_models.ledoit_wolf_shrink` computes `shrinkage = clip(b^2/d^2, 0, 1)` where `b^2` and `d^2` are the paper's own two quantities, but the paper's intensity is `clip((b^2/t)/d^2, 0, 1)` - the division by the observation count `t` is missing, so the intensity is `t` times too large and saturates at 1.0 on every panel narrow relative to its history. Measured on an iid Gaussian panel p=5, t=250: paper 0.6366 vs engine 1.0000, and the returned matrix sits on the scaled-identity target (`||Sigma-I||/||I||` 0.0444 against 0.0363 for the pure target). At p=11, t=252 the paper's own answer is 0.995, which is why it has never shown on an ETF-width panel. RECORDED, NOT FIXED, because the code matches its own module docstring (line 9) and its own pinned test (`test_strategies_covariance_models.py` recomputes the same `b^2/d^2` and asserts equality): correcting it changes a documented, tested convention whose output an analyst-facing covariance read also consumes, so it is the owner's call, not this wave's. The correction would be one line plus that docstring and that expectation. The finding shaped R3's design instead: its band is calibrated at an EXPLICIT intensity, refuses a zero band with its reason, and reports the engine's intensity beside it as `engine_shrinkage`, so a saturated input can never silently read as "everything moved".

- 2026-09-23 `(working tree)` - **The two wave-1 reads that were built but unreachable are now wired into their tools.** Both had landed their read and their rule-7 chain but left the analyst-facing tool alone, which by working agreement 1b is an unfinished item, not a landed one. V6: `get_tail_extreme_var` fetched the tape and passed only `rets`, so the jump leg had no window - it now passes the same `_ohlcv(ticker)` payload into `book_risk.extreme_quantile_var` and renders the leg (gate off = the line it always produced; gate on appends `, jump_share=0.75 (one_print)` on a one-print tape), with the EVT numbers untouched. R1: `get_shift_detection` called `bocpd(series)` with the default, so the duration-law modes existed and nothing could select them - it now reads a new MODE key `bocpd_hazard_mode` (env `TRADINGAGENTS_BOCPD_HAZARD_MODE`, default `constant`) whose helper falls back to `constant` on an unknown or unreadable value rather than guessing; `constant` is today's read bit-for-bit, `lognormal` adds `hazard_mode=lognormal covering=<x>` to the printed line, a bogus value is inert. The key is a mode and not a gate - no `enable_` prefix, no registry row, no change to the enable_* count, and the sibling app's derived allowlist does not see it - but it does appear in `docs/api_reference.md` §1.1, which is generated from every env var the config reads. Evidence: both real tool functions were run against a synthetic tape per state (gate off/on; constant/lognormal/bogus) and the outputs are as described. No tool signature, CLI flag or JSON shape changed.

- 2026-09-23 `(working tree)` - **Every claim statistic the panel and the scorecard publish now carries a block-bootstrap interval and the IID one it is judged against.** H11's attachment half, behind the same `enable_bootstrap_intervals` whose estimator half landed earlier in this round. `scripts/score_panel.py` gains public `claim_interval(series, *, window)` (the rule-7 caller of `conformal.block_bootstrap_interval` / `iid_interval`); `factor_statistics` attaches it to the rank-IC and decile-spread series, `evaluate_panel` to the pooled per-period mean spread series, and `render_text` prints bounds, point, `n`, `block=`, `draws=`, method and the IID baseline. `scripts/coverage_scorecard.py` keeps the per-tree present/absent indicator per declared field in tree order and attaches an interval beside the unchanged `fill_rate`. The block length and draw count are PRINTED in both reports, so the dependence assumption is visible rather than implied - which is why the attachment is proven to fail when swapped for the IID arm (block length 1 everywhere). Below the floor, on an unusable series, or on a ZERO-DISPERSION series the record is `{window, n, unavailable}` with no bounds at all, never a zero-width interval - the most confident possible lie. Gate off adds no key and no rendered line, asserted both ways. Failing-first: the IID swap fails `test_bootstrap_intervals_scripts.py::test_the_block_length_is_not_a_constant_across_persistences` by name (`assert 1 > 1`), restored byte-identical; existing panel + scorecard suites 70 passed.

- 2026-09-23 `(working tree)` - **A stress read that names where the stress is centred.** R7 of the survey's adoption set, behind `enable_triadic_stress` (default off). `strategies/triadic_stress.py` (new) builds `TSI = (C*D/M)*Coex` over the correlation network `statistical.correlation_matrix` already produces, with a per-node `diag(A^3)` **epicentre** attribution ordered descending (ties by name, cut at the cross-sectional mean, the cut printed as `epicentre_cut`). The engine could say concentration moved; it could not say which names the stress sits on. Every returned read carries `coincident: true` and the refusals ARE the honest half: `unavailable` below EIGHT names, below 60 aligned bars, with no usable matrix, or when `lambda_2 <= 1e-9` - a four-symbol book is a thin cross-section and refuses. H10's `coverage_window` supplies the reported window and `correlation_matrix` is read, never reimplemented. Registered PRINTED, not SCORED, in `risk_score.py`'s COMPONENTS: `tests/test_risk_score.py` pins the correlation category at exactly two scored components and scoring it would have meant editing an existing assertion, while PRINTED also keeps it from competing with `market_breadth` (rule 2) and leaves `COMPOSITE_ENGINES` alone (rule 3). Measured on a manufactured panel: attribution A 4.76 / B 4.73 / D 4.72 / C 4.71 against 0.03-0.27 outside, epicentre stable across five seeds. Card caveat carried: coincident, not leading; only marginally exceeds the Absorption Ratio; saturation on a small cross-section. One wrong citation recorded: the card named `tests/test_strategies_risk_score*.py`, which does not exist - the real file is `tests/test_risk_score.py` (33 green). Failing-first: emptying the epicentre fails `test_triadic_stress.py::test_epicentre_names_the_stressed_block` by name (`assert set(rec["epicentre"]) == set(BLOCK)`), restored byte-identical.

- 2026-09-23 `(working tree)` - **The pre-event options read is indexed in event time.** V4 of the survey's adoption set, behind `enable_event_iv_lift` (default off). `options_surface.pre_event_iv_lift` asks `catalyst.fed_imminence` - the sole authority on the scheduled catalyst - which meeting is in the window and indexes the ATM implied volatility by **days to the event** (`days_until_event - days_to_expiry`), not calendar days to expiry. Expiries at or after the catalyst are COUNTED (`n_post_event_expiries`) and never valued, so there is no forecasting leg past the meeting: the authors' own tests leave the forecasting half insignificant and adopting it would be adopting a negative result as a positive one, so the read is descriptive and says so. It refuses with a reason and never a zero when the as-of is missing, when the calendar certifies no meeting, when a named event_date is not that meeting, or when fewer than `PRE_EVENT_MIN_EXPIRIES` expiries sit at or before the catalyst. Its rule-7 caller is `get_vol_surface_shape`, which renders one additive line beside K3's proxy block. Two notes: the card's `(chain, event_date)` signature was settled by the design doc's own "the chain the engine already fetches plus the calendar it already has", so the CALENDAR names the event and there is no second authority on the day count; and a pre-existing wart is recorded, not fixed - `_rows_for` floors days-to-expiry (`int(max(days,1)/365*365)`), so a 9-day expiry can print as 2 days before a 10-day catalyst. Failing-first: indexing by calendar days fails `test_options_surface.py::test_pre_event_lift_is_indexed_in_event_time` by name, restored byte-identical; existing options + analysis-tool suites 232 passed.

- 2026-09-23 `(working tree)` - **An interval now says what it was computed as if.** H11 (first half) of the survey's adoption set, behind `enable_bootstrap_intervals` (default off). `conformal.iid_interval` resamples one observation at a time and is public because it is the BASELINE; `block_bootstrap_interval` resamples non-overlapping blocks whose length comes from the series' own ACF decay (first lag inside the `2/sqrt(n)` noise band, floored at `n**(1/3)`) checked against an ADF stationarity test - stationary earns the ACF horizon, a series that survives the check may carry a unit root and gets the base length. `block_length` travels with the interval: the dependence assumption cannot be recovered from the width. THE NUMBER THAT MATTERS IS THE ONE IT DOES NOT FIX: at an AR(1) with rho=0.8, n=250, nominal 0.90, the IID interval covers 0.45 and the block interval 0.75 - it recovers most of the loss and does not restore nominal coverage under strong persistence at panel scale, which is why the module states it as the most important thing to know about the estimator, and why the realized coverage is what the caller reads. The rule was itself corrected by measurement: the first cut had the ADF check inverted and left coverage at 0.65. The second axis, `information_gap`, scores a pooled scale against a conditional-scale alternative in nats per observation (measured 0.004 homoskedastic / 0.092 mild / 0.257 strong, threshold 0.02 with an order of magnitude of margin both sides), and `rolling_band` gains it only when the gate is on - gate off is key-for-key the dict it always was, asserted. DECISION: in-house moving-block bootstrap, not the public `tsbootstrap` (open decision 9) - the library is absent and the adoption set forbids a new dependency, so the only new import is `random`; switching later is additive. The card's attachment half (`scripts/score_panel.py` + `scripts/coverage_scorecard.py`) is a separate commit in this round. Failing-first: giving `_resample_means` the IID arm for every block length fails `test_bootstrap_intervals.py::test_block_interval_covers_ar1` by name, restored byte-identical.

- 2026-09-23 `(working tree)` - **The changepoint read stopped asserting a flat run-length prior.** R1 of the survey's adoption set, extending the EXISTING `enable_bocpd` (no new gate; default off). `regime.bocpd` gains `hazard_mode` = `constant` (today's behaviour, the default) or a duration law from {lognormal, pareto, geometric} whose parameters are re-estimated every step on the expanding window of observed run lengths (completed runs plus the open run's right-censored age). Same `O(t)` recursion; the change mass becomes `sum_r p_r*h(r)` instead of `h`. A changed hazard can only be judged if the segmentation is comparable, so the duration modes also report `duration_params`, the applied `hazard_curve`, and `covering` = the length-weighted Jaccard against the constant-hazard segmentation of the same series. Fewer than two observed runs means the scalar hazard (its geometric prior) governs; a degenerate fit takes the same `None` refusal path an out-of-range `hazard` takes; an unknown mode is refused, never guessed. Bit-for-bit: the builder compared against `git show HEAD:`'s module over 59 cases (4 seeds x 4 lengths x {default, hazard=1/20, warmup=5}, shift prefixes, degenerate inputs) with 0 field mismatches, the only added key being `hazard_mode`; `test_bocpd.py` 15 passed including the `map_run_length == 240` pin. STATED GAP: the modes are dormant in-run (`get_shift_detection` still calls `_bocpd(series)` with defaults) - the selector is queued because that file carries another item's edit this round, and it will default to `constant`, so it can only be inert. Caveat carried: the paper validates on high-frequency order flow, not daily bars, and its multivariate extension lost to univariate filters. Failing-first: collapsing `law_mode` to None so the duration law never compiles fails `test_bocpd.py::test_hazard_mode_lognormal_is_run_length_dependent` by name (`KeyError: 'hazard_curve'`), restored byte-identical. [SUPERSEDED 2026-09-23: the `get_shift_detection` selector landed in the same round via the new `bocpd_hazard_mode` mode key, so the modes are selectable in-run; see the newest entry.]

- 2026-09-23 `(working tree)` - **A memory parameter, not just a roughness exponent.** V2 of the survey's adoption set, behind `enable_long_memory` (default off). `strategies/long_memory.py` (new) estimates `d` by GPH log-periodogram regression AND local Whittle on the same ordinates (`log I(lambda_j) = c - 2d log lambda_j`, `m = isqrt(n)` frequencies, `se = pi/sqrt(24m)`), and `rv_forecast` is the HAR/HAR-X family whose extra regressor is a cross-sectional or sector persistence aggregate. `d` is reported BESIDE the forecast, never instead of it. Backwardness is enforced: a window containing any observation at or after the as-of date is REFUSED, not trimmed - the failure being guarded is a future stressed name entering a persistence aggregate meant to describe the past - and the X_t leg reads `unavailable` with its reason until a panel read exists. Validated on synthetic ARFIMA(0, d, 0): `d=0` recovers 0.020/0.028, `d=0.44` recovers 0.477/0.457, `se=0.123`, so both bracket the truth within their standard error. `mean_reversion.hurst_exponent` is untouched; the new `memory_profile` reports beside it. Card caveat carried: the gains are "moderate" and largest in stress, and the paper's cross-section is deeper than this engine's, so this is a re-measurement before any threshold is trusted. Failing-first: making the backwardness refusal unreachable fails `test_long_memory.py::test_memory_parameter_is_backward_and_reported_beside_forecast` by name (`assert 'ok' == 'unavailable'`), restored byte-identical.

- 2026-09-23 `(working tree)` - **A panel read that counts how many independent bets are left.** X3 of the survey's adoption set, behind `enable_mp_lower_spectrum` (default off). `market_breadth.mp_below_count` counts the panel correlation eigenvalues strictly below the Marchenko-Pastur lower bound `(1 - sqrt(n/w))^2`; `sector_breadth.mp_lower_spectrum` is the gated per-PANEL read (never per-symbol) and `market_breadth` attaches its single key only when the gate is on. The boundary is the item: `w == n` reads **`unavailable`, never `count == 0`** - "no eigenvalue is below the bound" and "this window cannot support the bound" are different claims, and only one is true. A missing/non-square/non-finite correlation matrix and a panel with fewer than two full-window names are refused the same way, with the coverage (`window`, `n_names`, `panel_n`) reported instead. `multi_breadth`'s output is pinned to a frozen literal snapshot and asserted unchanged. Read it as a STATE, never a signal: coincident, direction-blind, thin at this book's size. A third gate needed the literal-key fix (`GATE_NAME` reads as "nothing reads it"), so `docs/gate_registry.md` §8 now states the trap as part of the gate contract. Failing-first: `w <= n` -> `w < n` fails `test_strategies_market_breadth.py::test_mp_below_count_unavailable_when_window_not_longer_than_names` by name (`assert 'ok' == 'unavailable'`), restored byte-identical. Also corrected three `docs/gate_registry.md` cells from this round whose backticks had been shell-escaped.

- 2026-09-23 `(working tree)` - **A forward-rank study of the liquidity estimators the engine already has.** X6 of the survey's adoption set: `scripts/liquidity_forward_rank.py` (new, offline, no gate) computes `rank_ic(illiquidity_month_t, fwd_return_{t+1})` using the existing `liquidity_risk.amihud_illiquidity` / `kyle_lambda` and `signal_analysis.rank_ic` - no new estimator, no new statistic, neither module modified - and the reported table names its return window. Undetermined trade direction reads `unavailable` WITH its reason, an empty panel is reported not crashed on, and nothing in `tradingagents/**` calls it (ground rule 8). On the engine's own 2026-09 panel (25 symbols) every forward leg reads `unavailable` because the forward month is unrealized as of today - the honest output, not a defect. Two decisions recorded: `--panel` defaults to the repo's `reports/` tree, not the WP-10 metric panel (those rows carry no volume column and cannot feed either estimator), and `kyle_lambda`'s default `min_obs=30` exceeds a month of daily bars, so the study passes `min_obs=5` through its own flag with the formula untouched. Failing-first: shifting the return leg from `t+1` to `t` fails `test_liquidity_forward_rank.py::test_forward_window_not_contemporaneous` by name (`assert 'return_t' == 'fwd_return_t+1'`), restored byte-identical.

- 2026-09-23 `(working tree)` - **The tail read can say whether the tail came in one print or in a diffusion.** V6 of the survey's adoption set, behind `enable_jump_robust_proxies` (default off). `volatility_models.bipower_proxy` / `quarticity_proxy` are the daily-bar analogues (`BPV = (pi/2)*sum|r_i||r_{i-1}|`, `jump share = max(RV-BPV,0)/RV`, `RQ = (n/3)*sum r^4`) - a PROXY battery, named as such in every docstring, key and basis string, because the paper's value is its tick archive and only the daily subset is reproducible here. Below `min_obs` or on a zero-variance window every value is None with the reason, never 0.0. `book_risk.extreme_quantile_var` (the EVT/GPD tail read) gains a `jump` leg with `tail_shape` in {one_print, mixed, diffuse, unavailable} and thresholds named as `_ONE_PRINT_SHARE` / `_DIFFUSE_SHARE`; gate off or no bars means the key is absent and the record is byte-identical. Rule 2 shaped the record: neither proxy exposes its own realized variance, so no second authority sits beside `semivariance`'s `rv`. Two notes: the card-named `tests/test_volatility_models.py` did not exist and was created (the pre-existing suite `tests/test_strategies_volatility_models.py` is untouched and green), and the `get_tail_extreme_var` tool wrapper is not yet wired to pass the bars it already fetches - so the leg fires only when a caller supplies the window, a queued follow-up rather than a silent skip. Failing-first: dropping the lag-1 cross-product fails `test_volatility_models.py::test_jump_proxy_separates_one_print_from_diffusion` by name (`assert 0.0 > 0.0`), restored byte-identical. [SUPERSEDED 2026-09-23: the `get_tail_extreme_var` wiring landed in the same round - the leg now fires in-run; see the newest entry.]
