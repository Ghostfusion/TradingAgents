...
| `get_dupont_read(net_margin, asset_turnover, equity_multiplier, tax_burden?, interest_burden?)` | `strategies.dupont` | fundamentals | DuPont ROE decomposition (why ROE is high: margin vs turnover vs leverage) |
| `get_scenario_dcf(fcf, wacc, shares?, cash?, debt?, g_base?, g_bear?, g_bull?)` | `strategies.scenario_dcf` | fundamentals | bear/base/bull DCF value range (growth ±2% + optional margin shocks) |
| `get_earnings_quality_verdict(net_income, ocf, total_assets, fcf?, capex?, eps_growth?, fcf_growth?)` | `strategies.earnings_quality` | fundamentals | earnings-quality verdict (cash conversion, accruals, FCF-vs-EPS divergence) |
