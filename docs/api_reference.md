...
| `get_sector_rotation_screen(enable_breadth?, top_n?)` | `strategies.sector_screener` | market | Sector-first rotation screen: regime cap + multi-factor SPDR rank + RRG quadrant + pullback-divergence leaders + dispersion trend (+ breadth/EW-CW/Setup A-B with breadth) — advisory |
