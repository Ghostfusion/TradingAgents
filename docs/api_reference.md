...
| `TRADINGAGENTS_ENABLE_FORWARD_STRESS_PROBABILITY` | `enable_forward_stress_probability` | default `false` |
| `TRADINGAGENTS_ENABLE_PROMPT_CONDITION_HARNESS` | `enable_prompt_condition_harness` | default `false` |
...
| `get_patent_activity(ticker)` | USPTO PatentsView (free key) | fundamentals | Annual granted patents + recent titles (name-based assignee) — innovation/moat gauge |
...
| `get_trade_outcome_metrics(ticker, entry, stop?, target?, direction?)` | `prediction_ledger.outcome_metrics` | risk debators | MAE/MFE + stop/target hits over the trailing closes (W1-3) |
...
| `get_macro_regime_read(rate_change?, curve?, spread?, usd?, vol?)` | `regime_performance.macro_regime` | news | cross-asset Risk-On / Liquidity-Contraction / Stagflation (W4-6) |
...
| `get_position_sizing(confidence, stop_dist_pct, ...)` | `size.kelly + risk-budget` | market | min(kelly_quarter, risk/stop, cap). Every argument is a FRACTION of capital (0.01 = 1%); a percent number (`stop_dist_pct=8.371`, `risk_per_trade=1.5`) is refused with the corrected form instead of being read as a size |
| `get_risk_gate(size_pct, ...)` | `risk_governor.govern` | market/trader | PASS / WARN / REJECT for the trade-level limits. `size_pct` and every other `*_pct` argument is a FRACTION: a percent number is refused before the governor runs (`size_pct=1.0` used to be read as 100% and REJECT a 1% proposal - NVDA 2026-09-15). The measured book drawdown is NOT a caller input: a `drawdown_pct` passed here is reported as a hypothetical and never moves the verdict - the portfolio drawdown gate is `get_composed_risk_gate` (one resolver: `strategies/book_context.py`) |
...
| `get_analyst_verdict(ticker, date)` | `screener screen_ticker` + `normalized.trap_verdict` | fundamentals | EY, EV/EBIT, Tobin's Q, F/M/Z, Net-Net, trap risk, ROE, YoY |
...
| `get_sector_rotation_screen(enable_breadth?, top_n?, constituent_universe?, eodhd_cap?)` | `strategies.sector_screener` | market | Sector-first rotation screen: regime cap + multi-factor SPDR rank + RRG quadrant + pullback-divergence leaders + dispersion trend (+ breadth on the full S&P-500 universe n<20-gated, EW/CW vs the real RSP* index/own-50d-SMA, multi-timeframe 20/50/200 matrix + McClellan Osc/MSI + Setup A-B) — advisory |
| `get_sector_rank(ticker)` | `strategies.sector_rank.rank_sectors` + `sector_standing` (+ gated `rank_sectors_multifactor` / `rank_industry_group` / `constituent_breadth` / `leadership_ratio`) | market | 11-SPDR 1m/3m momentum ranking + the ticker's sector standing; when `enable_sector_multifactor` (P1) /enable_sector_industry (P2) /enable_sector_breadth (P3) are on, advisory score / industry / breadth / leadership lines append (default off → legacy output unchanged). The P2 industry layer uses the `sector_rank.INDUSTRY_ETFS` universe — extended to the IT subsector set (SOXX/IGV/CIBR/SKYY/AIQ/BOTZ/DTCR/NXTG/IYW/FINX/XSD parented XLK; VGT = benchmark, excluded from the pool) with dual-benchmark RS: on the XLK pool each row also gets `rs2` (percentile vs VGT), so "strongest IT subsector" (vs VGT) and "beats the market" (vs SPY RS) are answered from one pass (top-pick line appends `rs2_vs_vgt=…`). |
| `get_dupont_read(net_margin, asset_turnover, equity_multiplier, tax_burden?, interest_burden?)` | `strategies.dupont` | fundamentals | DuPont ROE decomposition + driver verdict (margin/turnover/leverage/mixed-led via log attribution) |
| `get_scenario_dcf(fcf, wacc, shares?, cash?, debt?, g_base?, g_bear?, g_bull?, margin_shock_bear?, margin_shock_bull?, market_price?)` | `strategies.scenario_dcf` | fundamentals | bear/base/bull DCF value range + market band (below bear/bear-base/base-bull/above bull) + margin of safety |
| `get_earnings_quality_verdict(net_income, ocf, total_assets, fcf?, capex?, eps_growth?, fcf_growth?)` | `strategies.earnings_quality` | fundamentals | earnings-quality CONCERN verdict (LOW/MEDIUM/HIGH = concern; cash conversion, accruals, FCF-vs-EPS divergence) |
...
| `get_exit_plan(entry, atr?, current?, peak?, stop?, giveback_pct?, ticker?)` | `strategies.exits.breakeven_after_confirmation` + `max_giveback_exit` | market | structure/R breakeven trigger + margin-giveback stop in one exit-management read. Same measured-level contract as `get_exit_check`: pass `ticker` for a MEASURED close + ATR, otherwise the output labels its levels caller-supplied |
...
| `get_risk_parity_alloc(ticker, returns_by_name)` | `strategies.portfolio_optimizer` (risk_parity + min_variance + **max_diversification** + risk_contribution) | market | risk-parity weights (equal marginal risk contribution via the converging `w ← b⊘Σw` fixed point — audit-fixed 2026-09-06; degrades to equal-weight with a note on non-convergence), min-variance weights, max-diversification weights (Choueifaty `Σ⁻¹σ`) and per-name risk contributions from a real covariance matrix |
...
| `get_composite_rank(ticker, factors?, current_date?)` | `strategies.factors.composite_score` (+ round-3 S3 under `enable_quality_composite`) | fundamentals | cross-sectional value+momentum composite percentile vs industry peers (the peers come from `finnhub.peer_symbols`, which PARSES the vendor's rendered `"Peers: A, B, ..."` string - iterating it made the peer set the sentence's characters); gated additively the 0-100 composite quality score over the same peer set with its metric set, coverage, peer count and quality band |
...
| `get_earnings_quality(ticker, date)` | `strategies.earnings_quality` + `strategies.normalized.trap_verdict` | fundamentals | provider-fed earnings-quality read: consensus CONCERN verdict (cash conversion, accrual ratio, FCF=OCF-|capex|, negative-FCF red flag) + forensic Beneish/Altman/F-Score trap. Gated additive lines: `altman_zone=` with its variant (`enable_altman_variants`) and `f_score_band=` (`enable_f_score_detail`), both render-only and never changing the severity level |
...
| `get_capex_quality(ticker, date)` | `strategies.capex_quality.capex_quality_read` | fundamentals | Capital-allocation read (advisory, deterministic): CapEx intensity z vs the name's own history, funding cover OCF/CapEx, CapEx-vs-revenue 5y CAGR elasticity, incremental ROIC (3y lag, dNOPAT/dInvestedCapital) + economic spread vs WACC, CapEx ROI 3y, payback, FCF-recovery gap to a 3% target yield, a 5-regime label (HARVEST / PRODUCTIVE / PRODUCTIVE_INVESTMENT / INVESTMENT_WATCH / OVERINVESTMENT / DISTRESS) and a 0-100 quality score + advisory valuation penalty. NEVER a hard gate - distinguishes negative-FCF reinvestment (PRODUCTIVE_INVESTMENT) from value destruction (OVERINVESTMENT / DISTRESS) |
...
| `get_constituent_cap_weights(weights, cap?, ceiling?)` | `strategies.capital_income.cap_and_redistribute` | fundamentals | exact-ceiling constituent capping (excess pro-rata to sub-soft-cap names; flags degenerate fallback when the ceiling is not enforceable) |
...
| `python scripts/jev_decide.py [--tree <dir>] [--verdict]` | judge a document with the TypeSafe decisions model (`typesafe/jev-1.13`) via `POST /api/alpha/decisions` — not a chat model; `--verdict` = the analyst reports only, position language neutralised, buy/hold/sell battery; also `--state-file` / `--state` / `--stems` / `--all` / `--neutralize` / `--rating` / `--questions`; exit 1 on any failed call |
...
- `docs/design_webull_data_provider.md` - provider study (2026-09-16, design only): Webull
