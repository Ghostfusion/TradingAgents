...
| `get_sector_rotation_screen(enable_breadth?, top_n?, constituent_universe?, eodhd_cap?)` | `strategies.sector_screener` | market | Sector-first rotation screen: regime cap + multi-factor SPDR rank + RRG quadrant + pullback-divergence leaders + dispersion trend (+ breadth n\<20-gated and EW/CW vs the real RSP* equal-weight index/own-50d-SMA + Setup A-B with breadth) — advisory |
