...
| `enable_mp_lower_spectrum` | `TRADINGAGENTS_ENABLE_MP_LOWER_SPECTRUM` | counts the panel correlation eigenvalues below the Marchenko-Pastur lower bound - a collapse in the effective number of independent bets as a count rather than a story. **Not yet visible anywhere:** with the gate on the read is attached to `market_breadth`'s dict as `mp_lower_spectrum`, but no report block prints it and no consumer reads the key, so turning the gate on currently changes no report (the read is computed and dropped) | `tradingagents/strategies/sector_breadth.py`::`mp_lower_spectrum` (the only reader of the gate; `None` when off), attached by `tradingagents/strategies/market_breadth.py`::`market_breadth`. `_market_breadth_read` (the engine's call site) does not pass `cfg`, so from a run the gate cannot even be on | `test_strategies_market_breadth.py` | wired |
