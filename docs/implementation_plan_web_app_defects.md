...
| W-P1-7 and W-P1-10 (both unassigned in §6) | **done** | W-P1-7: per-IP lockout dimension, bounded map, credential length bounds (+3 gates). W-P1-10: a credential is written to the file that DEFINES the key (the engine `.env` when that is the winner), the effective source per key is reported to the UI, newline values are 400 (+3 gates). |
