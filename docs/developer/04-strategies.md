...
- `volatility_models.py` — Parkinson / Garman-Klass / Yang-Zhang (scalar +
  per-bar series) / EWMA / GARCH(1,1) volatility estimators, plus
  upside/downside semivariance (+ `volatility_estimator` overlay switch
  `close` default | ewma | garch).
