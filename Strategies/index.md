...
| 20 | `quants.md` + `quant2.md` + `Quant_Formulas_Implementation_Plan.md` | Quant formula map implemented: volatility estimators (Parkinson/GK/EWMA/GARCH + overlay switch), tail decomposition (incremental/component VaR), mean-reversion quality (AR(1)/OU half-life), Roll spread, preferred YTM/duration/DV01, credit hazard, variance-swap strike, implementation shortfall | `strategies/{volatility_models,mean_reversion,fixed_income,book_risk,credit_spread,options_math,evaluate}.py` + `scan/capital_income --fi` | `volatility_estimator` (default close) |
