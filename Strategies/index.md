...
| 28 | `strategies/formulas/sector_rotation.md` | Sector-rotation reference & design — distilled 2026-09-05 from two deleted LLM-generated specs (`sector_calc.md`, `sector_instruction.md`): staged Market→Sector→Industry→Stock→Entry→Sizing→Risk pipeline (regime is a gate, not a factor), reconciled 8-factor 100-pt sector score (25/15/15/15/10/10/5/5), percentile normalization, score≠signal grade table + regime cap, two-level universe (11 SPDR + industry ETFs — never ranked in one pool), constituent breadth + EW/CW leadership ratio as the genuinely new factors | **P1-P3 adopted** (`sector_rank.py`: `rank_sectors_multifactor` tie-aware cross-sectional percentiles over momentum composite/RS-vs-benchmark/trend/risk + `rank_industry_group` parent-gated + `constituent_breadth`/`leadership_ratio`; `get_sector_rank` renders gated advisory lines; tests `test_sector_rank.py` 24) | P1 `enable_sector_multifactor`, P2 `enable_sector_industry`, P3 `enable_sector_breadth` (all default off) |
| 29 | `docs/design_quant_formulas_research_round2.md` + `docs/implementation_plan_quant_formula_additions.md` | Round-2 quant-formula gap scan (2026-09-11), delta vs round 1: records which round-1 items landed (variance ratio, CUSUM/EWMA, entropy, Ohlson/Zmijewski, Dechow-Dichev, Taylor rule, MAX/IVOL, AC + TWAP/VWAP/POV, vol-target scale, Cornish-Fisher/Kappa/Burke/ruin) and adopts the nine still-absent, style-relevant formulas — quote-free spread estimators (Corwin-Schultz / Abdi-Ranaldo), minimum-CVaR/CDaR sizing under the existing 3.00% budget (+ copula joint-tail scenarios), conformalised valuation bands, gross profitability (Novy-Marx), net operating assets (Hirshleifer et al.), overnight-vs-intraday return decomposition, Loughran-McDonald tone + readability + call-vs-filing divergence, White Reality Check / Hansen SPA, BOCPD (optional; complements the landed CUSUM) | **Q1-Q8 landed** (2026-09-11): `liquidity_risk.corwin_schultz`/`abdi_ranaldo`/`spread_estimate` + `execution_schedule.default_temp_impact`; `book_risk.min_cvar_weights`/`copula_scenarios` (+ additive governor `sizing`); `strategies/conformal.py`; `quantitative_scores.gross_profitability`/`net_operating_assets` (+ screener columns); `market_session.decompose_returns`; `strategies/text_factors.py`; `evaluate.reality_check`/`spa`; `regime.bocpd`; tools in `agents/utils/quant_formula_tools.py` bound to market/fundamentals/news analysts + risk debators, each with a prompt citation rule | phase-gated, default-off (`enable_spread_estimator`, `enable_book_risk_sizing`, `enable_conformal_bands`, `enable_return_decomposition`, `enable_text_factors`, `enable_bocpd`) |
| 30 | `docs/design_quant_formulas_research_round3.md` + `docs/implementation_plan_quant_formula_additions_round3.md` | Round-3 scoring/sentiment-formula gap scan (2026-09-13), delta vs rounds 1-2 (which swept return-prediction and risk formulas): 25 sources on *score construction* — Piotroski/Altman/G-Score/C-Score, composite point systems (fffinstill, the higher-ed Composite Financial Index), FinBERT per-day aggregation, Berkeley confidence-weighted aggregation (`Σ s_i·conf_i`), Fidelity/AlphaSense/MarketGrader/Adanos crowd scores, the MSCI analyst revision index and the Baker-Wurgler sentiment index — ledgered against the repo's shipped scores (F/M/Z/O, GP/A, NOA, the per-day sentiment mean, LM tone, percentile composites, the score↔rating bands) and adopts the ten still-absent items: Altman Z'/Z''/Z''-EM variants + distress zones, the F-Score's paper basis + bands + applicability gate, G/C scores (Mohanram/Montier), weighted-vs-unweighted news aggregation (relevance as a labelled confidence proxy), crowd ratio + dispersion + display-only extreme bands, the MSCI revision-ratio index, an equal-weight winsorised composite quality score 0-100, score IC/decile/coverage/stability rows, a weighted rolling sentiment window, a conditional reduced-proxy BW-style index — plus one orchestration item, S11 symmetric evidence for the paired debate roles (a measured symmetry row, deterministic default args that shrink the model-supplied tool pool, a mirrored discretionary budget and a gated argument-plan call), added because the judge's comparable scores require comparable evidence | **landed 2026-09-13 (S1-S11)** — `dataflows/quantitative_scores.py` (S1/S2/S10), `strategies/{normalized,peer_universe,factors,cross_section,analyst_revisions,sentiment,alpha_health}.py`, `agents/utils/{quant_formula_tools,analyst_revision_tools,analysis_tools,evidence_gather}.py`, `scripts/value_screener.py` (G/C/Qual/RevIdx columns), `graph/setup.py` (pre-debate symmetry node) | the ten `enable_*` gates, all default-off |
